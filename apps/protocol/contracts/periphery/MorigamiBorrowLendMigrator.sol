...
// Morigami (periphery/MorigamiBorrowLendMigrator.sol)
 ...
 */
contract MorigamiBorrowLendMigrator is IMorigamiFlashLoanReceiver, MorigamiElevatedAccess {
    using SafeERC20 for IERC20;

    /// @notice The Morigami flashLoan provider contract, which may be via Aave/Spark/Balancer/etc
    IMorigamiFlashLoanProvider public immutable flashLoanProvider;

    /// @notice The old Morigami borrow lend contract
    IMorigamiBorrowAndLend public immutable oldBorrowLend;

    /// @notice The new Morigami borrow lend contract
    IMorigamiBorrowAndLend public immutable newBorrowLend;
    ...
    constructor(
        ...
        oldBorrowLend = IMorigamiBorrowAndLend(_oldBorrowLend);
        collateralToken = IERC20(oldBorrowLend.supplyToken());
        debtToken = IERC20(oldBorrowLend.borrowToken());

        newBorrowLend = IMorigamiBorrowAndLend(_newBorrowLend);
