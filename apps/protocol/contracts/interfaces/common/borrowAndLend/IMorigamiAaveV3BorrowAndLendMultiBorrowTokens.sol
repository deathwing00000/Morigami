...
// Origami (interfaces/common/borrowAndLend/IMorigamiAaveV3BorrowAndLendMultiBorrowTokens.sol)
 ...
 * a single `supplyToken` and a multiple `borrowTokens`.
 * This is an Aave V3 specific interface, borrowing using variable debt only
 */
interface IMorigamiAaveV3BorrowAndLendMultiBorrowTokens is
    IMorigamiBorrowAndLendMultiBorrowTokens
{
    ...
    /**
     * @notice The Aave/Spark rebasing variable debt tokens received when borrowing `debtToken`
     */
    function aaveDebtTokens() external view returns (IERC20Metadata[] memory);
