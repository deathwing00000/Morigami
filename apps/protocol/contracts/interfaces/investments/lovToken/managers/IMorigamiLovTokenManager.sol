...
// Morigami (interfaces/investments/lovToken/managers/IMorigamiLovTokenManager.sol)
 ...
 */
interface IMorigamiLovTokenManager is IMorigamiOTokenManager, IWhitelisted {
     ...
     */
    function liabilities(IMorigamiOracle.PriceType debtPriceType) external view returns (uint256);
     ...
     * @dev Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function assetsAndLiabilities(IMorigamiOracle.PriceType debtPriceType) external view returns (
        uint256 assets,
        uint256 liabilities,
        uint256 ratio
    );
     ...
     */
    function effectiveExposure(IMorigamiOracle.PriceType debtPriceType) external view returns (uint128);
     ...
     * Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function userRedeemableReserves(IMorigamiOracle.PriceType debtPriceType) external view returns (uint256);

    /**
     * @notice How many reserve tokens would one get given a number of lovToken shares
     * @dev Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function sharesToReserves(uint256 shares, IMorigamiOracle.PriceType debtPriceType) external view returns (uint256);

    /**
     * @notice How many lovToken shares would one get given a number of reserve tokens
     * @dev Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function reservesToShares(uint256 reserves, IMorigamiOracle.PriceType debtPriceType) external view returns (uint256);
}
