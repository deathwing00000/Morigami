...
// Morigami (interfaces/investments/lovToken/managers/IMorigamiLovTokenMorphoManager.sol)
 ...
 */
interface IMorigamiLovTokenMorphoManager is IMorigamiLovTokenManager {
     ...
     */
    function borrowLend() external view returns (IMorigamiMorphoBorrowAndLend);

    /**
     * @notice The oracle to convert `debtToken` <--> `reserveToken`
     */
    function debtTokenToReserveTokenOracle() external view returns (IMorigamiOracle);
     ...
     */
    function dynamicFeePriceOracle() external view returns (IMorigamiOracle);
}