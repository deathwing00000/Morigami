...
// Morigami (common/MorigamiErc4626.sol)
    ...
    IMorigamiErc4626
{
    ...
    function convertToShares(uint256 assets) public view virtual override returns (uint256) {
        return _convertToShares(assets, MorigamiMath.Rounding.ROUND_DOWN);
    }

    /// @inheritdoc IERC4626
    function convertToAssets(uint256 shares) public view virtual override returns (uint256) {
        return _convertToAssets(shares, MorigamiMath.Rounding.ROUND_DOWN);
    }
    ...
    function _maxDeposit(uint256 feeBps) internal view returns (uint256 maxAssets) {
        ...
        return _convertToAssets(
            availableShares.inverseSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_UP),
    ...
    function _maxWithdraw(
        ...
        (shares,) = shares.splitSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_DOWN);
        return _convertToAssets(shares, MorigamiMath.Rounding.ROUND_DOWN);
    }
    ...
    function _previewDeposit(uint256 assets, uint256 feeBps) internal virtual view returns (
        ...
        shares = _convertToShares(assets, MorigamiMath.Rounding.ROUND_DOWN);

        // Deposit fees are taken from the shares in kind
        (shares, shareFeesTaken) = shares.splitSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_DOWN);
    }
    ...
    function _previewMint(uint256 shares, uint256 feeBps) internal virtual view returns (
        ...
        uint256 sharesPlusFees = shares.inverseSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_UP);
        unchecked {
            shareFeesTaken = sharesPlusFees - shares;
        }

        assets = _convertToAssets(sharesPlusFees, MorigamiMath.Rounding.ROUND_UP);
    }
    ...
    function _previewWithdraw(uint256 assets, uint256 feeBps) internal view returns (
        ...
        uint256 sharesExcludingFees = _convertToShares(assets, MorigamiMath.Rounding.ROUND_UP);
        // Withdrawal fees are taken from the shares the user redeems
        // so calculate the amount of shares required before fees are taken.
        shares = sharesExcludingFees.inverseSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_UP);
        unchecked {
    ...
    function _previewRedeem(uint256 shares, uint256 feeBps) internal view returns (
        ...
        (shares, shareFeesTaken) = shares.splitSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_DOWN);
        assets = _convertToAssets(shares, MorigamiMath.Rounding.ROUND_DOWN);
    }

    /**
     * @dev Internal conversion function (from assets to shares) with support for rounding direction.
     */
    function _convertToShares(uint256 assets, MorigamiMath.Rounding rounding) internal view virtual returns (uint256) {
     ...
     */
    function _convertToAssets(uint256 shares, MorigamiMath.Rounding rounding) internal view virtual returns (uint256) {
