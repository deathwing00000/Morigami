...
// Morigami (common/swappers/MorigamiCowSwapper.sol)
 ...
 */
contract MorigamiCowSwapper is IMorigamiCowSwapper, MorigamiElevatedAccess {
    using SafeERC20 for IERC20; 
    using GPv2Order for GPv2Order.Data;
    using MorigamiMath for uint256;

    /// @inheritdoc IMorigamiCowSwapper
    ...
    function setOrderConfig(
        ...
        if (config.verifySlippageBps > MorigamiMath.BASIS_POINTS_DIVISOR) revert CommonEventsAndErrors.InvalidParam();
    ...
    function isValidSignature(bytes32 /*hash*/, bytes memory signature) external override view returns (bytes4) {
        ...
        uint256 orderBuyAmountWithSlippage = (verifySlippageBps > 0)
            ? order.buyAmount.addBps(verifySlippageBps, MorigamiMath.Rounding.ROUND_DOWN)
            : order.buyAmount;
    ...
    function supportsInterface(bytes4 interfaceId) external override pure returns (bool) {
        return interfaceId == type(IMorigamiCowSwapper).interfaceId
    ...
    function _getBuyAmount(
        ...
        IMorigamiOracle limitPriceOracle = config.limitPriceOracle;
        ...
        if (address(limitPriceOracle) != address(0)) {
            ...
            if (limitPricePremiumBps > 0) {
                unroundedBuyAmount = unroundedBuyAmount.addBps(config.limitPricePremiumBps, MorigamiMath.Rounding.ROUND_DOWN);
            }
        }
