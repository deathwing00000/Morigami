...
// Morigami (investments/MorigamiInvestmentVault.sol)
 ...
 */
contract MorigamiInvestmentVault is IMorigamiInvestmentVault, RepricingToken, Whitelisted, ReentrancyGuard {
    ...
    constructor(
        ...
        if (_performanceFee > MorigamiMath.BASIS_POINTS_DIVISOR) revert CommonEventsAndErrors.InvalidParam();
    ...
    function areInvestmentsPaused() external override view returns (bool) {
        return IMorigamiInvestment(reserveToken).areInvestmentsPaused();
    }
    ...
    function areExitsPaused() external override view returns (bool) {
        return IMorigamiInvestment(reserveToken).areExitsPaused();
    }
     ...
     * For the Morigami Investment Vault, this is the underlying MorigamiInvestment's acceptedInvestTokens()
     * Plus the reserve token.
     */
    function acceptedInvestTokens() external override view returns (address[] memory) {
        return appendReserveToken(IMorigamiInvestment(reserveToken).acceptedInvestTokens());
    }

    /**
     * @notice The set of accepted tokens which can be used to exit into.
     * For the Morigami Investment Vault, this is the underlying MorigamiInvestment's acceptedExitTokens()
     * Plus the reserve token.
     */
    function acceptedExitTokens() external override view returns (address[] memory) {
        return appendReserveToken(IMorigamiInvestment(reserveToken).acceptedExitTokens());
    }

    /**
     * @notice Get a quote to invest into the Morigami investment vault using one of the accepted tokens. 
    ...
    function investQuote(
        ...
        if (fromToken == _reserveToken) {
            ...
            (quoteData, investFeeBps) = IMorigamiInvestment(_reserveToken).investQuote(
                fromTokenAmount, fromToken, MorigamiMath.BASIS_POINTS_DIVISOR, deadline
            );
    ...
    /** 
      * @notice User invests into this Morigami investment vault with an amount of one of the approved ERC20 tokens. 
    ...
    function investWithToken(
        ...
        // Otherwise pull the fromToken and use to invest in the underlying Morigami Investment
        address _reserveToken = reserveToken;
        ...
        if (quoteData.fromToken == _reserveToken) {
            ...
            reservesAmount = IMorigamiInvestment(_reserveToken).investWithToken(underlyingQuoteData);
        }
    ...
    /** 
      * @notice User invests into this Morigami investment vault with an amount of native chain token (ETH/AVAX)
      * @param quoteData The quote data received from investQuote()
      * @return investmentAmount The actual number of this Morigami investment tokens received.
      */
    function investWithNative(
        ...
        uint256 reservesAmount = IMorigamiInvestment(reserveToken).investWithNative{value: msg.value}(
            underlyingQuoteData
        );
    ...
    /**
     * @notice Get a quote to exit out of this Morigami investment vault to receive one of the accepted tokens.
     * @dev The 0x0 address can be used for native chain ETH/AVAX
    ...
    function exitQuote(
        ...
        // and then get a quote from the underlying Morigami Investment for the number of toTokens to expect
        if (toToken == reserveToken) {
            ...
            (quoteData, exitFeeBps) = IMorigamiInvestment(reserveToken).exitQuote(
                expectedReserveAmount, toToken, MorigamiMath.BASIS_POINTS_DIVISOR, deadline
            );
    ...
    /** 
      * @notice Exit out of this Morigami investment vault to receive one of the accepted tokens.
      * @param quoteData The quote data received from exitQuote()
      * @param recipient The receiving address of the `toToken`
      * @return toTokenAmount The number of `toToken` tokens received upon selling the Morigami investment tokens.
      */
    function exitToToken(
        ...
        // Otherwise first redeem the reserve tokens and then exit the underlying Morigami investment
        if (quoteData.toToken == reserveToken) {
            ...
            toTokenAmount = IMorigamiInvestment(reserveToken).exitToToken(
                underlyingQuoteData,
                recipient
            );
    ...
    /** 
      * @notice Sell this Morigami investment to native ETH/AVAX.
      * @param quoteData The quote data received from exitQuote()
      * @param recipient The receiving address of the native chain token.
      * @return nativeAmount The number of native chain ETH/AVAX/etc tokens received upon selling the Morigami investment tokens.
      */
    function exitToNative(
        ...
        // Now exchange the reserve token to the actual token the user requested.
        nativeAmount = IMorigamiInvestment(reserveToken).exitToNative(
            underlyingQuoteData,
            recipient
        );
    ...
    function maxInvest(address fromToken) external override view returns (uint256 amount) {
        ...
        amount = fromToken == _reserveToken
            ? type(uint256).max
            : IMorigamiInvestment(_reserveToken).maxInvest(fromToken);
    }
    ...
    function maxExit(address toToken) external override view returns (uint256 amount) {
        ...
        amount = toToken == _reserveToken
            ? type(uint256).max
            : reservesToShares(
                IMorigamiInvestment(_reserveToken).maxExit(toToken)
              );
    }
}
