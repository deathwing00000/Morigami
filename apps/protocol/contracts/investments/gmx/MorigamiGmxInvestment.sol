...
// Morigami (investments/gmx/MorigamiGmxInvestment.sol)
...
contract MorigamiGmxInvestment is MorigamiInvestment {
    using SafeERC20 for IERC20;

    /// @notice The Morigami contract managing the holdings of GMX and derived esGMX/mult point rewards
    IMorigamiGmxManager public origamiGmxManager;

    event MorigamiGmxManagerSet(address indexed origamiGmxManager);
    
    constructor(
        address _initialOwner
    ) MorigamiInvestment("Morigami GMX Token", "oGMX", _initialOwner) {}

    /// @notice Set the Morigami GMX Manager contract used to apply GMX to earn rewards.
    function setMorigamiGmxManager(address _origamiGmxManager) external onlyElevatedAccess {
        if (_origamiGmxManager == address(0)) revert CommonEventsAndErrors.InvalidAddress(address(0));
        emit MorigamiGmxManagerSet(_origamiGmxManager);
        origamiGmxManager = IMorigamiGmxManager(_origamiGmxManager);
    }
