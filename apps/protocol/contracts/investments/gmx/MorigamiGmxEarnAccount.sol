...
// Morigami (investments/gmx/MorigamiGmxEarnAccount.sol)
...
import { IGmxRewardTracker } from "contracts/interfaces/external/gmx/IGmxRewardTracker.sol";
import { IGmxRewardDistributor } from "contracts/interfaces/external/gmx/IGmxRewardDistributor.sol";
import { IGmxVester } from "contracts/interfaces/external/gmx/IGmxVester.sol";
...
/// @title Morigami's account used for earning rewards for staking GMX/GLP 
/// @notice The Morigami contract responsible for managing GMX/GLP staking and harvesting/compounding rewards.
...
/// @dev The Owner will be the Morigami Multisig, and only owner is able to upgrade.
/// Elevated Access will be granted to the MorigamiGmxManager and MorigamiGmxLocker/MorigamiGlpLocker
contract MorigamiGmxEarnAccount is IMorigamiGmxEarnAccount, Initializable, MorigamiElevatedAccessUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _initialOwner, address _gmxRewardRouter, address _glpRewardRouter, address _esGmxVester, address _stakedGlp) initializer external {
        __MorigamiElevatedAccess_init(_initialOwner);
    ...
    function stakeGmx(uint256 _amount) external override onlyElevatedAccess {
    ...
    function unstakeGmx(uint256 _amount) external override onlyElevatedAccess {
    ...
    function stakeEsGmx(uint256 _amount) external onlyElevatedAccess {
    ...
    /// @dev This will burn any aggregated multiplier points, so should be avoided where possible.
    function unstakeEsGmx(uint256 _amount) external onlyElevatedAccess {
    ...
    function mintAndStakeGlp(uint256 fromAmount, address fromToken, uint256 minUsdg, uint256 minGlp) external override onlyElevatedAccess returns (uint256) {
    ...
    /// @dev GMX.io takes fees dependent on the pool constituents.
    function unstakeAndRedeemGlp(uint256 glpAmount, address toToken, uint256 minOut, address receiver) external override onlyElevatedAccess returns (uint256) {
    ...
    function transferStakedGlp(uint256 glpAmount, address receiver) external override onlyElevatedAccess {
    ...
    function transferStakedGlpOrPause(uint256 glpAmount, address receiver) external override onlyElevatedAccess {
    ...
    /// for Morigami's share of the upstream GMX.io rewards.
    ...
    /// @notice The amount of $esGMX and $Native (ETH/AVAX) which are claimable by Morigami as of now
    ...
    /// @dev only the MorigamiGmxManager can call since we need to track and action based on the amounts harvested.
    function harvestRewards(uint256 _esGmxVestingRate) external onlyElevatedAccess override returns (
        ...
        claimedRewards = _handleGmxRewards(
            HandleGmxRewardParams({
                ...
                shouldStakeGmx: false, /* The MorigamiGmxManager will decide where to stake the vested GMX */
        ...
        uint256 esGmxVesting;   
        {
            ...
            if (totalEsGmxClaimed != 0) {
                ...
                (esGmxReinvested, esGmxVesting) = totalEsGmxClaimed.splitSubtractBps(
                    _esGmxVestingRate, 
    ...
    function handleRewards(HandleGmxRewardParams memory params) external override onlyElevatedAccess returns (ClaimedRewards memory claimedRewards) {
    ...
    function depositIntoEsGmxVesting(address _esGmxVester, uint256 _amount) external onlyElevatedAccess {
    ...
    function withdrawFromEsGmxVesting(address _esGmxVester) external onlyElevatedAccess {
        IGmxVester(_esGmxVester).withdraw();
    }

    /// @dev Morigamis share of the underlying GMX reward distributor's total 
    /// rewards per second
