...
// Morigami (investments/gmx/MorigamiGlpInvestment.sol)
...
contract MorigamiGlpInvestment is MorigamiInvestment {
    using SafeERC20 for IERC20;
    using Address for address payable;

    /// @notice The ETH (Arbitrum) or AVAX (Avalanche) native token which can be used for buying and selling the Morigami receipt token
    address public immutable wrappedNativeToken;

    /// @notice The Morigami contract managing the holdings of GLP and derived GMX/esGMX/mult point rewards
    IMorigamiGmxManager public origamiGlpManager;

    error InvalidSender(address caller);
    event MorigamiGlpManagerSet(address indexed origamiGlpManager);

    constructor(
        address _initialOwner,
        address _wrappedNativeToken
    ) MorigamiInvestment("Morigami GLP Token", "oGLP", _initialOwner) {
    ...
    /// @notice Set the Morigami GLP Manager contract used to apply GLP to earn rewards.
    function setMorigamiGlpManager(address _origamiGlpManager) external onlyElevatedAccess {
        if (_origamiGlpManager == address(0)) revert CommonEventsAndErrors.InvalidAddress(address(0));
        emit MorigamiGlpManagerSet(_origamiGlpManager);
        origamiGlpManager = IMorigamiGmxManager(_origamiGlpManager);
    }
    ...
    /** 
      ...
      * @return toTokenAmount The number of `toToken` tokens received upon selling the Morigami receipt token.
      */
    ...
    /** 
      ...
      * @return nativeAmount The number of native chain ETH/AVAX/etc tokens received upon selling the Morigami receipt token.
      */
