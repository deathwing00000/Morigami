...
// Morigami (investments/sky/MorigamiSuperSavingsUsdsManager.sol)
...
import { IMorigamiDelegated4626VaultManager } from "contracts/interfaces/investments/erc4626/IMorigamiDelegated4626VaultManager.sol";
...
/**
 * @title Morigami sUSDS+s Manager
 * @notice Handles USDS deposits and switching between farms
 */
contract MorigamiSuperSavingsUsdsManager is 
    IMorigamiSuperSavingsUsdsManager,
    ...
    ReentrancyGuard
{
    ...
    /// @inheritdoc IMorigamiDelegated4626VaultManager
    IMorigamiDelegated4626Vault public immutable override vault;

    /// @inheritdoc IMorigamiSuperSavingsUsdsManager
    ...
    uint16 private _performanceFeeBpsForMorigami;
    ...
    constructor(
        ...
        uint16 performanceFeeBpsForMorigami_
    ) 
        MorigamiElevatedAccess(initialOwner_)
    {
        vault = IMorigamiDelegated4626Vault(vault_);
        ...
        if (performanceFeeBpsForCaller_ + performanceFeeBpsForMorigami_ > MorigamiMath.BASIS_POINTS_DIVISOR) {
            revert CommonEventsAndErrors.InvalidParam();
        }
        _performanceFeeBpsForCaller = performanceFeeBpsForCaller_;
        _performanceFeeBpsForMorigami = performanceFeeBpsForMorigami_;
    ...
    function setPerformanceFees(uint16 callerFeeBps, uint16 origamiFeeBps) external override onlyElevatedAccess {
        ...
        uint16 existingTotalFee = _performanceFeeBpsForCaller + _performanceFeeBpsForMorigami;
        ...
        MorigamiSuperSavingsUsdsVault(address(vault)).logPerformanceFeesSet(newTotalFee);
        _performanceFeeBpsForCaller = callerFeeBps;
        _performanceFeeBpsForMorigami = origamiFeeBps;
    }
    ...
    /// @inheritdoc IMorigamiSuperSavingsUsdsManager
    function performanceFeeBps() external view returns (uint16 /*forCaller*/, uint16 /*forMorigami*/) {
        return (_performanceFeeBpsForCaller, _performanceFeeBpsForMorigami);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public override pure returns (bool) {
        return interfaceId == type(IMorigamiSuperSavingsUsdsManager).interfaceId
    ...
    function _transferRewards(
        ...
        amountForMorigami = totalRewardsClaimed.mulDiv(
            cache.feeBpsForMorigami, 
            MorigamiMath.BASIS_POINTS_DIVISOR, 
            MorigamiMath.Rounding.ROUND_DOWN
        );
        if (amountForMorigami > 0) {
            rewardsToken.safeTransfer(cache.feeCollector, amountForMorigami);
        }

        // The remainder is for the vault        
        unchecked {
            amountForVault = totalRewardsClaimed - amountForCaller - amountForMorigami;
        }
        if (amountForVault > 0) {
    ...
    function _populateRewardsCache(
        ...
        return HarvestRewardCache({
            ...
            feeBpsForMorigami: _performanceFeeBpsForMorigami
        });
    }
