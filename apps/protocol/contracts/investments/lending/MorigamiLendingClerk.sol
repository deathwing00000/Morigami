...
// Morigami (investments/lending/MorigamiLendingClerk.sol)
...
import { IMorigamiOToken } from "contracts/interfaces/investments/IMorigamiOToken.sol";
import { IMorigamiLendingClerk } from "contracts/interfaces/investments/lending/IMorigamiLendingClerk.sol";
import { IMorigamiIdleStrategyManager } from "contracts/interfaces/investments/lending/idleStrategy/IMorigamiIdleStrategyManager.sol";
 ...
 */
contract MorigamiLendingClerk is IMorigamiLendingClerk, MorigamiElevatedAccess {
    using SafeERC20 for IERC20Metadata;
    using SafeERC20 for IMorigamiDebtToken;
    ...
    /**
     * @notice The Morigami oToken which supplies the asset
     */
    IMorigamiOToken public immutable override oToken;

    /**
     * @notice Where idle funds (not yet borrowed) are deposited.
     */
    IMorigamiIdleStrategyManager public immutable override idleStrategyManager;
    ...
    constructor(
        ...
        oToken = IMorigamiOToken(_oToken);
        idleStrategyManager = IMorigamiIdleStrategyManager(_idleStrategyManager);
    ...
    function addBorrower(
        ...
        IMorigamiLendingBorrower _borrower = IMorigamiLendingBorrower(borrower);
        emit BorrowerAdded(
            borrower, 
            interestRateModel, 
            _borrower.name(),
    ...
    function repay(uint256 amount, address borrower) external override returns (uint256 amountRepaid) {
        ...
        uint256 _maxRepayAmount = _debtBalance.scaleDown(_assetScalar, MorigamiMath.Rounding.ROUND_UP);   // asset's dp
    ...
    function borrowerBalanceSheet(address borrower) external override view returns (
        ...
        if (borrower != address(idleStrategyManager)) {
            if (!_borrowersSet.contains(borrower)) {
                return (new IMorigamiLendingBorrower.AssetBalance[](0), 0);
            }
        }
    ...
    function _globalAvailableToBorrow() private view returns (uint256) {
        ...
        return _totalBorrowerCeiling > _totalBorrowerDebt 
            ? (_totalBorrowerCeiling - _totalBorrowerDebt).scaleDown(_assetScalar, MorigamiMath.Rounding.ROUND_DOWN)
            : 0;
    }
        ...
        uint256 _borrowerAmount;   // asset's dp
        {
            ...
            unchecked {
                _borrowerAmount = _borrowerDebtCeiling > _borrowerDebtBalance
                    // This scaleDown intentionally rounds down (so it's not in the borrower's benefit)
                    ? (_borrowerDebtCeiling - _borrowerDebtBalance).scaleDown(_assetScalar, MorigamiMath.Rounding.ROUND_DOWN)
                    : 0;
            }
        }
