...
// Morigami (investments/lending/MorigamiLendingManager.sol)
...
import { IMorigamiLendingSupplyManager } from "contracts/interfaces/investments/lending/IMorigamiLendingSupplyManager.sol";
 ...
 */
contract MorigamiLendingSupplyManager is IMorigamiLendingSupplyManager, MorigamiElevatedAccess, MorigamiManagerPausable, Whitelisted {
     ...
     */
    IMorigamiCircuitBreakerProxy public immutable override circuitBreakerProxy;
    ...
    constructor(
        ...
        oToken = _oToken;
        ovToken = _ovToken;
        circuitBreakerProxy = IMorigamiCircuitBreakerProxy(_circuitBreakerProxy);
    ...
    function setExitFeeBps(uint96 feeBps) external onlyElevatedAccess {
        if (feeBps > MorigamiMath.BASIS_POINTS_DIVISOR) revert CommonEventsAndErrors.InvalidParam();
    ...
    function exitToToken(
        ...
        if (nonFees != 0) {
            ...
            toTokenAmount = nonFees.scaleDown(_assetScalar, MorigamiMath.Rounding.ROUND_DOWN);
            if (toTokenAmount != 0) {
     ...
     * @param deadline The maximum deadline to execute the exit.
     * @return quoteData The quote data, including any params required for the underlying investment type.
     * @return _exitFeeBps Any fees expected when exiting the investment to the nominated token, either from Morigami or from the underlying investment.
     */
    function exitQuote(
        uint256 investmentAmount,
        address toToken,
        ...
        uint256 toExitAmount = investmentAmount.subtractBps(_feeBps, MorigamiMath.Rounding.ROUND_DOWN);

        // This scaleDown intentionally rounds down (so it's not in the user's benefit)
        uint256 amountOut = toExitAmount.scaleDown(_assetScalar, MorigamiMath.Rounding.ROUND_DOWN);
    ...
    function maxExit(address toToken) external override view returns (uint256 amount) {
        if (toToken == address(asset)) {
            ...
            amount = amount.inverseSubtractBps(exitFeeBps, MorigamiMath.Rounding.ROUND_DOWN);
        }
    }
}
