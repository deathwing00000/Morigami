...
// Morigami (investments/MorigamiOTokenWithNative.sol)
...
/// @title Morigami oToken (with native ETH support for deposits/exits)
///
...
contract MorigamiOTokenWithNative is MorigamiOToken {
    ...
    ) MorigamiOToken(_initialOwner, _name, _symbol) {
    ...
    function investWithNative(
        ...
        investmentAmount = IMorigamiOTokenManagerWithNative(_managerAddr).investWithWrappedNative(quoteData);
    ...
    function exitToNative(
        ...
        (nativeAmount, oTokenToBurn) = IMorigamiOTokenManagerWithNative(_managerAddr).exitToWrappedNative(quoteData, address(this));
