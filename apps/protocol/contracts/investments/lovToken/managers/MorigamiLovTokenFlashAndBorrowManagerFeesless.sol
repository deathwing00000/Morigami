...
/**
 * @title Morigami LovToken Flash And Borrow Manager
    ...
    MorigamiAbstractLovTokenManager
{
     ...
     */
    IMorigamiBorrowAndLend public borrowLend;

    /**
     * @notice The Morigami flashLoan provider contract, which may be via Aave/Spark/Balancer/etc
     */
    IMorigamiFlashLoanProvider public override flashLoanProvider;
     ...
     */
    IMorigamiOracle public override debtTokenToReserveTokenOracle;
    ...
    constructor(
        ...
        flashLoanProvider = IMorigamiFlashLoanProvider(_flashLoanProvider);
        borrowLend = IMorigamiBorrowAndLend(_borrowLend);
    }
    ...
    function setSwapper(address _swapper) external override onlyElevatedAccess {
        if (_swapper == address(0))
            revert CommonEventsAndErrors.InvalidAddress(_swapper);
    ...
    function setOracles(
        ...
        emit OraclesSet(_debtTokenToReserveTokenOracle, address(0));
    }
    ...
    function setFlashLoanProvider(
        ...
        flashLoanProvider = IMorigamiFlashLoanProvider(provider);
    ...
    function setBorrowLend(
        ...
        if (_address == address(0))
            revert CommonEventsAndErrors.InvalidAddress(address(0));
        borrowLend = IMorigamiBorrowAndLend(_address);
    ...
    function reservesBalance()
        public
        view
        override(MorigamiAbstractLovTokenManager, IMorigamiLovTokenManager)
        returns (uint256)
    {
        ...
        returns (uint256)
    {
        ...
        return
            debtTokenToReserveTokenOracle.convertAmount(
                address(_debtToken),
                debt,
                debtPriceType,
    ...
    /**
     * @notice Invoked from IMorigamiFlashLoanProvider once a flash loan is successfully
    ...
    function flashLoanCallback(
        ...
        if (address(token) != address(_debtToken))
        ...
        if (_rebalanceType == RebalanceCallbackType.REBALANCE_DOWN) {
            ...
            _rebalanceDownFlashLoanCallback(amount, fee, _rdParams, force);
        } else {
            RebalanceUpParams memory _ruParams = abi.decode(
                _rebalanceParams,
                (RebalanceUpParams)
            );
            _rebalanceUpFlashLoanCallback(amount, fee, _ruParams, force);
        }
    ...
    function _rebalanceUpFlashLoanCallback(
        ...
        if (flashLoanAmount != params.flashLoanAmount)
        ...
        IMorigamiBorrowAndLend _borrowLend = borrowLend;
        ...
        // Repay the [debtToken]
        {
            ...
            // be the exact `amount`
            (uint256 amountRepaid, uint256 withdrawnAmount) = _borrowLend
                .repayAndWithdraw(
                    flashLoanAmount,
        ...
        {
            uint256 debtTokenReceived = swapper.execute(
                _reserveToken,
                params.collateralToWithdraw,
                _debtToken,
                params.swapData
        ...
        {
            uint256 surplusAfterSwap = _debtToken.balanceOf(address(this)) -
                flashRepayAmount;
            uint256 borrowLendSurplus = _debtToken.balanceOf(
                address(_borrowLend)
            );
            ...
            if (totalSurplus > params.repaySurplusThreshold) {
                ...
                totalDebtRepaid =
                    totalDebtRepaid +
                    _borrowLend.repay(totalSurplus);
            }
        }
    ...
    function _rebalanceDownFlashLoanCallback(
        ...
        uint256 collateralSupplied = swapper.execute(
            _debtToken,
            flashLoanAmount,
            _reserveToken,
            params.swapData
     ...
     * In the feesless mode, there is no deposit fee
     */
     ...
     * In the feesless mode, there is no exit fee
     */
