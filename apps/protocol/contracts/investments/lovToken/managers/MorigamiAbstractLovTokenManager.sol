...
// Morigami (investments/lovToken/managers/MorigamiAbstractLovTokenManager.sol)
 ...
 */
abstract contract MorigamiAbstractLovTokenManager is IMorigamiLovTokenManager, MorigamiElevatedAccess, MorigamiManagerPausable, Whitelisted {
    ...
    function setFeeConfig(
        ...
        if (minDepositFeeBps > MorigamiMath.BASIS_POINTS_DIVISOR) revert CommonEventsAndErrors.InvalidParam();
        if (minExitFeeBps > MorigamiMath.BASIS_POINTS_DIVISOR) revert CommonEventsAndErrors.InvalidParam();
    ...
    function investWithToken(
        ...
        Cache memory cache = populateCache(IMorigamiOracle.PriceType.SPOT_PRICE);
        ...
        (investmentAmount, feeAmount) = investmentAmount.splitSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_DOWN);
        ...
        if (cache.liabilities != 0) {
            uint128 newAL = refreshCacheAL(cache, IMorigamiOracle.PriceType.SPOT_PRICE);
    ...
    function exitToToken(
        ...
        (uint256 reservesAmount, uint256 feeAmount) = toBurnAmount.splitSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_DOWN);
    ...
    function investQuote(
        ...
        _investmentAmount = _investmentAmount.subtractBps(_depositFeeRate, MorigamiMath.Rounding.ROUND_DOWN);
        ...
        quoteData.minInvestmentAmount = _investmentAmount.subtractBps(maxSlippageBps, MorigamiMath.Rounding.ROUND_UP);
    ...
    function exitQuote(
        ...
        uint256 toExitAmount = investmentAmount.subtractBps(_exitFeeRate, MorigamiMath.Rounding.ROUND_DOWN);
        ...
        quoteData.minToTokenAmount = toExitAmount.subtractBps(maxSlippageBps, MorigamiMath.Rounding.ROUND_UP);
        // quoteData.underlyingInvestmentQuoteData remains as bytes(0)
    ...
    function maxExit(address toToken) external override view returns (uint256 sharesAmount) {
        ...
        if (cache.assets > _minReserves) {
            ...
            sharesAmount = sharesAmount.inverseSubtractBps(_dynamicExitFeeBps(), MorigamiMath.Rounding.ROUND_DOWN);
     ...
     */
    function liabilities(IMorigamiOracle.PriceType debtPriceType) public virtual override view returns (uint256);
    ...
    function assetToLiabilityRatio() external override view returns (uint128) {
        return _assetToLiabilityRatio(populateCache(IMorigamiOracle.PriceType.SPOT_PRICE));
    }
     ...
     * @dev Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function assetsAndLiabilities(IMorigamiOracle.PriceType debtPriceType) external override view returns (
        uint256 /*assets*/,
        uint256 /*liabilities*/,
        uint256 /*ratio*/
    ) {
     ...
     */
    function effectiveExposure(IMorigamiOracle.PriceType debtPriceType) external override view returns (uint128) {
        Cache memory cache = populateCache(debtPriceType);
        if (cache.assets > cache.liabilities) {
            ...
            uint256 ee = cache.assets.mulDiv(PRECISION, redeemableReserves, MorigamiMath.Rounding.ROUND_UP);
     ...
     * Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function userRedeemableReserves(IMorigamiOracle.PriceType debtPriceType) external override view returns (uint256) {
     ...
     * @dev Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function sharesToReserves(uint256 shares, IMorigamiOracle.PriceType debtPriceType) external override view returns (uint256) {
     ...
     * @dev Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function reservesToShares(uint256 reserves, IMorigamiOracle.PriceType debtPriceType) external override view returns (uint256) {
    ...
    function populateCache(IMorigamiOracle.PriceType debtPriceType) internal view returns (Cache memory cache) {
        cache.assets = reservesBalance();
        cache.liabilities = liabilities(debtPriceType);
        cache.totalSupply = lovToken.totalSupply();
    }

    function refreshCacheAL(Cache memory cache, IMorigamiOracle.PriceType debtPriceType) internal view returns (uint128) {
        cache.assets = reservesBalance();
        cache.liabilities = liabilities(debtPriceType);
    ...
    function _assetToLiabilityRatio(Cache memory cache) internal pure returns (uint128) {
        if (cache.liabilities != 0) {
            ...
            uint256 alr = cache.assets.mulDiv(PRECISION, cache.liabilities, MorigamiMath.Rounding.ROUND_DOWN);
            if (alr < MAX_AL_RATIO) {
                return uint128(alr);
            }
        }
    ...
    function _sharesToReserves(Cache memory cache, uint256 shares) internal view returns (uint256) {
        ...
        return cache.totalSupply == 0
            ? shares.scaleDown(_reservesToSharesScalar(), MorigamiMath.Rounding.ROUND_DOWN)
            : shares.mulDiv(_userRedeemableReserves(cache), cache.totalSupply, MorigamiMath.Rounding.ROUND_DOWN);
    }
    ...
    function _reservesToShares(Cache memory cache, uint256 reserves) private view returns (uint256) {
        ...
        return reserves.mulDiv(cache.totalSupply, _redeemableReserves, MorigamiMath.Rounding.ROUND_DOWN);
    }
    ...
    function _validateAfterRebalance(
        ...
        alRatioAfter = refreshCacheAL(cache, IMorigamiOracle.PriceType.SPOT_PRICE);
    ...
    function _reservesCapacityFromTotalSupply(Cache memory cache) internal view returns (uint256) {
        ...
        _availableShares = _availableShares.inverseSubtractBps(_dynamicDepositFeeBps(), MorigamiMath.Rounding.ROUND_DOWN);
