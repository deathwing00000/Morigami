...
// Morigami (investments/lovToken/managers/MorigamiLovTokenMorphoManagerMarketAL.sol)
...
/**
 * @title Morigami LovToken Manager, for use with Morpho markets (with A/L conversion)
 * @notice The same as `MorigamiLovTokenMorphoManager`, however the `userALRange` and `rebalanceALRange`
 ...
 */
contract MorigamiLovTokenMorphoManagerMarketAL is MorigamiLovTokenMorphoManager {
    using SafeCast for uint256;

    event MorphoALToMarketALOracleSet(address indexed morphoALToMarketALOracle);

    /**
     * @notice An Morigami oracle to convert a 'Morpho' priced A/L into 'market' priced A/L terms
     * @dev For example Pendle PT tokens may be valued at maturity (1:1 to underlying) in Morpho
     * but within Morigami we want to use the market price.
     */
    address public morphoALToMarketALOracle;

    /// @dev The Morigami oracle precision is a fixed 18dp
    ...
    /**
     * @notice Set an Morigami oracle to convert a 'Morpho' priced A/L into 'market' priced A/L terms
     */
    ...
    function convertedAL(uint128 al, Cache memory cache) internal override view returns (uint128) {
        ...
        if (cache.implData == 0) {
            cache.implData = IMorigamiOracle(morphoALToMarketALOracle).latestPrice(
