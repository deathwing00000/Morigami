...
// Morigami (investments/lovToken/managers/MorigamiLovTokenErc4626Manager.sol)
...
import { MorigamiAbstractLovTokenManager } from "contracts/investments/lovToken/managers/MorigamiAbstractLovTokenManager.sol";
 ...
 */
contract MorigamiLovTokenErc4626Manager is IMorigamiLovTokenErc4626Manager, MorigamiAbstractLovTokenManager {
     ...
     */
    IMorigamiLendingClerk public override lendingClerk;

    /**
     * @notice The swapper for `debtAsset` <--> `depositAsset`
     */
    IMorigamiSwapper public override swapper;

    /**
     * @notice The oracle to convert `debtAsset` <--> `depositAsset`
     */
    IMorigamiOracle public debtAssetToDepositAssetOracle;
    ...
    constructor(
        ...
        address _lovToken
    ) MorigamiAbstractLovTokenManager(_initialOwner, _lovToken) {
    ...
    function setSwapper(address _swapper) external override onlyElevatedAccess {
        ...
        swapper = IMorigamiSwapper(_swapper);
    }
    ...
    function setOracle(address oracle) external override onlyElevatedAccess {
        ...
        IMorigamiOracle _debtAssetToDepositAssetOracle = IMorigamiOracle(oracle);
     ...
     */
    function reservesBalance() public override(MorigamiAbstractLovTokenManager,IMorigamiLovTokenManager) view returns (uint256) {
     ...
     */
    function reserveToken() public override(MorigamiAbstractLovTokenManager,IMorigamiLovTokenManager) view returns (address) {
        return address(_reserveToken);
    }
    ...
    /**
     * @notice The debt of the lovToken to the Morigami `lendingClerk`, converted into the `reserveToken`
     * @dev Use the Oracle `debtPriceType` to value any debt in terms of the reserve token
     */
    function liabilities(IMorigamiOracle.PriceType debtPriceType) public override(MorigamiAbstractLovTokenManager,IMorigamiLovTokenManager) view returns (uint256) {
     ...
     */
    function latestAssetBalances() public virtual override view returns (IMorigamiLendingBorrower.AssetBalance[] memory assetBalances) {
        assetBalances = new IMorigamiLendingBorrower.AssetBalance[](1);
        assetBalances[0] = IMorigamiLendingBorrower.AssetBalance(reserveToken(), _internalReservesBalance);
    }
