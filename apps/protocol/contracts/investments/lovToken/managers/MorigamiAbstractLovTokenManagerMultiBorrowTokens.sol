...
// Morigami (investments/lovToken/managers/MorigamiAbstractLovTokenManagerMultiBorrowTokens.sol)
...
/**
 * @title Abstract Morigami lovToken Manager
 ...
 */
abstract contract MorigamiAbstractLovTokenManagerMultiBorrowTokens is
    ...
    Whitelisted
{
     ...
     */
    IMorigamiLovToken public immutable override lovToken;
    ...
    constructor(
        ...
        lovToken = IMorigamiLovToken(_lovToken);
    }
    ...
    function setFeeConfig(
        ...
        if (minDepositFeeBps > MorigamiMath.BASIS_POINTS_DIVISOR)
            revert CommonEventsAndErrors.InvalidParam();
        if (minExitFeeBps > MorigamiMath.BASIS_POINTS_DIVISOR)
            revert CommonEventsAndErrors.InvalidParam();
        ...
        returns (uint256 investmentAmount)
    {
        ...
        uint256 newReservesAmount = _depositIntoReserves(
            quoteData.fromToken,
        ...
        (investmentAmount, feeAmount) = investmentAmount.splitSubtractBps(
            feeBps,
        ...
        if (investmentAmount < quoteData.minInvestmentAmount) {
            revert CommonEventsAndErrors.Slippage(
    ...
    function exitToToken(
        ...
        override
        onlyLovToken
        returns (uint256 toTokenAmount, uint256 toBurnAmount)
    {
        ...
        (uint256 reservesAmount, uint256 feeAmount) = toBurnAmount
            .splitSubtractBps(feeBps, MorigamiMath.Rounding.ROUND_DOWN);
        ...
        toTokenAmount = _redeemFromReserves(
            reservesAmount,
            quoteData.toToken,
            recipient
        );
    ...
    /**
     * @notice Get a quote to buy this Morigami investment using one of the accepted tokens.
        ...
        )
    {
        ...
        uint256 _newReservesAmount = _previewDepositIntoReserves(
            fromToken,
            fromTokenAmount
        );
        ...
        // However not all of these shares are minted and given to the user -- the deposit fee is removed
        uint256 _investmentAmount = _reservesToShares(
            cache,
            _newReservesAmount
        );
        uint256 _depositFeeRate = _dynamicDepositFeeBps();
        _investmentAmount = _investmentAmount.subtractBps(
            _depositFeeRate,
            MorigamiMath.Rounding.ROUND_DOWN
        );
        ...
        quoteData.minInvestmentAmount = _investmentAmount.subtractBps(
            maxSlippageBps,
    ...
    function maxInvest(
        address fromToken
    ) external view override returns (uint256 fromTokenAmount) {
        Cache memory cache = populateCache(
            IMorigamiOracle.PriceType.SPOT_PRICE
        );
        ...
        uint256 _remainingCapacityForAlCeiling = _reservesCapacityFromAlCeiling(
            cache
        );
        ...
        if (_minRemainingCapacity < type(uint256).max) {
            _minRemainingCapacity = _previewMintReserves(
                fromToken,
    ...
    /**
     * @notice Get a quote to sell this Morigami investment to receive one of the accepted tokens.
     * @param investmentAmount The number of Morigami investment tokens to sell
        ...
        )
    {
        ...
        uint256 toExitAmount = investmentAmount.subtractBps(
            _exitFeeRate,
            MorigamiMath.Rounding.ROUND_DOWN
        );
        ...
        quoteData.minToTokenAmount = toExitAmount.subtractBps(
            maxSlippageBps,
            MorigamiMath.Rounding.ROUND_UP
        );
        // quoteData.underlyingInvestmentQuoteData remains as bytes(0)
    ...
    function maxExit(
        ...
        if (cache.assets > _minReserves) {
            ...
            sharesAmount = _reservesToShares(
                cache,
                _amountFromAvailableCapacity
            );
            ...
            sharesAmount = sharesAmount.inverseSubtractBps(
                _dynamicExitFeeBps(),
                MorigamiMath.Rounding.ROUND_DOWN
            );
     ...
     */
    function reserveToken() public view virtual override returns (address);
     ...
     */
    function reservesBalance() public view virtual override returns (uint256);
     ...
     */
    function assetToLiabilityRatio() external view override returns (uint128) {
        return
            _assetToLiabilityRatio(
                populateCache(IMorigamiOracle.PriceType.SPOT_PRICE)
            );
    }
        ...
        returns (uint256 /*assets*/, uint256 /*liabilities*/, uint256 /*ratio*/)
    {
        ...
        return (cache.assets, cache.liabilities, _assetToLiabilityRatio(cache));
    }
    ...
    function effectiveExposure(
        ...
        if (cache.assets > cache.liabilities) {
            ...
            uint256 ee = cache.assets.mulDiv(
                PRECISION,
                redeemableReserves,
                MorigamiMath.Rounding.ROUND_UP
            );
     ...
     * @dev represented in basis points
     */
    function _dynamicDepositFeeBps() internal view virtual returns (uint256);
     ...
     * @dev represented in basis points
     */
    function _dynamicExitFeeBps() internal view virtual returns (uint256);
     ...
     */
    function _validateAlRange(Range.Data storage range) internal view virtual {}

    function _userRedeemableReserves(
        Cache memory cache
    ...
    function _assetToLiabilityRatio(
        Cache memory cache
    ) internal pure returns (uint128) {
        if (cache.liabilities != 0) {
            // Round down for A/L calc
            uint256 alr = cache.assets.mulDiv(
                PRECISION,
                cache.liabilities,
                MorigamiMath.Rounding.ROUND_DOWN
            );
    ...
    function _sharesToReserves(
        ...
        return
            cache.totalSupply == 0
                ? shares.scaleDown(
                    _reservesToSharesScalar(),
                    MorigamiMath.Rounding.ROUND_DOWN
                )
                : shares.mulDiv(
                    _userRedeemableReserves(cache),
                    cache.totalSupply,
                    MorigamiMath.Rounding.ROUND_DOWN
                );
    }

    function _reservesToShares(
        Cache memory cache,
        uint256 reserves
    ...
    function _reservesToSharesScalar() internal view returns (uint256) {
        ...
        if (_reservesDecimals > _sharesDecimals)
            revert CommonEventsAndErrors.InvalidToken(reserveToken());
    ...
    function _depositIntoReserves(
        address fromToken,
        uint256 fromTokenAmount
    ) internal virtual returns (uint256 newReservesAmount);
    ...
    function _previewDepositIntoReserves(
        address fromToken,
        uint256 fromTokenAmount
    ) internal view virtual returns (uint256 newReservesAmount);
    ...
    function _maxDepositIntoReserves(
        address fromToken
    ) internal view virtual returns (uint256 fromTokenAmount);
    ...
    ) internal view virtual returns (uint256 toTokenAmount);
    ...
    function _redeemFromReserves(
        ...
        address toToken,
        address recipient
    ) internal virtual returns (uint256 toTokenAmount);
    ...
    function _maxRedeemFromReserves(
        address toToken,
        Cache memory cache
    ) internal view virtual returns (uint256 reservesAmount);

    /**
     * @notice Validate that the A/L ratio hasn't moved beyond the given A/L range.
     */
    function _validateALRatio(
        Range.Data storage validRange,
        uint128 ratioBefore,
        uint128 ratioAfter,
        AlValidationMode alMode,
        Cache memory cache
    ) internal virtual {
        if (alMode == AlValidationMode.LOWER_THAN_BEFORE) {
            ...
            if (ratioAfter > ratioBefore)
                revert ALTooHigh(ratioBefore, ratioAfter, ratioBefore);
            ...
            if (ratioAfter < convertedAlFloor)
                revert ALTooLow(ratioBefore, ratioAfter, convertedAlFloor);
        } else {
            // A/L needs to be increasing (may be equal if a very small amount is deposited/exited)
            if (ratioAfter < ratioBefore)
                revert ALTooLow(ratioBefore, ratioAfter, ratioBefore);
            ...
            if (ratioAfter > convertedAlCeiling)
                revert ALTooHigh(ratioBefore, ratioAfter, convertedAlCeiling);
        }
    }
    ...
    function _validateAfterRebalance(
        ...
        // Ensure the A/L is within the expected slippage range
        {
            ...
            if (alRatioAfter < _convertedAL)
                revert ALTooLow(alRatioBefore, alRatioAfter, _convertedAL);
            _convertedAL = convertedAL(maxNewAL, cache);
            if (alRatioAfter > _convertedAL)
                revert ALTooHigh(alRatioBefore, alRatioAfter, _convertedAL);
        }
     ...
     */
    function _reservesCapacityFromTotalSupply(
        Cache memory cache
    ) internal view returns (uint256) {
        ...
        _availableShares = _availableShares.inverseSubtractBps(
            _dynamicDepositFeeBps(),
            MorigamiMath.Rounding.ROUND_DOWN
        );
     ...
     */
    function _reservesCapacityFromAlCeiling(
        Cache memory cache
    ) internal view returns (uint256) {
        if (cache.liabilities == 0) {
            return type(uint256).max;
    ...
    modifier onlyLovToken() {
        if (msg.sender != address(lovToken))
