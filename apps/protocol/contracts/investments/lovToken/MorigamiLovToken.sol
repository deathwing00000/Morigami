 ...
 */
contract MorigamiLovToken is IMorigamiLovToken, MorigamiInvestment {
    ...
    /**
     * @notice The annual performance fee which Morigami takes from harvested rewards before compounding into reserves.
    ...
    constructor(
        ...
        if (_annualPerformanceFeeBps > MorigamiMath.BASIS_POINTS_DIVISOR) revert CommonEventsAndErrors.InvalidParam();
    ...
    /** 
     * @notice Collect the performance fees to the Morigami Treasury
     */
    ...
    function sharesToReserves(uint256 shares) external override view returns (uint256) {
        return lovManager.sharesToReserves(shares, IMorigamiOracle.PriceType.SPOT_PRICE);
    }
    ...
    function reservesToShares(uint256 reserves) external override view returns (uint256) {
        return lovManager.reservesToShares(reserves, IMorigamiOracle.PriceType.SPOT_PRICE);
    }
    ...
    function reservesPerShare() external override view returns (uint256) {
        return lovManager.sharesToReserves(10 ** decimals(), IMorigamiOracle.PriceType.HISTORIC_PRICE);
    }
    ...
    function totalReserves() external override view returns (uint256) {
        return lovManager.userRedeemableReserves(IMorigamiOracle.PriceType.SPOT_PRICE);
    }
    ...
    function assetsAndLiabilities() external override view returns (
        ...
        return lovManager.assetsAndLiabilities(IMorigamiOracle.PriceType.SPOT_PRICE);
    }
    ...
    function effectiveExposure() external override view returns (uint128 /*effectiveExposure*/) {
        return lovManager.effectiveExposure(IMorigamiOracle.PriceType.SPOT_PRICE);
    }
    ...
    function accruedPerformanceFee() public override view returns (uint256) {
        ...
        return MorigamiMath.mulDiv(
            totalSupply(), 
            annualPerformanceFeeBps * _timeDelta, 
            MorigamiMath.BASIS_POINTS_DIVISOR * 365 days, 
