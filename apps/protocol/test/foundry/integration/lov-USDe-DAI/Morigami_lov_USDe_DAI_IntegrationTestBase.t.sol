...
import {ExternalContracts, LovTokenContracts, Morigami_lov_USDe_DAI_TestDeployer} from "test/foundry/deploys/lov-USDe-DAI/Morigami_lov_USDe_DAI_TestDeployer.t.sol";
...
contract Morigami_lov_USDe_DAI_IntegrationTestBase is MorigamiTest {
    ...
    Morigami_lov_USDe_DAI_TestDeployer internal deployer;
    ...
    function setUp() public virtual {
        ...
        deployer = new Morigami_lov_USDe_DAI_TestDeployer();
        ...
        )
    {
        ...
        (reservesAmount, params.swapData) = swapBorrowTokenToReserveTokenQuote(
            params.borrowAmount
        );
    ...
    function encode(bytes memory data) internal pure returns (bytes memory) {
        return
            abi.encode(
                MorigamiDexAggregatorSwapper.RouteData({
    ...
    function swapBorrowTokenToReserveTokenQuote(
        uint256 borrowAmount
    ) internal pure returns (uint256 reservesAmount, bytes memory swapData) {
...
-H "content-type: application/json"
    ...
    function swapReserveTokenToBorrowTokenQuote(
        uint256 reservesAmount
    ) internal pure returns (uint256 borrowAmount, bytes memory swapData) {
        // @note Ensure sDAI is listed as a connector token
