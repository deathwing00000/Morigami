...
import {Morigami_lov_sUSDe_DAI_IntegrationTestBase} from "test/foundry/integration/lov-sUSDe-DAI/Morigami_lov_sUSDe_DAI_IntegrationTestBase.t.sol";
...
contract Morigami_lov_sUSDe_DAI_IntegrationTest is
    Morigami_lov_sUSDe_DAI_IntegrationTestBase
{
    ...
    function test_lov_sUSDe_DAI_initialization() public {
...

        {
            ...
            assertEq(
                ...
                address(lovTokenContracts.sUsdeToDaiOracle)
            );
        }

        {
            assertEq(
                address(lovTokenContracts.usdeToDaiOracle.owner()),
                origamiMultisig
            );
            assertEq(
                lovTokenContracts.usdeToDaiOracle.description(),
                "USDe/DAI"
            );
            ...
            assertEq(
                lovTokenContracts.usdeToDaiOracle.stableHistoricPrice(),
                Constants.USDE_USD_HISTORIC_STABLE_PRICE
            );
            assertEq(
                address(lovTokenContracts.usdeToDaiOracle.spotPriceOracle()),
                address(externalContracts.redstoneUsdeToUsdOracle)
            );
            assertEq(
                lovTokenContracts.usdeToDaiOracle.spotPricePrecisionScaleDown(),
                false
            );
            assertEq(
                lovTokenContracts.usdeToDaiOracle.spotPricePrecisionScalar(),
                1e10
            ); // Redstone oracle is 8dp
            assertEq(
                lovTokenContracts.usdeToDaiOracle.spotPriceStalenessThreshold(),
        ...
        {
            assertEq(
                lovTokenContracts.sUsdeToDaiOracle.description(),
                "sUSDe/DAI"
            );
            ...
            assertEq(
                address(lovTokenContracts.sUsdeToDaiOracle.baseAsset()),
                Constants.SUSDE_ADDRESS
            );
            assertEq(
                address(lovTokenContracts.sUsdeToDaiOracle.quoteAssetOracle()),
                address(lovTokenContracts.usdeToDaiOracle)
            );
        }

        {
            ...
            assertEq(
                ...
                hex"42dcfb38bb98767afb6e38ccf90d59d0d3f0aa216beb3a234f12850323d17536"
            );
    ...
    function test_lov_sUSDe_DAI_rebalanceDown_success() public {
...

        {
            ...
            assertEq(
                ...
                50_565.885108409295130538e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.userRedeemableReserves(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
                ),
                50_200.637608409295147639e18
            );
        }
    }
    ...
    function test_lov_sUSDe_DAI_rebalanceUp_success() public {
...

        {
            ...
            assertEq(
                lovTokenContracts.lovTokenManager.assetToLiabilityRatio(),
                1.449197396029937048e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.SPOT_PRICE
                ),
                112_430.866305038768476660e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
                ),
                112_759.387047764765641991e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.reservesBalance(),
                162_934.518682652173198400e18
            );
            assertEq(
                lovTokenContracts.borrowLend.suppliedBalance(),
                162_934.518682652173198400e18
            ...
            assertEq(
                lovTokenContracts.borrowLend.debtBalance(),
                116_638.246864483591204295e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.userRedeemableReserves(
                    IMorigamiOracle.PriceType.SPOT_PRICE
                ),
                50_503.652377613404721740e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.userRedeemableReserves(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
                ),
                50_175.131634887407556409e18
            );

            // No surplus DAI left over
            assertEq(
                externalContracts.daiToken.balanceOf(
                    address(lovTokenContracts.lovTokenManager)
                ),
                0
            );
    ...
    function test_lov_sUSDe_DAI_shutdown() public {
        ...
        assertEq(
            ...
            780.758050501611577443e18
        );
    }
}

contract Morigami_lov_sUSDe_DAI_IntegrationTest_PegControls is
    Morigami_lov_sUSDe_DAI_IntegrationTestBase
{
    ...
    function test_latestPrices() public {
        (
            ...
            address baseAsset,
            address quoteAsset
        ) = lovTokenContracts.sUsdeToDaiOracle.latestPrices(
                IMorigamiOracle.PriceType.SPOT_PRICE,
