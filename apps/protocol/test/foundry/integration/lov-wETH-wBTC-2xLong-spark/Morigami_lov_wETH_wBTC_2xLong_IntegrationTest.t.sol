...
import {Morigami_lov_wETH_wBTC_2xLong_IntegrationTestBase} from "test/foundry/integration/lov-wETH-wBTC-2xLong-spark/Morigami_lov_wETH_wBTC_2xLong_IntegrationTestBase.t.sol";
import {Morigami_lovToken_TestConstants as Constants} from "test/foundry/deploys/lov-wETH-wBTC-2xLong-spark/Morigami_lovToken_TestConstants.t.sol";
...
contract Morigami_lov_wETH_wBTC_2xLong_IntegrationTest is
    Morigami_lov_wETH_wBTC_2xLong_IntegrationTestBase
{
    using MorigamiMath for uint256;

    uint256 decimalsScale =
        10 ** (Constants.WETH_DECIMALS - Constants.WBTC_DECIMALS);
    ...
    function test_lovToken_rebalanceDown_success() public {
        ...
        // 1Inch implied price
        assertApproxEqRel(
            (params.flashLoanAmount * 1e18) / reservesAmount,
            expectedSwapPrice,
            1e15
        );
    ...
    function test_lovToken_rebalanceUp_success() public {
...

        {
            ...
            assertEq(
                lovTokenContracts.lovTokenManager.assetToLiabilityRatio(),
                1.599967995678958566e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.SPOT_PRICE
                ),
                83.331666858386908490e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
...
contract Morigami_lov_wETH_wBTC_2xLong_IntegrationTest_PegControls is
    Morigami_lov_wETH_wBTC_2xLong_IntegrationTestBase
{
    uint256 decimalsScale =
        10 ** (Constants.WETH_DECIMALS - Constants.WBTC_DECIMALS);
