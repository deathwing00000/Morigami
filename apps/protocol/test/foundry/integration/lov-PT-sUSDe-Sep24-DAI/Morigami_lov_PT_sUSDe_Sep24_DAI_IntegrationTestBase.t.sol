...
import {ExternalContracts, LovTokenContracts, Morigami_lov_PT_sUSDe_Sep24_DAI_TestDeployer} from "test/foundry/deploys/lov-PT-sUSDe-Sep24-DAI/Morigami_lov_PT_sUSDe_Sep24_DAI_TestDeployer.t.sol";
import {DummyLovTokenSwapper} from "contracts/test/investments/lovToken/DummyLovTokenSwapper.sol";

contract Morigami_lov_PT_sUSDe_Sep24_DAI_IntegrationTestBase is MorigamiTest {
    ...
    Morigami_lov_PT_sUSDe_Sep24_DAI_TestDeployer internal deployer;
    ...
    function setUp() public virtual {
        ...
        deployer = new Morigami_lov_PT_sUSDe_Sep24_DAI_TestDeployer();
        origamiMultisig = address(deployer);
        (externalContracts, lovTokenContracts) = deployer.deployForked(
            origamiMultisig,
            feeCollector,
            overlord,
            vm
    ...
    function supplyIntoMorpho(uint256 amount) internal {
        ...
        morpho.supply(
            lovTokenContracts.borrowLend.getMarketParams(),
            amount,
            0,
            origamiMultisig,
            ""
    ...
    function exitLovToken(
        ...
        amountOut = lovTokenContracts.lovToken.exitToToken(
            quoteData,
            recipient
        );
    }
    ...
    function solveRebalanceDownAmount(
        ...
        */
        ...
        uint256 numerator = ((((targetLTV * marketDaiToPtPrice) / 1e18) *
            _currentAssets) / 1e18) - _currentDebt;
    ...
    function solveRebalanceUpAmount(
        ...
        */
        ...
        uint256 numerator = _currentDebt -
            ((((targetLTV * marketDaiToPtPrice) / 1e18) * _currentAssets) /
                1e18);
        uint256 denominator = 1e18 - targetLTV;
        ...
        )
    {
        ...
        params.borrowAmount = params.borrowAmount.inverseSubtractBps(
            swapSlippageBps,
            MorigamiMath.Rounding.ROUND_DOWN
        );

        params.swapData = abi.encode(
            DummyLovTokenSwapper.SwapData(params.supplyAmount)
        );

        {
            ...
            ) = lovTokenContracts.lovTokenManager.assetsAndLiabilities(
                    IMorigamiOracle.PriceType.SPOT_PRICE
                );
            uint256 newAssets = existingAssets + params.supplyAmount;
            uint256 newLiabilities = existingLiabilities +
                ((params.borrowAmount * 1e18) / marketPtToDaiPrice);
            uint256 targetAL = (newAssets * 1e18) / newLiabilities;

            params.minNewAL = uint128(
                targetAL.subtractBps(
                    alSlippageBps,
                    MorigamiMath.Rounding.ROUND_DOWN
                )
            );
            ...
            params.maxNewAL = uint128(
                targetAL.addBps(alSlippageBps, MorigamiMath.Rounding.ROUND_UP)
            );
        ...
        returns (IMorigamiLovTokenMorphoManager.RebalanceUpParams memory params)
    {
        ...
        params.withdrawCollateralAmount = solveRebalanceUpAmount(
            targetLTV,
            marketPtToDaiPrice
        );
        ...
        // Use the dex price
        params.repayAmount =
            (params.withdrawCollateralAmount * marketPtToDaiPrice) /
            1e18;
        ...
        uint256 feeBps = 0;
        params.repayAmount = params.repayAmount.inverseSubtractBps(
            feeBps,
            MorigamiMath.Rounding.ROUND_UP
        );
...

        {
            ...
            uint256 newAssets = existingAssets -
                params.withdrawCollateralAmount;
            uint256 newLiabilities = existingLiabilities -
                ((params.repayAmount * 1e18) / marketPtToDaiPrice);
            uint256 targetAL = (newAssets * 1e18) / newLiabilities;
            params.minNewAL = uint128(
                targetAL.subtractBps(
                    alSlippageBps,
