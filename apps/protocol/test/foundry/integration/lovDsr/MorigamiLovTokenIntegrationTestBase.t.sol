...
contract MorigamiLovTokenIntegrationTestBase is MorigamiTest {
    ...
    MorigamiLovTokenTestDeployer internal deployer;
    ...
    function setUp() public virtual {
        ...
        (externalContracts, oUsdcContracts, lovTokenContracts) = deployer
            .deployForked(origamiMultisig, feeCollector, overlord);
        aToken = MorigamiAaveV3IdleStrategy(
            address(oUsdcContracts.idleStrategy)
        ).aToken();
    }

    function investOvUsdc(
        address account,
        uint256 amount
    ) internal returns (uint256 amountOut) {
        doMint(externalContracts.usdcToken, account, amount);
        vm.startPrank(account);
        externalContracts.usdcToken.approve(
        ...
        ) = oUsdcContracts.ovUsdc.investQuote(
                amount,
    ...
    function exitOvUsdc(
        ...
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = oUsdcContracts
            .ovUsdc
            .exitQuote(amount, address(externalContracts.usdcToken), 0, 0);

        amountOut = oUsdcContracts.ovUsdc.exitToToken(quoteData, account);
    }

    function investLovDsr(
        address account,
        uint256 amount
    ) internal returns (uint256 amountOut) {
    ...
    function solveRebalanceDownAmount(
        ...
        uint256 currentAL = lovTokenContracts
            .lovDsrManager
        ...
        )
    {
        ...
        (daiDepositAmount, params.swapData) = swapUsdcToDaiQuote(
            params.borrowAmount
        );
        ...
        params.minReservesOut = MorigamiMath.subtractBps(
            reservesAmount,
            swapSlippageBps,
            MorigamiMath.Rounding.ROUND_DOWN
        );
    ...
    function doRebalanceDown(
        ...
        (
            IMorigamiLovTokenErc4626Manager.RebalanceDownParams memory params,
            uint256 _reservesAmount
        ) = rebalanceDownParams(targetAL, swapSlippageBps, alSlippageBps);
        ...
        )
    {
        ...
        (params.minDebtAmountToRepay, params.swapData) = swapDaiToUsdcQuote(
            params.depositAssetsToWithdraw
        );
        params.minDebtAmountToRepay = MorigamiMath.subtractBps(
    ...
    function swapDaiToUsdcQuote(
        uint256 daiAmount
    ) internal pure returns (uint256 usdcAmount, bytes memory swapData) {
        // REQUEST:
        /*
        curl -X GET \
...
-H "content-type: application/json"
    ...
    function swapUsdcToDaiQuote(
        uint256 usdcAmount
    ) internal pure returns (uint256 daiAmount, bytes memory swapData) {
        // REQUEST:
        /*
        curl -X GET \
