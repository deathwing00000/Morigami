...
import {Morigami_lov_sUSDe_USDT_IntegrationTestBase} from "test/foundry/integration/lov-sUSDe-USDT/Morigami_lov_sUSDe_USDT_IntegrationTestBase.t.sol";
import {Morigami_lov_sUSDe_USDT_TestConstants as Constants} from "test/foundry/deploys/lov-sUSDe-USDT/Morigami_lov_sUSDe_USDT_TestConstants.t.sol";
...
contract Morigami_lov_sUSDe_USDT_IntegrationTest is
    Morigami_lov_sUSDe_USDT_IntegrationTestBase
{
    ...
    function test_lov_sUSDe_USDT_initialization() public {
        {
            assertEq(
                address(lovTokenContracts.lovToken.owner()),
                origamiMultisig
            );
            assertEq(
                address(lovTokenContracts.lovToken.manager()),
                address(lovTokenContracts.lovTokenManager)
            );
            assertEq(
                lovTokenContracts.lovToken.annualPerformanceFeeBps(),
        ...
        {
            assertEq(
                address(lovTokenContracts.lovTokenManager.owner()),
                origamiMultisig
            );
            assertEq(
                address(lovTokenContracts.lovTokenManager.lovToken()),
                address(lovTokenContracts.lovToken)
            );
            ...
            ) = lovTokenContracts.lovTokenManager.getFeeConfig();
            ...
            assertEq(
                address(lovTokenContracts.lovTokenManager.debtToken()),
                address(externalContracts.usdtToken)
            );
            assertEq(
                address(lovTokenContracts.lovTokenManager.reserveToken()),
                address(externalContracts.sUsdeToken)
            );
            assertEq(
                address(lovTokenContracts.lovTokenManager.borrowLend()),
                address(lovTokenContracts.borrowLend)
            );
            assertEq(
                address(
                    lovTokenContracts
                ...
                address(lovTokenContracts.sUsdeToUsdtOracle)
            );
        }

        {
            assertEq(
                address(lovTokenContracts.usdeToUsdtOracle.owner()),
                origamiMultisig
            );
            assertEq(
                lovTokenContracts.usdeToUsdtOracle.description(),
                "USDe/USDT"
            );
            ...
            assertEq(
                lovTokenContracts.usdeToUsdtOracle.stableHistoricPrice(),
                Constants.USDE_USD_HISTORIC_STABLE_PRICE
            );
            assertEq(
                address(lovTokenContracts.usdeToUsdtOracle.spotPriceOracle()),
            ...
            assertEq(
                lovTokenContracts.usdeToUsdtOracle.spotPricePrecisionScalar(),
                1e10
            ); // Redstone oracle is 8dp
            assertEq(
                lovTokenContracts
                    .usdeToUsdtOracle
        ...
        {
            assertEq(
                lovTokenContracts.sUsdeToUsdtOracle.description(),
                "sUSDe/USDT"
            );
            ...
            assertEq(
                address(lovTokenContracts.sUsdeToUsdtOracle.baseAsset()),
                Constants.SUSDE_ADDRESS
            );
            assertEq(
                address(lovTokenContracts.sUsdeToUsdtOracle.quoteAssetOracle()),
                address(lovTokenContracts.usdeToUsdtOracle)
            );
        }

        {
            assertEq(
                address(lovTokenContracts.borrowLend.morpho()),
                Constants.MORPHO
            );
            assertEq(
                address(lovTokenContracts.borrowLend.supplyToken()),
                Constants.SUSDE_ADDRESS
            );
            assertEq(
                address(lovTokenContracts.borrowLend.borrowToken()),
                Constants.USDT_ADDRESS
            );
            assertEq(
                address(lovTokenContracts.borrowLend.morphoMarketOracle()),
                Constants.MORPHO_MARKET_ORACLE
            );
            assertEq(
                address(lovTokenContracts.borrowLend.morphoMarketIrm()),
                Constants.MORPHO_MARKET_IRM
            );
            assertEq(
                lovTokenContracts.borrowLend.morphoMarketLltv(),
                Constants.MORPHO_MARKET_LLTV
            );
            assertEq(
                MorphoMarketId.unwrap(lovTokenContracts.borrowLend.marketId()),
                hex"dc5333039bcf15f1237133f74d5806675d83d9cf19cfd4cfdd9be674842651bf"
            );

            assertEq(
                address(lovTokenContracts.borrowLend.swapper()),
    ...
    function test_lov_sUSDe_USDT_invest_success() public {
        ...
        {
            assertEq(
                lovTokenContracts.lovToken.balanceOf(alice),
                expectedAmountOut
            );
            assertEq(
                lovTokenContracts.lovToken.totalSupply(),
                expectedAmountOut
            );
            assertEq(
                externalContracts.sUsdeToken.balanceOf(
                    address(lovTokenContracts.lovTokenManager)
                ),
                0
            );
            ...
            assertEq(
                lovTokenContracts.lovTokenManager.reservesBalance(),
                amount
            );
            assertEq(
                lovTokenContracts.lovTokenManager.userRedeemableReserves(
            ...
            assertEq(
                lovTokenContracts.lovTokenManager.assetToLiabilityRatio(),
                type(uint128).max
            );
            assertEq(
                lovTokenContracts.lovTokenManager.effectiveExposure(
                    IMorigamiOracle.PriceType.SPOT_PRICE
                ),
                1e18
            );
    ...
    function test_lov_sUSDe_USDT_rebalanceDown_success() public {
...

        {
            ...
            assertEq(
                lovTokenContracts.lovTokenManager.assetToLiabilityRatio(),
                1.336954602629945952e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.SPOT_PRICE
                ),
                150_015.001501035597271686e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
                ),
                150_453.342335121593245678e18
            );
            ...
            assertEq(
                lovTokenContracts.borrowLend.suppliedBalance(),
                expectedReserves
            );
            assertEq(
                lovTokenContracts.borrowLend.debtBalance(),
                params.borrowAmount + 1
            );
            ...
            assertEq(
                ...
                50_548.245219312195296002e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.userRedeemableReserves(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
                ),
                50_109.904385226199322010e18
            );
        }
    }
    ...
    function test_lov_sUSDe_USDT_rebalanceUp_success() public {
...

        {
            ...
            assertEq(
                lovTokenContracts.lovTokenManager.assetToLiabilityRatio(),
                1.359500722596497211e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.SPOT_PRICE
                ),
                140_463.358585397943372549e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.liabilities(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
                ),
                140_873.789709917304435907e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.reservesBalance(),
                190_960.037495179404451563e18
            );
            assertEq(
                lovTokenContracts.borrowLend.suppliedBalance(),
                190_960.037495179404451563e18
            ...
            assertEq(
                ...
                50_496.678909781461079014e18
            );
            assertEq(
                lovTokenContracts.lovTokenManager.userRedeemableReserves(
                    IMorigamiOracle.PriceType.HISTORIC_PRICE
                ),
                50_086.247785262100015656e18
            );

            // No surplus USDT left over
            assertEq(
                externalContracts.usdtToken.balanceOf(
                    address(lovTokenContracts.lovTokenManager)
                ),
                0
            );
    ...
    function test_lov_sUSDe_USDT_exit_fail_al_limit() public {
        ...
        assertEq(
            lovTokenContracts.lovTokenManager.assetToLiabilityRatio(),
            1.336785853995107381e18
        );

        uint256 maxExitAmount = 15_838.234315576427343812e18;
        assertEq(
            lovTokenContracts.lovToken.maxExit(
                address(externalContracts.sUsdeToken)
            ),
            maxExitAmount
        );
    ...
    function test_lov_sUSDe_USDT_shutdown() public {
        ...
        (uint256 assets, uint256 liabilities, uint256 ratio) = lovTokenContracts
            .lovToken
        ...
        (
            ...
            params.swapData
        ) = swapBorrowTokenToReserveTokenQuote(params.repayAmount);
        (
            params.repayAmount,
            params.swapData
        ) = swapReserveTokenToBorrowTokenQuote(params.withdrawCollateralAmount);
        ...
        (assets, liabilities, ratio) = lovTokenContracts
            .lovToken
            .assetsAndLiabilities();
...
contract Morigami_lov_sUSDe_USDT_IntegrationTest_PegControls is
    Morigami_lov_sUSDe_USDT_IntegrationTestBase
{
    ...
    function test_lov_sUSDe_USDT_invest_fail_peg_controls() public {
        ...
        vm.startPrank(alice);
        externalContracts.sUsdeToken.approve(
            address(lovTokenContracts.lovToken),
            amount
        );
        ...
        ) = lovTokenContracts.lovToken.investQuote(
                amount,
                address(externalContracts.sUsdeToken),
                0,
                0
    ...
    function test_latestPrices() public {
        (
            ...
            address baseAsset,
            address quoteAsset
        ) = lovTokenContracts.sUsdeToUsdtOracle.latestPrices(
                IMorigamiOracle.PriceType.SPOT_PRICE,
        ...
        (spot, hist, baseAsset, quoteAsset) = lovTokenContracts
            .usdeToUsdtOracle
            .latestPrices(
                IMorigamiOracle.PriceType.SPOT_PRICE,
                MorigamiMath.Rounding.ROUND_UP,
