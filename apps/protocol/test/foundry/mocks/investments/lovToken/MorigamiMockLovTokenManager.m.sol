...
contract MorigamiMockLovTokenManager is MorigamiAbstractLovTokenManager {
    ...
    function rebalanceDown(uint256 borrowReservesAmount) external {
        ...
        _reserveToken.safeTransferFrom(
            msg.sender,
            address(this),
            borrowReservesAmount
        );
        ...
        cache.liabilities = liabilities(IMorigamiOracle.PriceType.SPOT_PRICE);
     ...
     */
    function reservesBalance() public view override returns (uint256) {
     ...
     */
    function reserveToken() public view override returns (address) {
        return address(_reserveToken);
    }
     ...
     */
    function _dynamicDepositFeeBps() internal view override returns (uint256) {
     ...
     * @dev represented in basis points
     */
    function _dynamicExitFeeBps() internal view override returns (uint256) {
    ...
    function _depositIntoReserves(
        ...
        if (fromToken == address(depositAsset)) {
            ...
            depositAsset.safeIncreaseAllowance(
                address(_reserveToken),
                fromTokenAmount
            );
            newReservesAmount = _reserveToken.deposit(
                fromTokenAmount,
                address(this)
            );
    ...
    function _redeemFromReserves(
        ...
        if (toToken == address(depositAsset)) {
            toTokenAmount = _reserveToken.redeem(
                reservesAmount,
                recipient,
                address(this)
            );
    ...
    function _maxRedeemFromReserves(
        ...
        if (toToken == address(depositAsset)) {
            ...
            uint256 _maxUnderlyingRedeem = _reserveToken.maxRedeem(
                address(this)
            );
