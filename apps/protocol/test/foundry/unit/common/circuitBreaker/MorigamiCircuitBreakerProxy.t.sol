...
contract MorigamiCircuitBreakerProxyTest is MorigamiTest {
    MorigamiCircuitBreakerAllUsersPerPeriod public templeCircuitBreaker;
    MorigamiCircuitBreakerAllUsersPerPeriod public daiCircuitBreaker;
    MorigamiCircuitBreakerProxy public circuitBreakerProxy;
    ...
    function setUp() public {
        daiToken = new DummyMintableToken(
            origamiMultisig,
            "DAI Token",
            "DAI",
            18
        );
        templeToken = new DummyMintableToken(
            origamiMultisig,
            "Temple Token",
            "TEMPLE",
            18
        );
        circuitBreakerProxy = new MorigamiCircuitBreakerProxy(origamiMultisig);
        templeCircuitBreaker = new MorigamiCircuitBreakerAllUsersPerPeriod(
            origamiMultisig,
        ...
        daiCircuitBreaker = new MorigamiCircuitBreakerAllUsersPerPeriod(
            origamiMultisig,
            address(circuitBreakerProxy),
            26 hours,
            13,
    ...
    function test_access_setCircuitBreaker() public {
        expectElevatedAccess();
        circuitBreakerProxy.setCircuitBreaker(
            EXTERNAL_ALL_USERS,
            address(templeToken),
            address(templeCircuitBreaker)
        );
    }
    ...
    function test_setIdentifierForCaller_success() public {
        ...
        assertEq(
            circuitBreakerProxy.callerToIdentifier(caller),
            EXTERNAL_ALL_USERS
        );
    }
    ...
    function test_setCircuitBreaker_disable() public {
        ...
        assertEq(
            address(
                circuitBreakerProxy.circuitBreakers(
                    EXTERNAL_ALL_USERS,
                    address(daiToken)
                )
            ),
            address(daiCircuitBreaker)
        );
        assertEq(circuitBreakerProxy.cap(address(daiToken), caller), 1_000e18);
        assertEq(
            circuitBreakerProxy.currentUtilisation(address(daiToken), caller),
            250e18
        );
        assertEq(
            circuitBreakerProxy.available(address(daiToken), caller),
            750e18
        );
    ...
    function test_unmapped_views() public {
        ...
        assertEq(
            circuitBreakerProxy.currentUtilisation(address(daiToken), alice),
            0
        );
