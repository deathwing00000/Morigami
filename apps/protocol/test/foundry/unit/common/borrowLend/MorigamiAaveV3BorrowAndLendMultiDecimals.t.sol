...
import {MorigamiAaveV3BorrowAndLendConstants as Constants} from "test/foundry/unit/common/borrowLend/MorigamiAaveV3BorrowAndLendConstants.t.sol";

contract MorigamiAaveV3BorrowAndLendMultiDecimalsTestBase is MorigamiTest {
    ...
    function generateNewLendBorrow(
        uint256 ind,
        address _supplyToken,
        address _borrowToken
    ) internal {
        MorigamiAaveV3BorrowAndLend borrowLend = new MorigamiAaveV3BorrowAndLend(
                origamiMultisig,
                address(_supplyToken),
                address(_borrowToken),
        ...
        AaveDataTypes.ReserveData memory _reserveData = IAavePool(
            Constants.SPARK_POOL
        ).getReserveData(_supplyToken);
        address supplyAToken = _reserveData.aTokenAddress;

        _reserveData = IAavePool(Constants.SPARK_POOL).getReserveData(
            _borrowToken
        );
    ...
    function isStable(address _token) internal pure returns (bool) {
        return
            _token == Constants.DAI_ADDRESS ||
            _token == Constants.USDC_ADDRESS ||
            _token == Constants.USDT_ADDRESS;
    }

    function revertPerToken(address _token) internal {
        if (
            _token == Constants.WBTC_ADDRESS || _token == Constants.USDT_ADDRESS
        ) vm.expectRevert();
        else if (_token == Constants.DAI_ADDRESS)
            vm.expectRevert("Dai/insufficient-balance");
        else vm.expectRevert("ERC20: transfer amount exceeds balance");
    }
}

contract MorigamiAaveV3BorrowAndLendMultiDecimalsTestAdmin is
    MorigamiAaveV3BorrowAndLendMultiDecimalsTestBase
{
    ...
    function test_initialization() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            assertEq(
                address(info.borrowLend.aaveDebtToken()),
                info.borrowDebtToken
            );
            ...
            // eMode updated
            assertEq(
                info.borrowLend.aavePool().getUserEMode(
                    address(info.borrowLend)
                ),
                eModes[ind]
            );
        }
    ...
    function test_constructor_zeroEMode() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            assertEq(
                borrowLendContracts[ind].borrowLend.aavePool().getUserEMode(
                    address(borrowLendContracts[ind].borrowLend)
                ),
                0
            );
        }
    ...
    function test_setPositionOwner_success() public {
        ...
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            MorigamiAaveV3BorrowAndLend borrowLendItem = borrowLendContracts[
                ind
            ].borrowLend;
    ...
    function test_setUserUseReserveAsCollateral_fail() public {
        ...
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            borrowLendContracts[ind].borrowLend.setUserUseReserveAsCollateral(
                true
            );
        }
    }
    ...
    function test_setUserUseReserveAsCollateral_success() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            BorrowLendContract storage info = borrowLendContracts[ind];
            MorigamiAaveV3BorrowAndLend borrowLendItem = info.borrowLend;
            ...
            emit ReserveUsedAsCollateralDisabled(
                address(info.supplyToken),
                address(borrowLendItem)
            );
    ...
    function test_setEModeCategory() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            assertEq(
                borrowLendItem.aavePool().getUserEMode(address(borrowLendItem)),
                0
            );
    ...
    function test_suppliedBalance_externalSupply() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            approveToken(
                info.supplyToken,
                address(borrowLendItem.aavePool()),
                externalSupply
            );
            borrowLendItem.aavePool().supply(
                address(info.supplyToken),
                externalSupply,
                address(alice),
                0
            ...
            uint256 balanceBefore = IERC20(info.supplyAToken).balanceOf(
                address(borrowLendItem)
            );
    ...
    function test_suppliedBalance_donation() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            uint256 actualBalance = IERC20(info.supplyAToken).balanceOf(
                address(borrowLendItem)
            );
    ...
    function test_recoverToken_rounding() public {
        ...
        checkBalances[1] = abi.encode(
            10660775309530783840050,
            12792930371436940608060,
            2132155061906156768010
        );
        checkBalances[2] = supplyTokens[2] == Constants.USDC_ADDRESS
            ? abi.encode(10665096086, 12798115304, 2133019218)
            : abi.encode(10680852313, 12817022776, 2136170463);
        checkBalances[3] = abi.encode(
            10005419817350963394,
            12006503780821156073,
            2001083963470192679
        );
        checkBalances[4] = abi.encode(
            5006688842884845476,
            6008026611461814572,
            1001337768576969096
        );
        ...
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            uint256 index = borrowLendItem
                .aavePool()
                .getReserveNormalizedIncome(address(info.supplyToken));
            ...
            (
                uint256 _expectedSuppliedBalance,
                uint256 _aTokenBalance,
                uint256 _recoverTokenAmount
            ) = abi.decode(checkBalances[ind], (uint256, uint256, uint256));
            assertApproxEqAbs(
                borrowLendItem.aaveAToken().scaledBalanceOf(
                    address(borrowLendItem)
                ),
                B,
                1
            ...
            assertEq(
                borrowLendItem.suppliedBalance(),
                _expectedSuppliedBalance
            );
            assertEq(
                borrowLendItem.aaveAToken().balanceOf(address(borrowLendItem)),
                _expectedSuppliedBalance
            );
            ...
            uint256 recoverTokenAmount = IERC20(info.supplyAToken).balanceOf(
                address(borrowLendItem)
            ) - borrowLendItem.suppliedBalance();
            assertEq(recoverTokenAmount, _recoverTokenAmount);

            vm.startPrank(origamiMultisig);
            borrowLendItem.recoverToken(
                info.supplyAToken,
                bob,
                recoverTokenAmount - 1
            );
            ...
            uint256 prevCollectorSupplyBalance = info.supplyToken.balanceOf(
                feeCollector
            );

            vm.startPrank(borrowLendItem.positionOwner());
            borrowLendItem.withdraw(
                borrowLendItem.suppliedBalance(),
                feeCollector
            );
            ...
            assertApproxEqAbs(
                info.supplyToken.balanceOf(feeCollector),
                prevCollectorSupplyBalance + _expectedSuppliedBalance,
                1
            );
        }
    }
...
contract MorigamiAaveV3BorrowAndLendMultiDecimalsTestAccess is
    MorigamiAaveV3BorrowAndLendMultiDecimalsTestBase
{
    ...
    function test_access_withdraw() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            vm.prank(posOwner);
            vm.expectRevert(
                bytes(AaveErrors.NOT_ENOUGH_AVAILABLE_USER_BALANCE)
            );
            borrowLendItem.withdraw(5, alice);
...
contract MorigamiAaveV3BorrowAndLendMultiDecimalsTestViews is
    MorigamiAaveV3BorrowAndLendMultiDecimalsTestBase
{
    ...
    function test_suppliedBalance_success() public {
        ...
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            assertApproxEqAbs(
                borrowLendItem.suppliedBalance(),
                checkBalances[ind] - withdrawAmount,
                1
            );
    ...
    function test_debtBalance() public {
        ...
        afterDebtAmounts[0] = supplyTokens[2] == Constants.USDC_ADDRESS
            ? 16226614248
            : 16226603677;
        afterDebtAmounts[1] = 481501638259417763;
        afterDebtAmounts[2] = 2185269356435658492118;
        afterDebtAmounts[3] = 15151355;
        afterDebtAmounts[4] = supplyTokens[2] == Constants.USDC_ADDRESS
            ? 2168983435
            : 2168984678;
        ...
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            assertApproxEqAbs(
                borrowLendItem.debtBalance(),
                afterDebt - borrowAmounts[ind] / 2,
                1
            );
            assertApproxEqAbs(
                borrowLendItem.aaveDebtToken().balanceOf(
                    address(borrowLendItem)
                ),
                afterDebt - borrowAmounts[ind] / 2,
                1
    ...
    function test_isSafeAlRatio_ethEMode() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            uint256 eMode = borrowLendItem.aavePool().getUserEMode(
                address(borrowLendItem)
            );
            uint256 ltvVal = eMode == 0
                ? ltvValues[ind]
                : IAavePool(Constants.SPARK_POOL)
                    .getEModeCategoryData(uint8(eMode))
                    .ltv;
    ...
    function test_availableToWithdraw() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            uint256 withdrawAmount = info.supplyToken.balanceOf(
                info.supplyAToken
            );
            ...
            assertEq(
                borrowLendItem.availableToWithdraw(),
                withdrawAmount + supplyAmounts[ind]
            );
        }
    }
    ...
    function test_availableToBorrow() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            AaveDataTypes.ReserveData memory _reserveData = IAavePool(
                Constants.SPARK_POOL
            ).getReserveData(address(info.borrowToken));
            uint256 borrowCap = _reserveData.configuration.getBorrowCap() *
                (10 ** _reserveData.configuration.getDecimals());
            uint256 balance = info.borrowToken.balanceOf(
                _reserveData.aTokenAddress
            );

            uint256 availableBorrow = (borrowCap > 0 && borrowCap < balance)
                ? borrowCap
                : balance;
            ...
            availableBorrow = (borrowCap > 0 && borrowCap < balance)
                ? borrowCap
                : balance;
            assertEq(borrowLendItem.availableToBorrow(), availableBorrow);
        }
    ...
    function test_availableToSupply() public {
        ...
        availableAmounts[2] = supplyTokens[2] == Constants.USDC_ADDRESS
            ? 348788114741636
            : 296907069247963; // USDT
        ...
        afterAvailableAmounts[2] = supplyTokens[2] == Constants.USDC_ADDRESS
            ? 348778101800015
            : 296897069247963; // USDT
        ...
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            (uint256 supplyCap, uint256 available) = borrowLendItem
                .availableToSupply();

            AaveDataTypes.ReserveData memory reserveinfo = IAavePool(
                Constants.SPARK_POOL
            ).getReserveData(address(info.supplyToken));

            assertEq(
                supplyCap,
                reserveinfo.configuration.getSupplyCap() *
                    (10 ** reserveinfo.configuration.getDecimals())
            );
            ...
            (supplyCap, available) = borrowLendItem.availableToSupply();
    ...
    function test_debtAccountData() public {
        ...
        debtInfos[0] = abi.encode(
            7114809232269,
            1500045390000,
            3693765349556,
            3699588851221362042
        );
        debtInfos[1] = abi.encode(
            1000285990000,
            205801690476,
            424378483224,
            3742535887429072704
        );
        debtInfos[2] = supplyTokens[2] == Constants.USDC_ADDRESS
            ? abi.encode(
                999950000000,
                200057198000,
                549905302000,
        ...
        debtInfos[3] = abi.encode(
            4287535218250,
            1067221384840,
            2298493761486,
            3254154738759910399
        );
        debtInfos[4] = abi.encode(
            2032435788180,
            199990000000,
            1314174662194,
            7825269047947397370
        ...
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            (checkVals[0], checkVals[1], checkVals[2], checkVals[3]) = abi
                .decode(debtInfos[ind], (uint256, uint256, uint256, uint256));
...
contract MorigamiAaveV3BorrowAndLendMultiDecimalsTestSupply is
    MorigamiAaveV3BorrowAndLendMultiDecimalsTestBase
{
    ...
    function test_withdraw_success() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            assertApproxEqAbs(amountOut, amount / 2, 1);
            assertApproxEqAbs(
                info.supplyToken.balanceOf(address(alice)),
                amount,
                1
            );
        }
    ...
    function test_borrow_fail() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            vm.expectRevert(
                bytes(AaveErrors.COLLATERAL_CANNOT_COVER_NEW_BORROW)
            );
    ...
    function test_borrow_success() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            uint256 beforeAliceSupplyBalance = info.supplyToken.balanceOf(
                address(alice)
            );
    ...
    function test_repay_success() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            assertApproxEqAbs(
                info.borrowToken.balanceOf(address(borrowLendItem)),
                borrowAmounts[ind] / 2,
                1
            );
            assertEq(
    ...
    function test_supplyAndBorrow_success() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            vm.startPrank(posOwner);
            borrowLendItem.supplyAndBorrow(
                supplyAmounts[ind],
                borrowAmounts[ind],
                alice
            );
    ...
    function test_repayAndWithdraw_success() public {
        for (uint256 ind; ind < borrowLendCount; ++ind) {
            ...
            uint256 beforeAliceBorrow = info.borrowToken.balanceOf(
                address(alice)
            );
            ...
            vm.startPrank(posOwner);
            (uint256 debtRepaidAmount, uint256 withdrawnAmount) = borrowLendItem
                .repayAndWithdraw(
                    borrowAmounts[ind] / 2,
                    supplyAmounts[ind] / 2,
                    alice
            ...
            assertEq(
                info.supplyToken.balanceOf(address(alice)),
                beforeAliceSupplyBalance + supplyAmounts[ind] / 2
            );
            assertEq(info.borrowToken.balanceOf(address(borrowLendItem)), 0);
            assertEq(
                info.borrowToken.balanceOf(address(alice)),
                beforeAliceBorrow + borrowAmounts[ind]
            );
        }
    }
}
