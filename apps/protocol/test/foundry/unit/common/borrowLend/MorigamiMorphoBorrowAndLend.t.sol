...
contract MorigamiMorphoBorrowAndLendTestBase is MorigamiTest {
    ...
    function increaseLeverage() internal {
        ...
        DummyLovTokenSwapper.SwapData memory swapData = DummyLovTokenSwapper
            .SwapData({buyTokenAmount: 500e18});
...
contract MorigamiMorphoBorrowAndLendTestAdmin is
    MorigamiMorphoBorrowAndLendTestBase
{
    ...
    function test_initialization() public {
        ...
        assertEq(
            daiToken.allowance(address(borrowLend), address(MORPHO)),
            type(uint256).max
        );
        assertEq(
            sUsdeToken.allowance(address(borrowLend), address(MORPHO)),
            type(uint256).max
        );
    }
    ...
    function test_setSwapper_success() public {
        ...
        assertEq(
            sUsdeToken.allowance(address(borrowLend), alice),
            type(uint256).max
        );
        assertEq(
            daiToken.allowance(address(borrowLend), alice),
            type(uint256).max
        );
        ...
        assertEq(
            sUsdeToken.allowance(address(borrowLend), bob),
            type(uint256).max
        );
        assertEq(daiToken.allowance(address(borrowLend), alice), 0);
        assertEq(
            daiToken.allowance(address(borrowLend), bob),
            type(uint256).max
        );
    }
    ...
    function test_suppliedBalance_externalSupply() public {
        ...
        borrowLend.morpho().supplyCollateral(
            borrowLend.getMarketParams(),
            externalSupply,
            address(alice),
            ""
...
contract MorigamiMorphoBorrowAndLendTestAccess is
    MorigamiMorphoBorrowAndLendTestBase
{
...
contract MorigamiMorphoBorrowAndLendTestViews is
    MorigamiMorphoBorrowAndLendTestBase
{
...
contract MorigamiMorphoBorrowAndLendTestSupply is
    MorigamiMorphoBorrowAndLendTestBase
{
    ...
    function test_repayAndWithdraw_success() public {
        ...
        (uint256 debtRepaidAmount, uint256 withdrawnAmount) = borrowLend
            .repayAndWithdraw(borrowAmount / 2, borrowAmount / 2, alice);
...
contract MorigamiMorphoBorrowAndLendTestIncreaseLeverage is
    MorigamiMorphoBorrowAndLendTestBase
{
    ...
    function test_increaseLeverage_success_withSurplus() public {
        ...
        deal(address(sUsdeToken), address(swapper), 100e18);
        DummyLovTokenSwapper.SwapData memory swapData = DummyLovTokenSwapper
            .SwapData({buyTokenAmount: 52.5e18});
...
contract MorigamiMorphoBorrowAndLendTestDecreaseLeverage is
    MorigamiMorphoBorrowAndLendTestBase
{
    ...
    function test_decreaseLeverage_success_withSurplus() public {
        ...
        // sUSDe -> DAI = 1.025
        DummyLovTokenSwapper.SwapData memory swapData = DummyLovTokenSwapper
            .SwapData({buyTokenAmount: 102.5e18});

        vm.startPrank(posOwner);
        (uint256 repaid, uint256 surplusRepaid) = borrowLend.decreaseLeverage(
            100e18,
            100e18,
            abi.encode(swapData),
            100e18
    ...
    function test_decreaseLeverage_tooMuch_overSurplusThreshold() public {
        ...
        DummyLovTokenSwapper.SwapData memory swapData = DummyLovTokenSwapper
            .SwapData({buyTokenAmount: 602e18});
