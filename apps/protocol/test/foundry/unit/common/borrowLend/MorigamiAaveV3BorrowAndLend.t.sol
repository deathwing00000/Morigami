...
contract MorigamiAaveV3BorrowAndLendTestBase is MorigamiTest {
    ...
    address internal constant SPARK_WSTETH_A_TOKEN =
        0x12B54025C112Aa61fAce2CDB7118740875A566E9;
    address internal constant SPARK_WETH_DEBT_TOKEN =
...
contract MorigamiAaveV3BorrowAndLendTestAdmin is
    MorigamiAaveV3BorrowAndLendTestBase
{
    ...
    function test_initialization() public {
        ...
        assertEq(
            borrowLend.aavePool().getUserEMode(address(borrowLend)),
            SPARK_EMODE_ETH
        );
    }
    ...
    function test_setAavePool() public {
        ...
        assertEq(
            wethToken.allowance(address(borrowLend), alice),
            type(uint256).max
        );
        assertEq(
            wstEthToken.allowance(address(borrowLend), alice),
            type(uint256).max
        );
    }
    ...
    function test_suppliedBalance_externalSupply() public {
        ...
        uint256 balanceBefore = IERC20(SPARK_WSTETH_A_TOKEN).balanceOf(
            address(borrowLend)
        );
    ...
    function test_suppliedBalance_donation() public {
        ...
        uint256 actualBalance = IERC20(SPARK_WSTETH_A_TOKEN).balanceOf(
            address(borrowLend)
        );
    ...
    function test_recoverToken_rounding() public {
        uint256 RAY = 1e27;
        uint256 index = borrowLend.aavePool().getReserveNormalizedIncome(
            address(wstEthToken)
        );
        ...
        assertEq(
            borrowLend.aaveAToken().scaledBalanceOf(address(borrowLend)),
            B
        );
        assertEq(borrowLend.suppliedBalance(), expectedSuppliedBalance);
        assertEq(
            borrowLend.aaveAToken().balanceOf(address(borrowLend)),
            expectedSuppliedBalance
        );
        ...
        assertEq(
            borrowLend.aaveAToken().balanceOf(address(borrowLend)),
            8.555896290300169237e18
        );

        uint256 recoverTokenAmount = IERC20(SPARK_WSTETH_A_TOKEN).balanceOf(
            address(borrowLend)
        ) - borrowLend.suppliedBalance();
        assertEq(recoverTokenAmount, 3.000119478416942480e18);

        vm.startPrank(origamiMultisig);
        borrowLend.recoverToken(
            SPARK_WSTETH_A_TOKEN,
            bob,
    ...
    function test_claim_rewards() public {
        ...
        assertEq(
            wstEthToken.balanceOf(address(rewardController)),
            1_000e18 - 6.9e18
        );
    }
}

contract MorigamiAaveV3BorrowAndLendTestAccess is
    MorigamiAaveV3BorrowAndLendTestBase
{
    ...
    function test_access_claimRewards() public {
        expectElevatedAccess();
        borrowLend.claimAllRewards(alice, new address[](0), alice);
    }
}

contract MorigamiAaveV3BorrowAndLendTestViews is
    MorigamiAaveV3BorrowAndLendTestBase
{
    ...
    function test_debtBalance() public {
        ...
        assertEq(
            borrowLend.aaveDebtToken().balanceOf(address(borrowLend)),
            25.742658211257227091e18
        );
    }
    ...
    function test_availableToSupply() public {
        ...
        (supplyCap, available) = borrowLend.availableToSupply();
...
contract MorigamiAaveV3BorrowAndLendTestSupply is
    MorigamiAaveV3BorrowAndLendTestBase
{
    ...
    function test_repayAndWithdraw_success() public {
        ...
        (uint256 debtRepaidAmount, uint256 withdrawnAmount) = borrowLend
            .repayAndWithdraw(amount / 2, amount / 2, alice);
