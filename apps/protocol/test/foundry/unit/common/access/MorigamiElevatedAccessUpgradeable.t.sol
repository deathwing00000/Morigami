...
contract MockUpgradeable is
    Initializable,
    MorigamiElevatedAccessUpgradeable,
    UUPSUpgradeable
{
    constructor() {
    ...
    ) internal override onlyElevatedAccess {}
...
contract MorigamiElevatedAccessUpgradeableTestBase is MorigamiTest {
    ...
    function setUp() public {
        ...
        mock = MockUpgradeable(
            address(new ERC1967Proxy(address(template), data))
        );
    }
}

contract MorigamiElevatedAccesUpgradeablesTest is
    MorigamiElevatedAccessUpgradeableTestBase
{
...
contract MorigamiElevatedAccessUpgradeableTestSetters is
    MorigamiElevatedAccessUpgradeableTestBase
{
    event ExplicitAccessSet(
        address indexed account,
        bytes4 indexed fnSelector,
        bool indexed value
    );
    ...
    function test_setExplicitAccess_single() public {
        ...
        bytes4 fnSig2 = bytes4(
            keccak256("someFunctionSignature(uint256,string)")
        );
    ...
    function test_setExplicitAccess_multiple() public {
        ...
        access[0] = IMorigamiElevatedAccess.ExplicitAccess(fnSig, false);
        access[1] = IMorigamiElevatedAccess.ExplicitAccess(fnSig2, true);
...
contract MorigamiElevatedAccessTestUpgradeableModifiers is
    MorigamiElevatedAccessUpgradeableTestBase
{
    function test_onlyElevatedAccess() public {
        ...
        setExplicitAccess(
            mock,
            alice,
            MockUpgradeable.validateOnlyElevatedAccess.selector,
            true
        );
        vm.startPrank(alice);
        mock.validateOnlyElevatedAccess();
    ...
    function test_onlyElevatedAccess_explicitExternal() public {
        ...
        // ie checkSig()
        {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    CommonEventsAndErrors.InvalidAccess.selector
                )
            );
            mock.checkSig();
            ...
            setExplicitAccess(
                mock,
                alice,
                MockUpgradeable.checkSig.selector,
                true
            );
