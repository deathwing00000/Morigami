...
import {MorigamiVolatileCurveEmaOracle} from "contracts/common/oracle/MorigamiVolatileCurveEmaOracle.sol";
...
contract MorigamiVolatileCurveEmaOracleTestBase is MorigamiTest {
    MorigamiVolatileCurveEmaOracle public oOracleReciprocal;
    MorigamiVolatileCurveEmaOracle public oOracleNotReciprocal;

    IERC20Metadata public constant USD0_TOKEN =
        IERC20Metadata(0x73A15FeD60Bf67631dC6cd7Bc5B6e8da8190aCF5);
    IERC20Metadata public constant USD0PP_TOKEN =
        IERC20Metadata(0x35D8949372D46B7a3D5A56006AE77B215fc69bC0);

    ICurveStableSwapNG public constant STABLE_SWAP_NG =
        ICurveStableSwapNG(0x1d08E7adC263CfC70b1BaBe6dC5Bb339c16Eec52);

    function setUp() public {
        fork("mainnet", 20308622);
        vm.warp(1721006984);

        oOracleReciprocal = new MorigamiVolatileCurveEmaOracle(
            origamiMultisig,
        ...
        oOracleNotReciprocal = new MorigamiVolatileCurveEmaOracle(
            origamiMultisig,
            IMorigamiOracle.BaseOracleParams(
                "USD0++/USD0",
                address(USD0PP_TOKEN),
...
contract MorigamiVolatileCurveEmaOracleTestInit is
    MorigamiVolatileCurveEmaOracleTestBase
{
    function test_initialization_reciprocal() public {
        ...
        assertEq(
            address(oOracleReciprocal.stableSwapNg()),
            address(STABLE_SWAP_NG)
        );
        assertEq(oOracleReciprocal.reciprocal(), true);

        (uint128 floor, uint128 ceiling) = oOracleReciprocal
            .validSpotPriceRange();
    ...
    function test_initialization_notReciprocal() public {
        ...
        assertEq(
            address(oOracleNotReciprocal.stableSwapNg()),
            address(STABLE_SWAP_NG)
        );
        assertEq(oOracleNotReciprocal.reciprocal(), false);

        (uint128 floor, uint128 ceiling) = oOracleNotReciprocal
            .validSpotPriceRange();
        assertEq(floor, 0.99e18);
        assertEq(ceiling, 1.01e18);
    }
...
contract MorigamiVolatileCurveEmaOracleTestAdmin is
    MorigamiVolatileCurveEmaOracleTestBase
{
    ...
    function test_setValidSpotPriceRange_fail() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Range.InvalidRange.selector, 5, 4)
        );
...
contract MorigamiVolatileCurveEmaOracleReciprocal_LatestPrice is
    MorigamiVolatileCurveEmaOracleTestBase
{
    function test_latestPrice_success() public {
        assertEq(
            oOracleReciprocal.latestPrice(
    ...
    function test_latestPrices() public {
        (
            ...
            address baseAsset,
            address quoteAsset
        ) = oOracleReciprocal.latestPrices(
                IMorigamiOracle.PriceType.SPOT_PRICE,
                MorigamiMath.Rounding.ROUND_DOWN,
                IMorigamiOracle.PriceType.HISTORIC_PRICE,
                MorigamiMath.Rounding.ROUND_DOWN
...
contract MorigamiVolatileCurveEmaOracleNotReciprocal_LatestPrice is
    MorigamiVolatileCurveEmaOracleTestBase
{
    function test_latestPrice_success() public {
        assertEq(
            oOracleNotReciprocal.latestPrice(
                IMorigamiOracle.PriceType.SPOT_PRICE,
                MorigamiMath.Rounding.ROUND_UP
            ),
    ...
    function test_latestPrices() public {
        (
            ...
            address baseAsset,
            address quoteAsset
        ) = oOracleNotReciprocal.latestPrices(
                IMorigamiOracle.PriceType.SPOT_PRICE,
                MorigamiMath.Rounding.ROUND_UP,
                IMorigamiOracle.PriceType.HISTORIC_PRICE,
                MorigamiMath.Rounding.ROUND_UP
