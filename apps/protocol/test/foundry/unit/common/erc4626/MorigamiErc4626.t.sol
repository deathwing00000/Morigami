...
contract MorigamiErc4626TestBase is MorigamiTest {
    ...
    function deposit(address user, uint256 amount) internal {
        ...
        uint256 expectedFeeAmout = vault.convertToShares(amount) -
            expectedShares;
        if (expectedFeeAmout > 0) {
            vm.expectEmit(address(vault));
            emit InKindFees(
                IMorigamiErc4626.FeeType.DEPOSIT_FEE,
                DEPOSIT_FEE,
                expectedFeeAmout
            );
        }
    ...
    function withdraw(address user, uint256 assets) internal {
        ...
        vm.expectEmit(address(vault));
        emit InKindFees(
            IMorigamiErc4626.FeeType.WITHDRAWAL_FEE,
            WITHDRAWAL_FEE,
            expectedShares -
...
contract MorigamiErc4626TestAdmin is MorigamiErc4626TestBase {
    ...
    function test_default_nofees() public {
        ...
        vault = MockErc4626VaultWithFees(
            address(new MorigamiErc4626(origamiMultisig, "VAULT", "VLT", asset))
        );
        ...
        assertEq(
            ...
            bytes32(
                0xd2e843a44a91122d6c30863b00ef4f6cef005ce2bdfbe801e527b67fd3cc222c
            )
        );
        assertEq(vault.areDepositsPaused(), false);
        assertEq(vault.areWithdrawalsPaused(), false);
    ...
    function test_initialization() public {
        ...
        assertEq(
            vault.DOMAIN_SEPARATOR(),
            bytes32(
                0xf07a1e21026e15847c4f454c9eb8f87a35787510bc37aee10796c2c8aa85ff16
            )
        );
        assertEq(vault.areDepositsPaused(), false);
        assertEq(vault.areWithdrawalsPaused(), false);
    ...
    function test_supportsInterface() public {
        ...
        assertEq(
            vault.supportsInterface(type(IMorigamiInvestment).interfaceId),
            false
        );
    }
    ...
    function test_seedDeposit_failure_maxTooLow() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiErc4626.ERC4626ExceededMaxDeposit.selector,
                origamiMultisig,
                123e18,
                1e18
            )
    ...
    function test_withdraw_fullyUnwind() public {
        ...
        assertEq(
            vault.redeem(
                vault.maxRedeem(origamiMultisig),
                origamiMultisig,
                origamiMultisig
            ),
            0.1e18
...
contract MorigamiErc4626TestDeposit is MorigamiErc4626TestBase {
...
contract MorigamiErc4626TestMint is MorigamiErc4626TestBase {
...
contract MorigamiErc4626TestWithdraw is MorigamiErc4626TestBase {
...
contract MorigamiErc4626TestRedeem is MorigamiErc4626TestBase {
...
contract MorigamiErc4626TestPermit is MorigamiErc4626TestBase {
    ...
    function test_permit() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiErc4626.ERC2612ExpiredSignature.selector,
                99999999
            )
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiErc4626.ERC2612InvalidSigner.selector,
                wrongRecoveryAddr,
                signer
            )
        );
...
contract MorigamiErc4626TestAttacksSB is MorigamiErc4626TestBase {
...
contract MorigamiErc4626TestAttacksJP is MorigamiErc4626TestBase {
    function test_erc4626_donationAttack_withFees() public {
        ...
        assertEq(
            ...
            9796.430765655325281390e18
        );

        vm.startPrank(attacker);
        assertEq(
            vault.redeem(vault.maxRedeem(attacker), attacker, attacker),
            14_775.112743048338451082e18
        );

        // Morigami Multisig ends up with the donated amount!
        vm.startPrank(origamiMultisig);
        assertEq(
            vault.redeem(
                vault.maxRedeem(origamiMultisig),
        ...
        assertEq(
            asset.balanceOf(origamiMultisig),
            10_219.985361470409439467e18
        ); // up big from donation + fees
        assertEq(vault.balanceOf(attacker), 0);
        assertEq(vault.balanceOf(alice), 0);
        assertEq(vault.totalSupply(), 0);
