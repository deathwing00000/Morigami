...
import {MorigamiCowSwapper} from "contracts/common/swappers/MorigamiCowSwapper.sol";
import {IMorigamiCowSwapper} from "contracts/interfaces/common/swappers/IMorigamiCowSwapper.sol";
...
contract MorigamiCowSwapperTestBase is MorigamiTest {
    MorigamiCowSwapper public swapper;
    MorigamiFixedPriceOracle public limitPriceOracle;
    ...
    function setUp() public {
        ...
        DAI = address(
            new DummyMintableToken(origamiMultisig, "DAI", "DAI", 18)
        );
        USDC = address(
            new DummyMintableToken(origamiMultisig, "USDC", "USDC", 6)
        );

        swapper = new MorigamiCowSwapper(origamiMultisig, cowSwapRelayer);
        limitPriceOracle = new MorigamiFixedPriceOracle(
            IMorigamiOracle.BaseOracleParams("DAI/USDC", DAI, 18, USDC, 6),
            0.95e18,
            address(0)
        );
    ...
    function defaultOrderConfig()
        internal
        view
        returns (IMorigamiCowSwapper.OrderConfig memory)
    {
        return
            IMorigamiCowSwapper.OrderConfig({
    ...
    function defaultConditionalOrderParams()
        internal
        view
...
contract MorigamiCowSwapperTestAdmin is MorigamiCowSwapperTestBase {
    ...
    function test_setCowApproval() public {
        ...
        assertEq(
            IERC20(DAI).allowance(address(swapper), cowSwapRelayer),
            12345e18
        );
    ...
    function test_setOrderConfig_failures() public {
        ...
        IMorigamiCowSwapper.OrderConfig memory config = defaultOrderConfig();
        ...
        config.limitPriceOracle = MorigamiFixedPriceOracle(address(0));
        ...
        config.limitPriceOracle = new MorigamiFixedPriceOracle(
            IMorigamiOracle.BaseOracleParams("alice/USDC", alice, 18, USDC, 6),
            0.95e18,
            address(0)
        );
        vm.expectRevert(
    ...
    function test_setOrderConfig_success() public {
        ...
        // Unset to start
        IMorigamiCowSwapper.OrderConfig memory newConfig = swapper.orderConfig(
            DAI
        );
        ...
        newConfig = swapper.orderConfig(DAI);
        {
            ...
            assertEq(
                address(newConfig.limitPriceOracle),
    ...
    function test_updateAmountsAndPremiumBps_failNotConfigured() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiCowSwapper.InvalidSellToken.selector,
                DAI
            )
        );
    ...
    function test_updateAmountsAndPremiumBps_failLimitPricePremium() public {
        vm.startPrank(origamiMultisig);
        IMorigamiCowSwapper.OrderConfig memory config = defaultOrderConfig();
        config.limitPriceOracle = IMorigamiOracle(address(0));
        config.limitPricePremiumBps = 0;
        swapper.setOrderConfig(DAI, config);
...
contract MorigamiCowSwapperTestAccess is MorigamiCowSwapperTestBase {
...
contract MorigamiCowSwapperTestLimitOrders is MorigamiCowSwapperTestBase {
    ...
    function test_getTradeableOrderWithSignature_fail_notConfiguredToken()
        public
    {
        vm.startPrank(origamiMultisig);
    ...
    function test_getTradeableOrderWithSignature_success_2() public {
        vm.startPrank(origamiMultisig);
        IMorigamiCowSwapper.OrderConfig memory config = IMorigamiCowSwapper
            .OrderConfig({
                maxSellAmount: 123e6,
                buyToken: IERC20(DAI),
                minBuyAmount: 1,
        ...
        (GPv2Order.Data memory order, bytes memory signature) = swapper
    ...
    function test_isValidSignature_fail_notConfiguredToken() public {
        ...
        bytes32 hash = GPv2Order.hash(
            order,
            cowSwapSettlement.domainSeparator()
        );
    ...
    function test_isValidSignature_success_lower() public {
        ...
        vm.mockCall(
            address(limitPriceOracle),
            abi.encodeWithSelector(IMorigamiOracle.convertAmount.selector),
        ...
        assertEq(
            swapper.isValidSignature(hash, signature),
            swapper.isValidSignature.selector
        );
    }
    ...
    function test_isValidSignature_fail_recipient() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiCowSwapper.OrderDoesNotMatchTradeableOrder.selector
            )
        );
        swapper.isValidSignature(hash, signature);
    }
...
contract MorigamiCowSwapperTestLimitOrderViews is MorigamiCowSwapperTestBase {
    ...
    function test_getBuyAmount_success_nothingToRound() public {
        configureDai();
        (uint256 buyAmount, uint256 roundedBuyAmount) = swapper.getBuyAmount(
            DAI
        );
    ...
    function test_getBuyAmount_success_somethingToRound_10() public {
        ...
        assertEq(
            buyAmount,
            (forcedAmount * (10_000 + PRICE_PREMIUM_BPS)) / 10_000
        );
    ...
    function test_supportsInterface() public {
        assertEq(
            swapper.supportsInterface(type(IMorigamiCowSwapper).interfaceId),
            true
        );
        assertEq(
            swapper.supportsInterface(type(IConditionalOrder).interfaceId),
            true
        );
        ...
        assertEq(
            swapper.supportsInterface(type(IMorigamiOracle).interfaceId),
            false
        );
    }
}

contract MorigamiCowSwapperTestMarketOrdersSellToken is
    MorigamiCowSwapperTestBase
{
    uint96 public constant USDC_BUY_AMOUNT = 5_000.789e6;
    uint96 public constant DAI_BALANCE = 6_001e18;

    function marketOrderConfig()
        internal
        view
        returns (IMorigamiCowSwapper.OrderConfig memory)
    {
        return
            IMorigamiCowSwapper.OrderConfig({
                ...
                limitPriceOracle: IMorigamiOracle(address(0)),
                recipient: address(swapper),
                roundDownDivisor: 0,
                partiallyFillable: false,
                useCurrentBalanceForSellAmount: true,
    ...
    function test_isValidSignature_success_lower() public {
        ...
        swapper.updateAmountsAndPremiumBps(
            DAI,
            DAI_SELL_AMOUNT,
            (USDC_BUY_AMOUNT * 0.8e6) / 1e6,
            0
        );
    ...
    function test_isValidSignature_fail_recipient() public {
        ...
        // Update the config to a different recipient
        IMorigamiCowSwapper.OrderConfig memory config = marketOrderConfig();
        config.recipient = alice;
        swapper.setOrderConfig(DAI, config);
...
contract MorigamiCowSwapperTestMarketOrdersBuyToken is
    MorigamiCowSwapperTestBase
{
