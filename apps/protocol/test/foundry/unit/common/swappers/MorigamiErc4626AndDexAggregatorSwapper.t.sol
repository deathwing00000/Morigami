...
contract MorigamiErc4626AndDexAggregatorSwapperTestBase is MorigamiTest {
    MorigamiErc4626AndDexAggregatorSwapper public swapper;
    address public constant router = 0x111111125421cA6dc452d289314280a0f8842A65;

    IERC20 public constant DAI =
        IERC20(0x6B175474E89094C44Da98b954EedeAC495271d0F);
    IERC4626 public constant SUSDE =
        IERC4626(0x9D39A5DE30e57443BfF2A8307A4256c8797A3497);
    IERC20 public constant USDE =
...
contract MorigamiErc4626AndDexAggregatorSwapperTestAdmin is
    MorigamiErc4626AndDexAggregatorSwapperTestBase
{
...
contract MorigamiErc4626AndDexAggregatorSwapperTestExecute_NonVault is
    MorigamiErc4626AndDexAggregatorSwapperTestBase
{
    function getQuoteData(
        uint256 fromAmount
    ) internal pure returns (uint256 toAmount, bytes memory swapData) {
        // REQUEST:
        /*
        curl -X GET \
...
-H "content-type: application/json"
    ...
    function encode(bytes memory data) internal pure returns (bytes memory) {
        return
            abi.encode(
                MorigamiErc4626AndDexAggregatorSwapper.RouteData({
                    routeType: MorigamiErc4626AndDexAggregatorSwapper
                        .RouteType
    ...
    function test_execute_fail_invalidRouter() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiSwapper.InvalidRouter.selector,
                alice
            )
        );
        swapper.execute(
    ...
    function test_execute_success_normal() public {
        uint256 sellTokenAmount = 1_000e18;
        (uint256 expectedBuyTokenAmount, bytes memory data) = getQuoteData(
            sellTokenAmount
        );
    ...
    function test_execute_fail_sellRemainder() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IMorigamiSwapper.InvalidSwap.selector)
        );
        swapper.execute(DAI, sellTokenAmount, SDAI, data);
    }
    ...
    function test_execute_failure_balance() public {
        ...
        DAI.approve(address(swapper), sellTokenAmount);
        vm.expectRevert(
            abi.encodeWithSelector(SafeTransferFromFailed.selector)
        );
        swapper.execute(DAI, sellTokenAmount, SDAI, data);
    }
    ...
    function test_execute_failure_customError() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiSwapper.UnknownSwapError.selector,
                ""
            )
        );
        swapper.execute(DAI, sellTokenAmount, SDAI, data);
...
contract MorigamiErc4626AndDexAggregatorSwapperTestExecute_ToVaultDirect is
    MorigamiErc4626AndDexAggregatorSwapperTestBase
{
    function getQuoteData(
        uint256 fromAmount
...
contract MorigamiErc4626AndDexAggregatorSwapperTestExecute_ToVaultWithStake is
    MorigamiErc4626AndDexAggregatorSwapperTestBase
{
    function getQuoteData(
        uint256 fromAmount
    ) internal view returns (uint256 toAmount, bytes memory swapData) {
        // REQUEST:
        /*
        curl -X GET \
...
contract MorigamiErc4626AndDexAggregatorSwapperTestExecute_FromVault is
    MorigamiErc4626AndDexAggregatorSwapperTestBase
{
    function getQuoteData(
        uint256 fromAmount
