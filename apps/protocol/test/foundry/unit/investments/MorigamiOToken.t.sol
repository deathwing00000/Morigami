...
contract MorigamiOTokenTestBase is MorigamiTest {
    MorigamiOToken public oToken;
    MorigamiMockManager public oTokenManager;
    DummyMintableToken public depositToken;
    uint128 public exitFeeBps = 500;

    function setUp() public {
        oToken = new MorigamiOToken(origamiMultisig, "O Token", "oToken");
        depositToken = new DummyMintableToken(
            origamiMultisig,
            "Deposit Token",
            "token",
            18
        );
        oTokenManager = new MorigamiMockManager(
            origamiMultisig,
            address(oToken),
...
contract MorigamiOTokenTestAccess is MorigamiOTokenTestBase {
...
contract MorigamiOTokenTestAdmin is MorigamiOTokenTestBase {
    ...
    function test_setManager_success() public {
        ...
        vm.startPrank(origamiMultisig);
        MorigamiMockManager newManager = new MorigamiMockManager(
            origamiMultisig,
            address(oToken),
            address(depositToken),
            feeCollector,
    ...
    function test_areInvestmentsPaused() public {
        ...
        oTokenManager.setPaused(IMorigamiManagerPausable.Paused(true, false));
    ...
    function test_areExitsPaused() public {
        ...
        vm.prank(origamiMultisig);
        oTokenManager.setPaused(IMorigamiManagerPausable.Paused(false, true));
        assertEq(oToken.areExitsPaused(), true);
    }
}

contract MorigamiOTokenTestInvest is MorigamiOTokenTestBase {
    event Invested(
    ...
    function test_maxInvest() public {
        ...
        assertEq(
            oToken.maxInvest(address(depositToken)),
            oTokenManager.maxInvest(address(depositToken))
        );
    ...
    function test_investQuote() public {
        ...
        ) = oToken.investQuote(1e18, address(depositToken), 100, 0);
    ...
    function test_investWithToken_failureNoTokens() public {
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = oToken
            .investQuote(1e18, address(depositToken), 100, 0);
    ...
    function test_investWithToken_fail_badToken() public {
        vm.startPrank(alice);
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = oToken
            .investQuote(100, address(depositToken), 100, 123);
...
contract MorigamiOTokenTestExit is MorigamiOTokenTestBase {
    event Exited(
        address indexed user,
        uint256 investmentAmount,
    ...
    function test_maxExit() public {
        // Calls the underlying manager
        assertEq(
            oToken.maxExit(address(depositToken)),
            oTokenManager.maxExit(address(depositToken))
        );
    ...
    function test_exitQuote() public {
        ...
        ) = oToken.exitQuote(1e18, address(depositToken), 100, 0);
    ...
    function test_exitToToken_failureNoTokens() public {
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = oToken
            .exitQuote(1e18, address(depositToken), 100, 0);
        vm.expectRevert("ERC20: transfer amount exceeds balance");
        oToken.exitToToken(quoteData, alice);
    }
...
contract MorigamiOTokenTestAmo is MorigamiOTokenTestBase {
