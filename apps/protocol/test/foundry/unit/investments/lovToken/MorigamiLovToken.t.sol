...
contract MorigamiLovTokenTestAdmin is MorigamiLovTokenTestBase {
    ...
    function test_initialization() public {
        ...
        (uint256 assets, uint256 liabilities, uint256 ratio) = lovToken
...
contract MorigamiLovTokenTestAccess is MorigamiLovTokenTestBase {
...
contract MorigamiLovTokenTestViews is MorigamiLovTokenTestBase {
    ...
    function test_assetsAndLiabilities() public {
        ...
        (uint256 assets, uint256 liabilities, uint256 ratio) = lovToken
            .assetsAndLiabilities();
        (uint256 massets, uint256 mliabilities, uint256 mratio) = manager
    ...
    function test_getDynamicFeesBps() public {
        ...
        assertEq(depositFee, 20);
        assertEq(exitFee, 50);
        (uint256 managerDepositFee, uint256 managerExitFee) = manager
            .getDynamicFeesBps();
...
contract MorigamiLovTokenTestInvest is MorigamiLovTokenTestBase {
    using MorigamiMath for uint256;
    event Invested(
    ...
    function test_investWithNative_failure() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IMorigamiInvestment.Unsupported.selector)
        );
    ...
    function test_maxInvest() public {
        assertEq(
            lovToken.maxInvest(address(daiToken)),
            10_020_040.080160320641282565e18
        );
    }
    ...
    function test_investQuote_success_depositAsset() public {
        ...
        uint256 expectedShares = ((depositAmount * 1e18) / sharePrice)
            .subtractBps(expectedFeeBps, MorigamiMath.Rounding.ROUND_DOWN);
    ...
    function test_investQuote_success_reserveAsset() public {
        ...
        uint256 expectedFeeBps = 20;
        uint256 expectedShares = depositAmount.subtractBps(
            expectedFeeBps,
    ...
    function test_investWithToken_fail_badToken() public {
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = lovToken
            .investQuote(100, address(sDaiToken), 100, 123);
        DummyMintableToken badToken = new DummyMintableToken(
            origamiMultisig,
            "BAD",
            "BAD",
            18
    ...
    function test_investWithToken_nothingToMint() public {
        ...
        vm.mockCall(
            address(manager),
            abi.encodeWithSelector(
                IMorigamiOTokenManager.investWithToken.selector
            ),
            abi.encode(0)
        );

        assertEq(lovToken.investWithToken(quoteData), 0);
    }
    ...
    function test_investWithToken_fail_breachedMaxSupply() public {
        ...
        assertEq(
            lovToken.maxInvest(address(sDaiToken)),
            10_020_040.080160320641282565e18
        );
        ...
        // Fails if just over
        {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    CommonEventsAndErrors.BreachedMaxTotalSupply.selector,
                    MAX_TOTAL_SUPPLY + 1,
                    MAX_TOTAL_SUPPLY
                )
            );
...
contract MorigamiLovTokenTestExit is MorigamiLovTokenTestBase {
    using MorigamiMath for uint256;
    ...
    function test_exitQuote_noDeposits() public {
        ...
        ) = lovToken.exitQuote(exitAmount, address(daiToken), slippageBps, 123);
    ...
    function test_exitToToken_fail_slippage() public {
        ...
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = lovToken
            .exitQuote(exitAmount, address(daiToken), slippageBps, 0);
    ...
    function test_exitToToken_nothingToBurn() public {
        ...
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = lovToken
            .exitQuote(shares, address(daiToken), slippageBps, 123);

        // Mock that there's nothing to burn
        vm.mockCall(
            address(manager),
            abi.encodeWithSelector(IMorigamiOTokenManager.exitToToken.selector),
            abi.encode(100, 0)
        );

        vm.startPrank(alice);
        assertEq(lovToken.exitToToken(quoteData, alice), 100);
        // The fees weren't burned
        assertEq(lovToken.totalSupply(), 19.009523809523809523e18);
    }
}

contract MorigamiLovTokenTestFees is MorigamiLovTokenTestBase {
    ...
    function test_accruedPerformanceFee() public {
        ...
        assertEq(
            lovToken.accruedPerformanceFee(),
            (expectedTotalSupply * 500) / 10_000 / 2
        );

        vm.warp(block.timestamp + 182.5 days);
        assertEq(
            lovToken.accruedPerformanceFee(),
            (expectedTotalSupply * 500) / 10_000
        );
    }
    ...
    function test_collectPerformanceFees_success() public {
        ...
        uint256 expectedAmount2 = ((totalSupply + expectedAmount) * 500) /
            10_000 /
            2;
        vm.warp(block.timestamp + 182.5 days);
        vm.expectEmit(address(lovToken));
        ...
        assertEq(
            lovToken.balanceOf(feeCollector),
    ...
    function test_collectPerformanceFees_overMaxTotalSupply() public {
        ...
        assertEq(lovToken.totalSupply(), totalSupply + expectedAmount);
        assertEq(
            totalSupply + expectedAmount,
            10_249_999.999999999999999998e18
        );
        assertEq(lovToken.lastPerformanceFeeTime(), block.timestamp);
