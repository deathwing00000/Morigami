...
contract MorigamiAbstractLovTokenManagerTestAdmin is MorigamiLovTokenTestBase {
    event FeeConfigSet(
    ...
    function test_setUserALRange_fail_floor() public {
        vm.startPrank(origamiMultisig);
        vm.expectRevert(
            abi.encodeWithSelector(Range.InvalidRange.selector, 1e18, 2e18)
        );
    ...
    function test_setUserALRange_fail_order() public {
        vm.startPrank(origamiMultisig);
        vm.expectRevert(
            abi.encodeWithSelector(Range.InvalidRange.selector, 2e18, 1e18)
        );
    ...
    function test_paused() public {
        ...
        manager.setPaused(IMorigamiManagerPausable.Paused(true, false));
        assertEq(manager.areInvestmentsPaused(), true);
        assertEq(manager.areExitsPaused(), false);

        manager.setPaused(IMorigamiManagerPausable.Paused(false, true));
        assertEq(manager.areInvestmentsPaused(), false);
        assertEq(manager.areExitsPaused(), true);
    }
}

contract MorigamiAbstractLovTokenManagerTestAccess is MorigamiLovTokenTestBase {
    ...
    function test_access_investWithToken() public {
        IMorigamiInvestment.InvestQuoteData memory quote;
    ...
    function test_access_exitToToken() public {
        IMorigamiInvestment.ExitQuoteData memory quote;
        vm.prank(origamiMultisig);
        vm.expectRevert(
            abi.encodeWithSelector(CommonEventsAndErrors.InvalidAccess.selector)
        );
...
contract MorigamiAbstractLovTokenManagerTestViews is MorigamiLovTokenTestBase {
    using MorigamiMath for uint256;

    function investWithSDai(
        uint256 sDaiAmount,
        address to
    ) internal override returns (uint256 sharesAmount) {
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = manager
            .investQuote(sDaiAmount, address(sDaiToken), 0, 0);
    ...
    function test_reservesToShares() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiLovTokenManager.NoAvailableReserves.selector
            )
        );
        assertEq(
            manager.reservesToShares(
    ...
    function test_assetToLiabilityRatio() public {
        ...
        assertEq(manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE), 1);
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            1
        );
    ...
    function test_assetsAndLiabilities_hist() public {
        ...
        // No reserves, no liabilities
        (uint256 assets, uint256 liabilities, uint256 ratio) = manager
            .assetsAndLiabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE);
        assertEq(assets, 0);
        assertEq(liabilities, 0);
        assertEq(ratio, type(uint128).max);
...
contract MorigamiAbstractLovTokenManagerTestInvest is MorigamiLovTokenTestBase {
    using MorigamiMath for uint256;
    event InKindFees(
    ...
    function test_maxInvest() public {
        ...
        lovToken.setMaxTotalSupply(type(uint256).max);
    ...
    function test_investQuote_fail() public {
        ...
        manager.investQuote(depositAmount, address(daiToken), slippageBps, 123);
    }
    ...
    function test_investWithToken_failPaused() public {
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = manager
            .investQuote(100, address(sDaiToken), 100, 123);
        vm.startPrank(address(lovToken));
        vm.expectRevert(
            abi.encodeWithSelector(CommonEventsAndErrors.IsPaused.selector)
        );
    ...
    function test_investWithToken_fail_zero() public {
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = manager
            .investQuote(100, address(daiToken), slippageBps, 123);
    ...
    function test_investWithToken_fail_slippage() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                quoteData.expectedInvestmentAmount + 1,
    ...
    function test_investWithToken_success_depositToken() public {
        ...
        uint256 expectedShares = MorigamiMath.subtractBps(
        ...
        assertEq(
            sDaiToken.balanceOf(address(manager)),
    ...
    function test_invesWithToken_success_afterFirstSupply() public {
        ...
        emit InKindFees(
            DynamicFees.FeeType.DEPOSIT_FEE,
            20,
            0.038019047619047620e18
        );
        uint256 shares = manager.investWithToken(alice, quoteData);
        uint256 expectedShares = 18.971504761904761903e18;
        assertEq(shares, expectedShares);
...
contract MorigamiAbstractLovTokenManagerTestExit is MorigamiLovTokenTestBase {
    using MorigamiMath for uint256;
    event InKindFees(
        DynamicFees.FeeType feeType,
        uint256 feeBps,
    ...
    function test_maxExit() public {
        ...
        assertEq(manager.maxExit(address(daiToken)), 582.411789473684210048e18);
        ...
        assertEq(
            (expectedShares * (10_000 - 500)) / 10_000 + 1,
            manager.reservesToShares(
                100e18,
                IMorigamiOracle.PriceType.SPOT_PRICE
            )
    ...
    function test_exitQuote_fail_zero() public {
        ...
        manager.exitQuote(exitAmount, address(daiToken), slippageBps, 123);
    }
    ...
    function test_exitQuote_noDeposits() public {
        ...
        assertEq(
            ...
            ((exitAmount * sharePrice1) / 1e18).subtractBps(
                MIN_EXIT_FEE_BPS,
                MorigamiMath.Rounding.ROUND_DOWN
            )
        );
        assertEq(
            ...
            quoteData.expectedToTokenAmount.subtractBps(
                slippageBps,
                MorigamiMath.Rounding.ROUND_UP
            )
        );
    ...
    function test_exitToToken_success_depositToken() public {
        ...
        vm.startPrank(address(lovToken));
        vm.expectEmit(address(manager));
        emit InKindFees(DynamicFees.FeeType.EXIT_FEE, 50, 0.075e18);
        (uint256 toTokenAmount, uint256 toBurnAmount) = manager.exitToToken(
            alice,
            quoteData,
            alice
        );
        ...
        assertEq(
            manager.reservesBalance(),
            100e18 + (900e18 + 901) - expectedSDaiAmount
        );
        assertEq(
            sDaiToken.balanceOf(address(manager)),
            manager.reservesBalance()
