...
contract MorigamiLovTokenFlashAndBorrowManagerTestBase is MorigamiTest {
    using MorigamiMath for uint256;

    MorigamiAaveV3FlashLoanProvider internal flProvider;
    IERC20 internal wethToken;
    IERC20 internal stEthToken;
    IERC20 internal wstEthToken;
    MorigamiLovToken internal lovToken;
    MorigamiLovTokenFlashAndBorrowManager internal manager;
    TokenPrices internal tokenPrices;
    DummyLovTokenSwapper internal swapper;
    MorigamiAaveV3BorrowAndLend internal borrowLend;
    ...
    address internal constant SPARK_POOL_ADDRESS_PROVIDER =
        0x02C3eA4e34C0cBd694D2adFa2c690EECbC1793eE;
    address internal constant SPARK_POOL =
    ...
    address internal constant STETH_ETH_ORACLE =
        0x86392dC19c0b719886221c78AB11eb8Cf5c52812;
    address internal constant SPARK_A_WSTETH_ADDRESS =
        0x12B54025C112Aa61fAce2CDB7118740875A566E9;
    address internal constant SPARK_D_WETH_ADDRESS =
    ...
    uint128 internal constant STETH_ETH_STALENESS_THRESHOLD =
        1 days + 15 minutes; // It should update every 86400 seconds. So set to 1day 15mins
    ...
    function setUp() public {
        ...
        flProvider = new MorigamiAaveV3FlashLoanProvider(
            SPARK_POOL_ADDRESS_PROVIDER
        );
        ...
        manager = new MorigamiLovTokenFlashAndBorrowManager(
            origamiMultisig,
            address(wstEthToken),
            address(wethToken),
            address(stEthToken),
    ...
    function investLovStEth(
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = lovToken
            .investQuote(amount, address(wstEthToken), 0, 0);

        amountOut = lovToken.investWithToken(quoteData);
    }

    function exitLovStEth(
        address account,
        uint256 amount,
        address recipient
    ) internal returns (uint256 amountOut) {
        ...
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = lovToken
            .exitQuote(amount, address(wstEthToken), 0, 0);
    ...
    function doRebalanceDown(
        ...
        IMorigamiLovTokenFlashAndBorrowManager.RebalanceDownParams
            memory params;
        (params, reservesAmount) = rebalanceDownParams(
            targetAL,
            slippageBps,
            alSlippageBps
        );
        ...
        )
    {
        ...
        params.swapData = abi.encode(
            DummyLovTokenSwapper.SwapData({buyTokenAmount: reservesAmount})
        );

        params.minNewAL = uint128(
        ...
        params.minExpectedReserveToken = MorigamiMath.subtractBps(
            reservesAmount,
            swapSlippageBps,
            MorigamiMath.Rounding.ROUND_DOWN
        );
    ...
    function doRebalanceUp(
        ...
        IMorigamiLovTokenFlashAndBorrowManager.RebalanceUpParams
            memory params = rebalanceUpParams(
                targetAL,
                slippageBps,
                alSlippageBps
            );
        ...
        )
    {
        ...
        params.collateralToWithdraw = LovTokenHelpers.solveRebalanceUpAmount(
            manager,
            targetAL
        );
        ...
        params.flashLoanAmount = params.flashLoanAmount.mulDiv(
            ...
            MorigamiMath.BASIS_POINTS_DIVISOR + flFee,
            MorigamiMath.Rounding.ROUND_DOWN
        );
        ...
        params.flashLoanAmount = params.flashLoanAmount.subtractBps(
            swapSlippageBps,
            MorigamiMath.Rounding.ROUND_DOWN
        );
...
contract MorigamiLovTokenFlashAndBorrowManagerTestAdmin is
    MorigamiLovTokenFlashAndBorrowManagerTestBase
{
    event OraclesSet(
    ...
    function test_initialization() public {
        ...
        assertEq(manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE), 0);
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            0
        );
        assertEq(manager.reservesBalance(), 0);
        assertEq(manager.assetToLiabilityRatio(), type(uint128).max);
        assertEq(
            manager.effectiveExposure(IMorigamiOracle.PriceType.SPOT_PRICE),
            type(uint128).max
        );
        assertEq(
            manager.effectiveExposure(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            type(uint128).max
        );
        assertEq(
            manager.userRedeemableReserves(
                IMorigamiOracle.PriceType.SPOT_PRICE
            ),
            0
        );
    ...
    function test_setSwapper_success() public {
        ...
        assertEq(
            wstEthToken.allowance(address(manager), alice),
            type(uint256).max
        );
        assertEq(
            wethToken.allowance(address(manager), alice),
            type(uint256).max
        );
        ...
        assertEq(
            wstEthToken.allowance(address(manager), bob),
            type(uint256).max
        );
    ...
    function test_setOracleConfig_fail() public {
        ...
        MorigamiWstEthToEthOracle badOracle = new MorigamiWstEthToEthOracle(
            IMorigamiOracle.BaseOracleParams(
                "wstETH/alice",
                address(wstEthToken),
                18,
        ...
        badOracle = new MorigamiWstEthToEthOracle(
            IMorigamiOracle.BaseOracleParams(
                "alice/ETH",
                alice,
                18,
    ...
    function test_setOracles() public {
        ...
        MorigamiWstEthToEthOracle oracle1 = new MorigamiWstEthToEthOracle(
            IMorigamiOracle.BaseOracleParams(
                "wstETH/ETH",
                address(wstEthToken),
                18,
        ...
        MorigamiWstEthToEthOracle oracle2 = new MorigamiWstEthToEthOracle(
            IMorigamiOracle.BaseOracleParams(
                "stETH/ETH",
                address(stEthToken),
                18,
    ...
    function test_setUserAlRange_failValidate() public {
        vm.startPrank(origamiMultisig);
        vm.expectRevert(
            abi.encodeWithSelector(Range.InvalidRange.selector, 1.10e18, 2e18)
        );
...
contract MorigamiLovTokenFlashAndBorrowManagerTestAccess is
    MorigamiLovTokenFlashAndBorrowManagerTestBase
{
...
contract MorigamiLovTokenFlashAndBorrowManagerTestViews is
    MorigamiLovTokenFlashAndBorrowManagerTestBase
{
    function test_reservesBalance() public {
        ...
        assertEq(
            manager.reservesBalance(),
            IERC20(SPARK_A_WSTETH_ADDRESS).balanceOf(address(borrowLend))
        );
    ...
    function test_liabilities_success() public {
        ...
        investLovStEth(alice, amount);
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
            0
        );
        assertEq(manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE), 0);
        assertEq(
        ...
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
            467.986509655154274272e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            404.530744336569579289e18
        ); // weth / stEthToEthPrice / wstEthToStEthPrice
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            404.409355519127022522e18
        ); // weth / 1.0 / wstEthToStEthPrice

        doRebalanceUp(rebalanceALRange.ceiling, 0, 5);
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
            321.350736629872601532e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            277.777777777777777662e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            277.694424123133888682e18
        );
    ...
    function test_liabilities_withDebt_notPricingToken() public {
...

        {
            ...
            MorigamiStableChainlinkOracle oOne = new MorigamiStableChainlinkOracle(
                    origamiMultisig,
                    IMorigamiOracle.BaseOracleParams(
                        "ONE/ONE",
                        address(wethToken),
            ...
            MorigamiCrossRateOracle ethToWstEth = new MorigamiCrossRateOracle(
                IMorigamiOracle.BaseOracleParams(
                    "ETH/wstETH",
                    address(wethToken),
                    18,
        ...
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            404.530744336569579360e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            404.409355519127022552e18
        );
    }
...
contract MorigamiLovTokenFlashAndBorrowManagerTestInvest is
    MorigamiLovTokenFlashAndBorrowManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function test_maxInvest_reserveToken() public {
        ...
        (, uint256 expectedAvailable) = borrowLend.availableToSupply();
        {
            ...
            assertEq(
                manager.maxInvest(address(wstEthToken)),
                expectedAvailable
            );
        }
...

        {
            ...
            (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = lovToken
                .investQuote(investAmount, address(wstEthToken), 0, 0);

            vm.expectRevert(
                abi.encodeWithSelector(
                    IMorigamiLovTokenManager.ALTooHigh.selector,
                    1.1236e18,
                    1.160000000000000001e18,
                    1.16e18
                )
            );
    ...
    function test_investQuote_badToken_gives0() public {
        ...
        ) = manager.investQuote(100, alice, 100, 123);
    ...
    function test_investQuote_reserveToken() public {
        (
            IMorigamiInvestment.InvestQuoteData memory quoteData,
            uint256[] memory investFeeBps
        ) = manager.investQuote(1e18, address(wstEthToken), 100, 123);
    ...
    function test_investWithToken_fail_badToken() public {
        uint256 amount = 1e18;
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = manager
            .investQuote(amount, address(wstEthToken), 100, 123);
    ...
    function test_investWithToken_success_aaveUpstreamPolicy() public {
        ...
        {
            vm.startPrank(
                IPoolAddressesProvider(SPARK_POOL_ADDRESS_PROVIDER)
                    .getPoolConfigurator()
            );
            AaveDataTypes.EModeCategory memory catData = borrowLend
                .aavePool()
                .getEModeCategoryData(SPARK_EMODE_ETH);
            catData.ltv = 8900;
            borrowLend.aavePool().configureEModeCategory(
                SPARK_EMODE_ETH,
                catData
            );
        }
...
contract MorigamiLovTokenFlashAndBorrowManagerTestExit is
    MorigamiLovTokenFlashAndBorrowManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function test_maxExit_reserveToken() public {
        ...
        // Rebalance down properly
        {
            ...
            assertEq(
                manager.maxExit(address(wstEthToken)),
                0.306591722023505365e18
            );
        }
        ...
        // Do a large external borrow to use all the wstETH supply up
        {
            ...
            AaveDataTypes.ReserveData memory _reserveData = pool.getReserveData(
                address(wstEthToken)
            );
            ...
            // Set the borrow cap to the supply cap
            {
                ...
                AaveReserveConfiguration.setBorrowCap(
                    _reserveData.configuration,
                    800_000
                );
    ...
    function test_exitQuote_badToken_gives0() public {
        ...
        ) = manager.exitQuote(100, alice, 100, 123);
    ...
    function test_exitQuote_reserveToken() public {
        (
            IMorigamiInvestment.ExitQuoteData memory quoteData,
            uint256[] memory exitFeeBps
        ) = manager.exitQuote(1e18, address(wstEthToken), 100, 123);
    ...
    function test_exitToToken_fail_badToken() public {
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = manager
            .exitQuote(1e18, address(wstEthToken), 100, 123);
    ...
    function test_exitToToken_success() public {
        ...
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = manager
            .exitQuote(shares, address(wstEthToken), 100, 123);

        vm.startPrank(address(lovToken));
        (uint256 amountBack, uint256 toBurn) = manager.exitToToken(
            alice,
            quoteData,
            bob
        );
    ...
    function test_exitToToken_fail_aaveUpstreamPolicy() public {
        ...
        vm.startPrank(address(lovToken));
        vm.expectRevert(
            bytes(AaveErrors.HEALTH_FACTOR_LOWER_THAN_LIQUIDATION_THRESHOLD)
        );
        manager.exitToToken(alice, quoteData, bob);
    }
}

contract MorigamiLovTokenFlashAndBorrowManagerTestRebalanceDown is
    MorigamiLovTokenFlashAndBorrowManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function test_rebalanceDown_fail_fresh() public {
        ...
        (
            IMorigamiLovTokenFlashAndBorrowManager.RebalanceDownParams
                memory params,
            uint256 reservesAmount
        ) = rebalanceDownParams(targetAL, slippageBps, slippageBps);
        doMint(wstEthToken, address(swapper), reservesAmount);
    ...
    function test_rebalanceDown_fail_slippage() public {
        ...
        bytes memory swapData = abi.encode(
            DummyLovTokenSwapper.SwapData({buyTokenAmount: flashLoanAmount})
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CommonEventsAndErrors.Slippage.selector,
                flashLoanAmount + 1,
                flashLoanAmount
            )
        );
    ...
    function test_rebalanceDown_success_noSupply() public {
        ...
        (uint256 assets, uint256 liabilities, uint256 ratio) = manager
            .assetsAndLiabilities(IMorigamiOracle.PriceType.SPOT_PRICE);
        assertEq(assets, 10e18);
        assertEq(liabilities, 8.644068493227648519e18);
        assertEq(ratio, 1.156862651867541365e18);

        (assets, liabilities, ratio) = manager.assetsAndLiabilities(
            IMorigamiOracle.PriceType.HISTORIC_PRICE
        );
        assertEq(assets, 10e18);
    ...
    function test_rebalanceDown_fail_al_validation() public {
        ...
        // Meaning we have more reserves, so A/L is higher than we started out.
        {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    IMorigamiLovTokenManager.ALTooHigh.selector,
                    1.123799999999999999e18,
                    1.123923101777059773e18,
                    1.123799999999999999e18
                )
            );
            manager.rebalanceDown(params);
    ...
    function test_rebalanceDown_success_withEvent() public {
        ...
        assertEq(
            IERC20(SPARK_A_WSTETH_ADDRESS).balanceOf(address(borrowLend)),
            amount + expectedCollateralAdded
        );
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
    ...
    function test_rebalanceDown_success_withFlashloanFee() public {
        ...
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            404.935275080906148868e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            404.813764874646149544e18
        );
...
contract MorigamiLovTokenFlashAndBorrowManagerTestRebalanceUp is
    MorigamiLovTokenFlashAndBorrowManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function test_rebalanceUp_fail_noAaveDebt() public {
        IMorigamiLovTokenFlashAndBorrowManager.RebalanceUpParams
            memory params = IMorigamiLovTokenFlashAndBorrowManager
                .RebalanceUpParams({
                    collateralToWithdraw: 10e18,
                    flashLoanAmount: 10e18,
                    swapData: abi.encode(
                        DummyLovTokenSwapper.SwapData({buyTokenAmount: 10e18})
                    ),
    ...
    function test_rebalanceUp_fail_repayTooMuch() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CommonEventsAndErrors.InvalidAmount.selector,
                address(wethToken),
                10e18
            )
        );
    ...
    function test_rebalanceUp_fail_withdrawMaxCollateral() public {
        ...
        IMorigamiLovTokenFlashAndBorrowManager.RebalanceUpParams memory params;
        {
    ...
    function test_rebalanceUp_success_forceRepayTooMuch_noSurplus() public {
        ...
        uint256 currentDebt = IERC20(SPARK_D_WETH_ADDRESS).balanceOf(
            address(borrowLend)
        );
        assertEq(
            IERC20(SPARK_A_WSTETH_ADDRESS).balanceOf(address(borrowLend)),
        ...
        assertEq(wstEthToken.balanceOf(address(manager)), 0);
        assertEq(
            wethToken.balanceOf(address(borrowLend)),
            0.542864351199979363e18
        );
        assertEq(
            IERC20(SPARK_A_WSTETH_ADDRESS).balanceOf(address(borrowLend)),
            49.530744336569579290e18
        );
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
            0
    ...
    function test_rebalanceUp_success_forceRepayTooMuch_withSurplus() public {
        ...
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            0.030744336569578939e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            0.030735111019453304e18
        );
        ...
        assertEq(
            IERC20(SPARK_A_WSTETH_ADDRESS).balanceOf(address(borrowLend)),
            50.030744336569579289e18
        );
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
            0.035566974733791320e18
        );
    }
    ...
    function test_rebalanceUp_fail_slippage() public {
        ...
        IMorigamiLovTokenFlashAndBorrowManager.RebalanceUpParams
            memory params = rebalanceUpParams(targetAL, 10, 50);
        params.swapData = abi.encode(
            DummyLovTokenSwapper.SwapData({
    ...
    function test_rebalanceUp_fail_al_validation() public {
        ...
        targetAL = TARGET_AL + 0.05e18;
        IMorigamiLovTokenFlashAndBorrowManager.RebalanceUpParams
            memory params = rebalanceUpParams(targetAL, 0, 50);
        ...
        // Meaning we withdraw more collateral, so A/L is higher than we started out.
        {
            ...
            params.collateralToWithdraw =
                (params.collateralToWithdraw * 1.2e18) /
                1e18;
            vm.expectRevert(
                abi.encodeWithSelector(
                    IMorigamiLovTokenManager.ALTooLow.selector,
                    1.123599999999999999e18,
                    1.092693851132686084e18,
                    1.123599999999999999e18
                )
            );
            manager.rebalanceUp(params);
    ...
    function test_rebalanceUp_success_withEvent() public {
        ...
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            288.018433179723502200e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            287.932006579286290124e18
        );
        ...
        assertEq(
            IERC20(SPARK_A_WSTETH_ADDRESS).balanceOf(address(borrowLend)),
            338.018433179723502301e18
        );
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
            333.197768395029195156e18
        );
    }
    ...
    function test_rebalanceUp_success_withFlashloanFee() public {
        ...
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            285.807314916750736071e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            285.721551813550571054e18
        );
        ...
        assertEq(
            IERC20(SPARK_A_WSTETH_ADDRESS).balanceOf(address(borrowLend)),
            335.283656212250011184e18
        );
        assertEq(
            IERC20(SPARK_D_WETH_ADDRESS).balanceOf(address(borrowLend)),
            330.639808257733769232e18
        );
    }
    ...
    function test_rebalanceUp_success_surplusUnderThreshold() public {
        ...
        targetAL = TARGET_AL + 0.05e18;
        IMorigamiLovTokenFlashAndBorrowManager.RebalanceUpParams
            memory params = rebalanceUpParams(targetAL, 20, 50);
        ...
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            288.251457802037194354e18
        );
        assertEq(
            manager.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            288.164961277165971589e18
        );
