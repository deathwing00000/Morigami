...
import {MorigamiLovTokenTestBase} from "test/foundry/unit/investments/lovToken/MorigamiLovTokenBase.t.sol";
...
contract MorigamiLovTokenErc4626ManagerTestBase is MorigamiLovTokenTestBase {
    ...
    MorigamiLovTokenErc4626Manager public managerImpl;
    ...
    MorigamiStableChainlinkOracle public origamiUsdcUsdOracle; // 6dp USDC
    MorigamiStableChainlinkOracle public origamiIUsdcUsdOracle; // 18dp iUSDC
    MorigamiCrossRateOracle public daiUsdcOracle; // 6dp USDC
    MorigamiCrossRateOracle public daiIUsdcOracle; // 18dp iUSDC
    ...
    function setUp() public override {
        ...
        managerImpl = new MorigamiLovTokenErc4626Manager(
            origamiMultisig,
            address(daiToken),
            address(usdcToken),
            address(sDaiToken),
        ...
        MorigamiDebtToken iUsdc = new MorigamiDebtToken(
            "Morigami iUSDC",
            "iUSDC",
            origamiMultisig
        );
        lendingClerk = new DummyLendingClerk(
            address(usdcToken),
            address(iUsdc)
        );
    ...
    function doRebalanceDown(
        uint256 targetAL
    ) internal override returns (uint256 reservesAmount) {
        // reserves (sDAI) amount
        reservesAmount = solveRebalanceDownAmount(
            address(managerImpl),
            targetAL
        );
    ...
    function doRebalanceDownFor(uint256 reservesAmount) internal {
        ...
        DummyLovTokenSwapper.SwapData memory swapData = DummyLovTokenSwapper
            .SwapData({
...
contract MorigamiLovTokenErc4626ManagerTestAdmin is
    MorigamiLovTokenErc4626ManagerTestBase
{
    function test_initialization() public {
        ...
        assertEq(
            address(managerImpl.debtAssetToDepositAssetOracle()),
            address(daiIUsdcOracle)
        );
        ...
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            0
        );
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            0
        );
        assertEq(managerImpl.reservesBalance(), 0);
        assertEq(managerImpl.assetToLiabilityRatio(), type(uint128).max);
        assertEq(
            managerImpl.effectiveExposure(IMorigamiOracle.PriceType.SPOT_PRICE),
            type(uint128).max
        );
        assertEq(
            managerImpl.effectiveExposure(
                IMorigamiOracle.PriceType.HISTORIC_PRICE
            ),
            type(uint128).max
        );
        assertEq(
            managerImpl.userRedeemableReserves(
                IMorigamiOracle.PriceType.SPOT_PRICE
            ),
            0
        );
    ...
    function test_setLendingClerk_success() public {
        ...
        assertEq(
            usdcToken.allowance(address(managerImpl), alice),
            type(uint256).max
        );
        ...
        assertEq(
            usdcToken.allowance(address(managerImpl), bob),
            type(uint256).max
        );
    }
    ...
    function test_setSwapper_success() public {
        ...
        assertEq(
            daiToken.allowance(address(managerImpl), alice),
            type(uint256).max
        );
        assertEq(
            usdcToken.allowance(address(managerImpl), alice),
        ...
        assertEq(
            daiToken.allowance(address(managerImpl), bob),
            type(uint256).max
        );
        assertEq(usdcToken.allowance(address(managerImpl), alice), 0);
        assertEq(
    ...
    function test_setOracleConfig_fail() public {
        ...
        MorigamiStableChainlinkOracle badOracle = new MorigamiStableChainlinkOracle(
                origamiMultisig,
                IMorigamiOracle.BaseOracleParams(
                    "alice/IUSDC",
                    alice,
        ...
        badOracle = new MorigamiStableChainlinkOracle(
            origamiMultisig,
            IMorigamiOracle.BaseOracleParams(
                "DAI/alice",
                address(daiToken),
    ...
    function test_setOracle() public {
        ...
        assertEq(
            address(managerImpl.debtAssetToDepositAssetOracle()),
            address(daiUsdcOracle)
        );

        MorigamiCrossRateOracle oracle = new MorigamiCrossRateOracle(
            IMorigamiOracle.BaseOracleParams(
                "DAI/IUSDC",
                address(daiToken),
                18,
...
contract MorigamiLovTokenErc4626ManagerTestAccess is
    MorigamiLovTokenErc4626ManagerTestBase
{
...
contract MorigamiLovTokenErc4626ManagerTestRebalanceDown is
    MorigamiLovTokenErc4626ManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function test_rebalanceDown_fail_fresh() public {
        ...
        DummyLovTokenSwapper.SwapData memory swapData = DummyLovTokenSwapper
            .SwapData({buyTokenAmount: 100e18});
    ...
    function test_rebalanceDown_fail_slippage() public {
        ...
        // reserves (sDAI) amount
        uint256 reservesAmount = solveRebalanceDownAmount(
            address(managerImpl),
            targetAL
        );
    ...
    function test_rebalanceDown_fail_al_validation() public {
        ...
        // Meaning we have more reserves, so A/L is higher than we started out.
        {
            ...
            swapData = DummyLovTokenSwapper.SwapData(
                (daiDepositAmount * 1.2e18) / 1e18
            );

            vm.expectRevert(
                abi.encodeWithSelector(
                    IMorigamiLovTokenManager.ALTooHigh.selector,
                    1.111200000466539707e18,
                    1.111359714099955673e18,
                    1.111200000466539707e18
                )
            );
            managerImpl.rebalanceDown(
    ...
    function test_rebalanceDown_success() public {
        ...
        assertEq(
            managerImpl.reservesBalance(),
            100e18 + 900.900900900900900901e18
        );
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            900.900900466820806851e18
        );
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            901.238779e18
        ...
        assertEq(
            sDaiToken.balanceOf(address(managerImpl)),
            100e18 + 900.900900900900900901e18
        );
    }
...
contract MorigamiLovTokenErc4626ManagerTestRebalanceUp is
    MorigamiLovTokenErc4626ManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function test_rebalanceUp_fail_slippage() public {
        ...
        // reserves (sDAI) amount
        uint256 reservesAmount = solveRebalanceUpAmount(
            address(managerImpl),
            targetAL
        );
        ...
        DummyLovTokenSwapper.SwapData memory swapData = DummyLovTokenSwapper
            .SwapData({buyTokenAmount: minUsdcRepayAmount});
    ...
    function test_rebalanceUp_fail_al_validation() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiLovTokenManager.ALTooLow.selector,
                1.100000000189139637e18,
                1.129999999765167550e18,
                1.14e18
            )
        );
        managerImpl.rebalanceUp(
        ...
        // Meaning A/L is lower than we started out.
        {
            ...
            reservesAmount = solveRebalanceUpAmount(
                address(managerImpl),
                targetAL
            );
    ...
    function test_rebalanceUp_fail_al_ceiling() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiLovTokenManager.ALTooHigh.selector,
                1.110000000301817474e18,
                1.149999999016916610e18,
                1.14e18
            )
        );
        managerImpl.rebalanceUp(
    ...
    function test_rebalanceUp_success() public {
        ...
        assertEq(
            managerImpl.reservesBalance(),
            1_009.090909090909090910e18 - 242.424240529128609040e18
        );
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            666.666668884495139473e18
        );
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            666.916699e18
...
contract MorigamiLovTokenErc4626ManagerTestInvest is
    MorigamiLovTokenErc4626ManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function exitToSDai(uint256 amount, address to) internal returns (uint256) {
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = lovToken
            .exitQuote(amount, address(sDaiToken), 0, 0);
    ...
    function test_maxInvest_depositAsset() public {
        ...
        // Close to: usdcDaiPrice.mulDiv(1e12, sDaiPrice, MorigamiMath.Rounding.ROUND_UP))
        ...
        assertEq(
            managerImpl.assetToLiabilityRatio(),
            1_050_404_301_367.940673477109787377e18
        );
        assertEq(
            managerImpl.maxInvest(address(daiToken)),
            340_154_792_458_435.510413478806428735e18
        );
        ...
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            9_003_465.440301904761904762e18
        );
        // Share price isn't accurate for full precision, so fully recalculate sdai from first principles
        uint256 expectedFreeShares = (((expectedLiabilities) *
            type(uint128).max) / 1e18) - expectedReserves;
        uint256 expectedFreeAssets = expectedFreeShares.mulDiv(
            sDaiToken.totalAssets(),
            sDaiToken.totalSupply(),
        ...
        // so a little more dust is required to actually tip it over.
        {
            ...
            (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = lovToken
                .investQuote(investAmount, address(daiToken), 0, 0);

            vm.startPrank(alice);
            vm.expectRevert(
                abi.encodeWithSelector(
                    IMorigamiLovTokenManager.ALTooHigh.selector,
                    1.111111111111194928e18,
                    1.200000000000000001e18,
                    1.2e18
                )
            );
    ...
    function test_maxInvest_depositAsset_withMaxTotalSupply() public {
        ...
        assertEq(
            managerImpl.maxInvest(address(daiToken)),
            2_210_526.315789473684210527e18
        );
        ...
        assertEq(
            managerImpl.maxInvest(address(daiToken)),
            2_326_858.753462603878116344e18
        );
        ...
        assertEq(
            managerImpl.maxInvest(address(daiToken)),
            2_326_858.764625964605751428e18
        );
        ...
        assertEq(
            managerImpl.maxInvest(address(daiToken)),
            1_344_061.150107480596298090e18
        ...
        assertEq(
            managerImpl.maxInvest(address(daiToken)),
            1_344_061.150108393108150186e18
        ); // restricted by total supply
    ...
    function test_maxInvest_reserveToken() public {
        ...
        assertEq(
            managerImpl.maxInvest(address(sDaiToken)),
            323_956_945_198_510.009917598863265461e18
        );
        ...
        // so a little more dust is required to actually tip it over.
        {
            ...
            (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = lovToken
                .investQuote(investAmount, address(sDaiToken), 0, 0);
    ...
    function test_maxInvest_reserveToken_withMaxTotalSupply() public {
        ...
        assertEq(
            managerImpl.maxInvest(address(sDaiToken)),
            2_105_263.157894736842105263e18
        );
        ...
        assertEq(
            managerImpl.maxInvest(address(sDaiToken)),
            2_216_055.955678670360110803e18
        );
        ...
        assertEq(
            managerImpl.maxInvest(address(sDaiToken)),
            2_216_055.966310442481668026e18
        );
        ...
        assertEq(
            managerImpl.maxInvest(address(sDaiToken)),
            1_280_058.238197600567902942e18
        );
        ...
        assertEq(
            managerImpl.maxInvest(address(sDaiToken)),
            1_280_058.238198469626809700e18
        );
    ...
    function test_investQuote_badToken_gives0() public {
        ...
        ) = managerImpl.investQuote(100, alice, 100, 123);
    ...
    function test_investQuote_depositToken_success_fresh() public {
        ...
        assertEq(
            ...
            (depositAmount * 0.999e18) / sharePrice
        );
        assertEq(
            quoteData.minInvestmentAmount,
            quoteData.expectedInvestmentAmount.subtractBps(
                slippageBps,
                MorigamiMath.Rounding.ROUND_UP
            )
        );
    ...
    function test_investWithToken_fail_badToken() public {
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = managerImpl
            .investQuote(depositAmount, address(sDaiToken), slippageBps, 123);
    ...
    function test_investWithToken_success_depositAsset() public {
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = managerImpl
            .investQuote(depositAmount, address(daiToken), slippageBps, 123);
...
contract MorigamiLovTokenErc4626ManagerTestExit is
    MorigamiLovTokenErc4626ManagerTestBase
{
    using MorigamiMath for uint256;
    ...
    function test_maxExit_depositAsset() public {
        ...
        assertEq(
            managerImpl.maxExit(address(daiToken)),
            lovToken.totalSupply()
        );
    ...
    function test_maxExit_reserveToken() public {
        ...
        // Capped at the total supply
        assertEq(
            managerImpl.maxExit(address(sDaiToken)),
            lovToken.totalSupply()
        );
    ...
    function test_exitQuote_badAsset_gives0() public {
        ...
        ) = managerImpl.exitQuote(100, alice, 100, 123);
        ...
        uint256 expectedFee = ((15 * 1e18 * (expectedSpotPrice - 1e18)) /
            expectedSpotPrice /
            (10 ** (18 - 4))) + 1; // rounded up
    ...
    function test_exitToToken_fail_badToken() public {
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = managerImpl
            .exitQuote(100e18, address(daiToken), 100, 123);
    ...
    function test_exitToToken_success_depositToken() public {
        ...
        investWithSDai(100e18, alice);
        assertEq(
            managerImpl.sharesToReserves(
                1e18,
                IMorigamiOracle.PriceType.SPOT_PRICE
            ),
            1.001001001001001001e18
        );
        assertEq(
            managerImpl.sharesToReserves(
                1e18,
        ...
        assertEq(
            ...
            0.997622222222222231e18
        );
        assertEq(
            managerImpl.reservesBalance(),
            100e18 + expectedRebalanceReserves
        );
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            899.999999345436839954e18
        );
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            900.337540000000000000e18
        );
        ...
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.SPOT_PRICE),
            887.837837192120125900e18
        ); // previous liabilities * sDaiSharePrice0 / sDaiSharePrice
        assertEq(
            managerImpl.liabilities(IMorigamiOracle.PriceType.HISTORIC_PRICE),
            888.170816486486486487e18
        );

        uint256 slippageBps = 100;
        uint256 exitAmount = 15e18;
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = managerImpl
            .exitQuote(exitAmount, address(daiToken), slippageBps, 123);
        ...
        vm.startPrank(address(lovToken));
        (uint256 toTokenAmount, uint256 toBurnAmount) = managerImpl.exitToToken(
            alice,
            quoteData,
            alice
        );
    ...
    function test_exitToToken_success_reserveToken() public {
        ...
        uint256 exitAmount = 15e18;
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = managerImpl
            .exitQuote(exitAmount, address(sDaiToken), slippageBps, 123);
...
contract MorigamiLovTokenErc4626ManagerTestViews is
    MorigamiLovTokenErc4626ManagerTestBase
{
    ...
    function test_getDynamicFeesBps_notPricingToken() public {
        (uint256 depositFeeBps, uint256 exitFeeBps) = managerImpl
            .getDynamicFeesBps();
    ...
    function test_assetBalances() public {
        ...
        IMorigamiLendingBorrower.AssetBalance[]
            memory assetBalances = managerImpl.latestAssetBalances();
