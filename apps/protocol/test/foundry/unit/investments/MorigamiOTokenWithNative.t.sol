...
import {MorigamiMockManagerWithNative} from "test/foundry/mocks/investments/MorigamiMockManagerWithNative.m.sol";
import {MorigamiOTokenWithNative} from "contracts/investments/MorigamiOTokenWithNative.sol";
...
contract MorigamiOTokenWithNativeTestBase is MorigamiTest {
    MorigamiOTokenWithNative public oToken;
    DummyWrappedNative public weth;
    MorigamiMockManagerWithNative public oTokenManager;
    ...
    function setUp() public {
        ...
        oToken = new MorigamiOTokenWithNative(
            origamiMultisig,
            address(weth),
            "O Token",
            "oToken"
        );
        oTokenManager = new MorigamiMockManagerWithNative(
            origamiMultisig,
            address(oToken),
            address(weth),
            feeCollector,
...
contract MorigamiOTokenWithNativeTestAccess is
    MorigamiOTokenWithNativeTestBase
{
...
contract MorigamiOTokenWithNativeTestAdmin is MorigamiOTokenWithNativeTestBase {
    ...
    function test_sendEth_failure() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MorigamiOTokenWithNative.InvalidSender.selector,
                msg.sender
            )
        );
    ...
    function test_setManager_success() public {
        ...
        vm.startPrank(origamiMultisig);
        MorigamiMockManagerWithNative newManager = new MorigamiMockManagerWithNative(
                origamiMultisig,
                address(oToken),
                address(weth),
                feeCollector,
...
contract MorigamiOTokenWithNativeTestInvest is
    MorigamiOTokenWithNativeTestBase
{
    event Invested(
    ...
    function test_investWithNative_failureMismatchedAmount() public {
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = oToken
            .investQuote(1e18, ZERO_ADDRESS, 100, 0);
        vm.expectRevert(
            abi.encodeWithSelector(
...
contract MorigamiOTokenWithNativeTestExit is MorigamiOTokenWithNativeTestBase {
    event Exited(
        address indexed user,
        uint256 investmentAmount,
    ...
    function test_exitToNative_failureNotEth() public {
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = oToken
            .exitQuote(1e18, ZERO_ADDRESS, 100, 0);
