...
contract MorigamiLendingRewardsMinterTestBase is MorigamiTest {
    DummyMintableToken public oToken;
    MorigamiInvestmentVault public ovToken;
    MorigamiDebtToken public debtToken;
    ...
    function _setUp() public {
        ...
        oToken = new DummyMintableToken(
            origamiMultisig,
            "oToken",
            "oToken",
            18
        );
        ovToken = new MorigamiInvestmentVault(
            origamiMultisig,
            "ovToken",
            "ovToken",
            address(oToken),
        ...
        debtToken = new MorigamiDebtToken(
            "Morigami iToken",
            "iToken",
            origamiMultisig
        );
...
contract MorigamiLendingRewardsMinterTestAdmin is
    MorigamiLendingRewardsMinterTestBase
{
...
contract MorigamiLendingRewardsMinterTestAccess is
    MorigamiLendingRewardsMinterTestBase
{
    function setUp() public {
        _setUp();
...
contract MorigamiLendingRewardsMinterTestMint is
    MorigamiLendingRewardsMinterTestBase
{
    ...
    function test_checkpointDebtAndMintRewards_manualCheckpoint() public {
        ...
        uint256 expectedMinted = expectedInterest.subtractBps(
            carryOverBps,
            MorigamiMath.Rounding.ROUND_DOWN
        );
        uint256 expectedReserves = expectedMinted.subtractBps(
            performanceFeeBps,
            MorigamiMath.Rounding.ROUND_DOWN
        );
    ...
    function test_checkpointDebtAndMintRewards_100pctPerfFees() public {
        ovToken.setPerformanceFee(MorigamiMath.BASIS_POINTS_DIVISOR);
    ...
    function test_checkpointDebtAndMintRewards_100pctCarryOver() public {
        rewardsMinter.setCarryOverRate(MorigamiMath.BASIS_POINTS_DIVISOR);
    ...
    ) internal view returns (Expected memory) {
        uint256 expectedMinted = expectedInterest.subtractBps(
            carryOverBps,
            MorigamiMath.Rounding.ROUND_DOWN
        );
    ...
    function test_checkpointDebtAndMintRewards_successiveMints() public {
        ...
        assertEq(
            rewardsMinter.cumulativeInterestCheckpoint(),
            expected1.minted
        );
        ...
        Expected memory expected2 = makeExpected(
            expected1.interest - expected1.minted
        );
        ...
        assertEq(
            rewardsMinter.cumulativeInterestCheckpoint(),
            expected1.minted + expected2.minted
        );
        assertEq(
            oToken.balanceOf(feeCollector),
            expected1.fees + expected2.fees
        );
        assertEq(
            oToken.balanceOf(address(ovToken)),
            expected1.reserves + expected2.reserves
        );

        // A change after another year
        Expected memory expected3 = makeExpected(
            (TWO_PCT_730DAY - TWO_PCT_365DAY) +
                (expected2.interest - expected2.minted) -
                1
        );
        ...
        assertEq(
            oToken.totalSupply(),
            expected1.minted + expected2.minted + expected3.minted
        );
        assertEq(
            rewardsMinter.cumulativeInterestCheckpoint(),
            expected1.minted + expected2.minted + expected3.minted
        ...
        assertEq(
            oToken.balanceOf(feeCollector),
            expected1.fees + expected2.fees + expected3.fees
        );
        assertEq(
            oToken.balanceOf(address(ovToken)),
            expected1.reserves + expected2.reserves + expected3.reserves
        );
    }
    ...
    function test_checkpointDebtAndMintRewards_extraBalance() public {
        ...
        assertEq(
            oToken.totalSupply(),
            expected1.minted + expected2.minted + extraTokens
        );
        assertEq(
            rewardsMinter.cumulativeInterestCheckpoint(),
            expected1.minted + expected2.minted
        ...
        assertEq(
            oToken.balanceOf(address(ovToken)),
            expected1.reserves + expected2.reserves + extraTokens
        );
        ...
        assertEq(
            oToken.totalSupply(),
            expected1.minted + expected2.minted + extraTokens + expected3.minted
        );
        assertEq(
            rewardsMinter.cumulativeInterestCheckpoint(),
            expected1.minted + expected2.minted + expected3.minted
