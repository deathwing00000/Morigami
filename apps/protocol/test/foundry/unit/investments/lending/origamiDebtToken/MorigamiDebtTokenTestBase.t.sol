...
import {IMorigamiDebtToken, MorigamiDebtToken} from "contracts/investments/lending/MorigamiDebtToken.sol";
import {CommonEventsAndErrors} from "contracts/libraries/CommonEventsAndErrors.sol";
import {console2} from "forge-std/Test.sol";

/* solhint-disable func-name-mixedcase, contract-name-camelcase, not-rely-on-time */
contract MorigamiDebtTokenTestBase is MorigamiTest {
    ...
    function dumpBase() internal view {
        ...
        IMorigamiDebtToken.DebtOwed memory debtOwed = iUSDC.currentTotalDebt();
    ...
    function checkTotals(
        ...
        assertEq(
            debtOwed.interest,
            expectedEstimatedInterest,
            "totalEstimatedInterest"
        );

        assertEq(
            iUSDC.totalPrincipal(),
            debtOwed.principal,
            "totalPrincipal alt"
        );
        assertEq(
            iUSDC.totalSupply(),
            expectedTotalPrincipal + expectedEstimatedInterest,
            "totalSupply"
        );
        assertEq(
            iUSDC.estimatedTotalInterest(),
            expectedEstimatedInterest,
            "estimatedTotalInterest"
        );
        assertEq(
            iUSDC.repaidTotalInterest(),
            expectedRepaidTotalInterest,
            "expectedRepaidTotalInterest"
        );
        assertEq(
            iUSDC.estimatedCumulativeInterest(),
            expectedEstimatedInterest + expectedRepaidTotalInterest,
            "estimatedCumulativeInterest"
        );
    }
    ...
    function currentDebtsOf(
        ...
        view
        returns (
            IMorigamiDebtToken.DebtOwed memory,
            IMorigamiDebtToken.DebtOwed memory
        )
    {
        address[] memory debtors = new address[](2);
        (debtors[0], debtors[1]) = (debtor1, debtor2);
        IMorigamiDebtToken.DebtOwed[] memory debts = iUSDC.currentDebtsOf(
            debtors
        );
...
contract MorigamiDebtTokenTestAdmin is MorigamiDebtTokenTestBase {
    ...
    function test_initalization() public {
        ...
        assertEq(iUSDC.name(), "Morigami iUSDC Debt");
...
contract MorigamiDebtTokenTestAccess is MorigamiDebtTokenTestBase {
    function setUp() public {
        _setUp();
    }
...
contract MorigamiDebtTokenTestZeroInterest is MorigamiDebtTokenTestBase {
