...
import {MorigamiDebtTokenTestBase} from "./MorigamiDebtTokenTestBase.t.sol";
...
contract MorigamiDebtTokenTest is MorigamiDebtTokenTestBase {
    ...
    function test_mint_aliceAndBob_inSameBlock() public {
        ...
        checkTotals(
            2 * amount,
            aliceExpectedInterestOnly + bobExpectedInterestOnly,
            0
        );
        checkDebtor(
            alice,
    ...
    function test_mint_aliceAndBob_inDifferentBlock() public {
        ...
        checkTotals(
            2 * amount,
            aliceExpectedDebt + bobExpectedDebt - 2 * amount,
            0
        );
        checkDebtor(
            alice,
    ...
    function test_transferFrom_interestRepayOnly() public {
        ...
        emit DebtorBalance(
            alice,
            uint128(amount),
            uint128(remainingAmount - amount)
        );
        vm.expectEmit(address(iUSDC));
        ...
        checkTotals(
            ...
            expectedBal - amount - transferAmount,
            transferAmount
        );
        checkDebtor(
            alice,
    ...
    function test_checkpointDebtorsInterest_success() public {
        ...
        checkTotals(
            amount + interestAliceRepaid + repayAmount,
            bobBal - amount,
            aliceBal - amount
        );
        checkDebtor(
            alice,
            aliceInterestRate,
            interestAliceRepaid + repayAmount,
            0,
            block.timestamp,
            aliceBal - repayAmount
        );
        checkDebtor(
            bob,
            bobInterestRate,
    ...
    function test_currentDebtOf() public {
        ...
        (
            IMorigamiDebtToken.DebtOwed memory aliceDebt,
            IMorigamiDebtToken.DebtOwed memory bobDebt
        ) = currentDebtsOf(alice, bob);
        assertEq(aliceDebt.principal, amount);
        assertEq(aliceDebt.interest, TEN_PCT_365DAY_1 - amount);
        assertEq(
            iUSDC.balanceOf(alice),
            aliceDebt.principal + aliceDebt.interest
        );
    ...
    function test_getDebtorPosition() public {
        ...
        IMorigamiDebtToken.DebtorPosition memory cache = iUSDC
            .getDebtorPosition(alice);
    ...
    function mkList(
        address addr1,
        address addr2
    ) private pure returns (address[] memory list) {
        list = new address[](2);
        list[0] = addr1;
        list[1] = addr2;
    }
    ...
    function test_totalSupplyExcluding() public {
        ...
        emit Checkpoint(
            alice,
            uint128(amount),
            uint128(TEN_PCT_365DAY_1 - amount)
        );
        vm.expectEmit(address(iUSDC));
        emit Checkpoint(
            bob,
            uint128(amount),
            uint128(FIVE_PCT_729DAY - amount)
        );
