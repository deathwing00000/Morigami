...
contract MorigamiLendingSupplyManagerTestBase is MorigamiTest {
...
contract MorigamiLendingSupplyManagerTestAdmin is
    MorigamiLendingSupplyManagerTestBase
{
    ...
    function test_initialization() public {
        ...
        assertEq(
            address(supplyManager.circuitBreakerProxy()),
            address(cbProxy)
        );
    ...
    function test_setLendingClerk_success() public {
        ...
        assertEq(
            usdcToken.allowance(address(supplyManager), address(lendingClerk)),
            type(uint256).max
        );
        ...
        assertEq(
            usdcToken.allowance(address(supplyManager), bob),
            type(uint256).max
        );
    }
    ...
    function test_areInvestmentsPaused() public {
        ...
        supplyManager.setPaused(IMorigamiManagerPausable.Paused(true, false));
    ...
    function test_areExitsPaused() public {
        ...
        vm.startPrank(origamiMultisig);
        supplyManager.setPauser(origamiMultisig, true);
        supplyManager.setPaused(IMorigamiManagerPausable.Paused(false, true));
        assertEq(supplyManager.areExitsPaused(), true);
    }
}

contract MorigamiLendingSupplyManagerTestAccess is
    MorigamiLendingSupplyManagerTestBase
{
    ...
    function test_access_investWithToken() public {
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = supplyManager
            .investQuote(100e6, address(usdcToken), 0, 0);
    ...
    function test_access_exitToToken() public {
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = supplyManager
            .exitQuote(100e6, address(usdcToken), 0, 0);
...
contract MorigamiLendingSupplyManagerTestInvestExit is
    MorigamiLendingSupplyManagerTestBase
{
    function test_maxInvest() public {
        assertEq(supplyManager.maxInvest(bob), 0);
        assertEq(
            supplyManager.maxInvest(address(usdcToken)),
            type(uint256).max
        );
    }
    ...
    function test_maxExit_underCircuitBreaker() public {
        ...
        assertEq(
            supplyManager.maxExit(address(usdcToken)),
            100.100100100100100100e18
        );
    }
    ...
    function test_maxExit_overCircuitBreaker() public {
        ...
        assertEq(
            supplyManager.maxExit(address(usdcToken)),
            2_002_002.002002002002002002e18
        );
    }
    ...
    function test_investQuote_success() public {
        ...
        ) = supplyManager.investQuote(100e6, address(usdcToken), 123, 100);
    ...
    function test_investQuote_fail() public {
        ...
        supplyManager.investQuote(100e6, bob, 0, 0);
        ...
        supplyManager.investQuote(0, address(usdcToken), 0, 0);
    }
    ...
    function test_exitQuote_success() public {
        ...
        ) = supplyManager.exitQuote(100e18, address(usdcToken), 123, 100);
    ...
    function test_investWithToken_success_eoa() public {
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = supplyManager
            .investQuote(amountIn, address(usdcToken), 0, 0);

        vm.startPrank(address(oUsdc));
        deal(address(usdcToken), address(supplyManager), amountIn);
        uint256 investmentAmount = supplyManager.investWithToken(
            alice,
            quoteData
        );
    ...
    function test_exitToToken_fail_paused() public {
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = supplyManager
            .exitQuote(100e18, address(usdcToken), 0, 0);
    ...
    function test_exitToToken_fail_zeroRecipient() public {
        ...
        (IMorigamiInvestment.ExitQuoteData memory quoteData, ) = supplyManager
            .exitQuote(amountOut, address(usdcToken), 0, 0);
    ...
    function _invest(
        ...
        (IMorigamiInvestment.InvestQuoteData memory quoteData, ) = supplyManager
            .investQuote(amount, address(usdcToken), 0, 0);
    ...
    function test_exitToToken_fail_circuitBreaker() public {
        ...
        // max out the circuit breaker
        {
            ...
            ) = supplyManager.exitQuote(amount, address(usdcToken), 0, 0);
        ...
        // trying for 1 more wei fails
        {
            ...
            ) = supplyManager.exitQuote(1, address(usdcToken), 0, 0);

            vm.expectRevert(
                abi.encodeWithSelector(
                    MorigamiCircuitBreakerAllUsersPerPeriod
                        .CapBreached
                        .selector,
                    amount + 1,
                    amount
    ...
    function test_exitToToken_success_eoa() public {
        ...
        deal(address(oUsdc), address(supplyManager), amountOut);
        (uint256 toTokenAmount, uint256 toBurnAmount) = supplyManager
            .exitToToken(alice, quoteData, bob);
    ...
    function test_exitToToken_success_contract() public {
        ...
        deal(address(oUsdc), address(supplyManager), amountOut);
        (uint256 toTokenAmount, uint256 toBurnAmount) = supplyManager
            .exitToToken(address(lendingClerk), quoteData, bob);
        assertEq(toTokenAmount, expectedToToken);
        assertEq(toBurnAmount, expectedToBurn);
        assertEq(usdcToken.balanceOf(address(supplyManager)), 0);
        assertEq(usdcToken.balanceOf(address(lendingClerk)), expectedFee);
    ...
    function test_exitToToken_success_noFees() public {
        ...
        assertEq(usdcToken.balanceOf(address(supplyManager)), 0);
        assertEq(
            usdcToken.balanceOf(address(lendingClerk)),
            amountIn - expectedToToken
        );
        assertEq(usdcToken.balanceOf(bob), expectedToToken);
        assertEq(usdcToken.balanceOf(alice), 0);
        assertEq(
