...
contract MorigamiLendingClerkTestAdmin is MorigamiLendingClerkTestBase {
    ...
    function test_initialization() public {
        ...
        assertEq(
            address(lendingClerk.idleStrategyManager()),
        ...
        assertEq(
            address(lendingClerk.globalInterestRateModel()),
    ...
    function test_construction_fail() public {
        DummyMintableToken token24 = new DummyMintableToken(
            origamiMultisig,
            "Deposit Token",
            "token",
            24
    ...
    function test_setBorrowerPaused_success() public {
        ...
        IMorigamiLendingClerk.BorrowerDetails memory details = lendingClerk
            .borrowerDetails(address(borrower));
    ...
    function test_addBorrower_fail_alreadyExisting() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiLendingClerk.AlreadyEnabled.selector
            )
        );
        lendingClerk.addBorrower(
            address(borrower),
    ...
    function test_addBorrower_success() public {
        ...
        emit DebtCeilingUpdated(address(borrower), 0, 100e6);
        lendingClerk.addBorrower(
            address(borrower),
            address(borrowerInterestRateModel),
            100e6
    ...
    function test_setBorrowerInterestRateModel_fail_badModel() public {
        ...
        lendingClerk.setBorrowerInterestRateModel(
            address(borrower),
            address(0)
        );
    }
    ...
    function test_setIdleStrategyInterestRate() public {
        ...
        IMorigamiDebtToken.DebtorPosition memory debtPosition = iUsdc
            .getDebtorPosition(address(idleStrategyManager));
        assertEq(debtPosition.principal, 0);
        assertEq(debtPosition.interest, 0);
        assertEq(debtPosition.interestDelta, 0);
    ...
    function test_refreshBorrowersInterestRate_success_global() public {
        ...
        MockBorrower borrower2 = new MockBorrower(
            address(usdcToken),
        ...
        emit Checkpoint(
            address(idleStrategyManager),
            10_000e18 - 1_000e18 - 500e18,
            35.003382344037053e18
        );
        vm.expectEmit(address(iUsdc));
...
contract MorigamiLendingClerkTestAccess is MorigamiLendingClerkTestBase {
    ...
    function test_access_setGlobalInterestRateModel() public {
        expectElevatedAccess();
        lendingClerk.setGlobalInterestRateModel(
            address(globalInterestRateModel)
        );
    }
