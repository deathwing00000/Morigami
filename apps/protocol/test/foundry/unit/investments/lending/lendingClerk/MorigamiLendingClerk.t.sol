...
import {MorigamiLendingClerkTestBase, MockBorrower} from "./MorigamiLendingClerkBase.t.sol";
...
contract MorigamiLendingClerkTestDepositWithdraw is
    MorigamiLendingClerkTestBase
{
    ...
    function test_withdraw_fail_aboveMaxUtilisation_withCheckpoint() public {
        ...
        assertEq(
            usdcToken.balanceOf(address(idleStrategyManager)),
            1_000e6 - 250e6
        );
...

        {
            ...
            assertEq(
                idleStrategyManager.availableToWithdraw(),
                expectedAvailable
            );
            assertEq(iUsdc.totalSupply(), 1_000e18);
            assertEq(
                iUsdc.balanceOf(address(idleStrategyManager)),
                750e18 + expectedIdleStrategyInterest
            );
            assertEq(
                lendingClerk.totalBorrowerDebt(),
                1_000e18 - _scaleUp(expectedAvailable)
            );
            assertEq(
                lendingClerk.totalAvailableToWithdraw(),
                expectedAvailable
            );
        }
        ...
        uint256 expectedAvailable2 = 747.360593e6;
        {
            ...
            assertEq(
                ...
                0.252639406412895889e18
            ); // 252.64/1000 = 25.26% UR
            assertEq(idleStrategyManager.availableToWithdraw(), 750e6);
            assertEq(
                iUsdc.totalSupply(),
                750e18 + expectedIdleStrategyInterest + expectedBorrowerDebt
            );
            assertEq(
                iUsdc.balanceOf(address(idleStrategyManager)),
                750e18 + expectedIdleStrategyInterest
...

        {
            ...
            assertEq(
                iUsdc.totalSupply(),
                750e18 +
                    expectedIdleStrategyInterest +
                    expectedBorrowerDebt -
                    _scaleUp(expectedAvailable2)
            );
            assertEq(
                iUsdc.balanceOf(address(idleStrategyManager)),
                750e18 +
        ...
        {
            assertEq(
                oUsdc.circulatingSupply(),
                1_000e18 - _scaleUp(expectedAvailable2)
            );
            assertEq(
                lendingClerk.globalUtilisationRatio(),
                0.999999997676118235e18
            ); // Just under 100% UR
    ...
    function test_withdraw_fail_aboveMaxUtilisation_noCheckpoint() public {
...

        {
            ...
            assertEq(
                lendingClerk.totalBorrowerDebt(),
                1_000e18 - _scaleUp(expectedAvailable)
            ); // borrower debt still not checkpoint
            assertEq(lendingClerk.totalAvailableToWithdraw(), 0);
        }
...

        {
            ...
            assertEq(
                iUsdc.totalSupply(),
                expectedBorrowerDebt + expectedIdleStrategyInterest
            );
            assertEq(
                iUsdc.balanceOf(address(idleStrategyManager)),
                expectedIdleStrategyInterest
...
contract MorigamiLendingClerkTestBorrow is MorigamiLendingClerkTestBase {
    event Borrow(
    ...
    function test_borrow_fail_zeroCapacity() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiLendingClerk.AboveMaxUtilisation.selector,
                type(uint256).max
            )
        );
        lendingClerk.borrow(100e6, alice);
    ...
    function test_availableToBorrow() public {
        ...
        assertEq(
            lendingClerk.availableToBorrow(address(borrower)),
            1_000_000e6
        );
    ...
    function test_borrow_fail_notEnoughGlobalCapacity() public {
        ...
        // Move forward in time to accrue iUSDC
        {
            assertEq(
                iUsdc.balanceOf(address(idleStrategyManager)),
                _scaleUp(globalCapacity - borrowAmount)
            );
            assertEq(
                iUsdc.balanceOf(address(borrower)),
                _scaleUp(borrowAmount)
            );
            ...
            _scaleAndAssert(
                iUsdc.balanceOf(address(idleStrategyManager)),
                globalCapacity - borrowAmount + expectedIdleInterest
            );
            _scaleAndAssert(
                iUsdc.balanceOf(address(borrower)),
        ...
        // include the accrued interest yet, just the 80e6 borrowed
        {
            ...
            assertEq(
                lendingClerk.borrowerUtilisationRatio(address(borrower)),
                0.088807497622378286e18
            ); // 88.88 / 1000
            assertEq(lendingClerk.globalUtilisationRatio(), 0.8e18); // 80 / 100
            assertEq(
                lendingClerk.calculateCombinedInterestRate(address(borrower)),
                0.104933749867909905e18
            );
            assertEq(
                lendingClerk.calculateBorrowerInterestRate(address(borrower)),
                0.104933749867909905e18
            );
            assertEq(
                lendingClerk.calculateGlobalInterestRate(),
                0.094444444444444445e18
            );
        }
        ...
        // principal which is transferred from idle strategy => borrower
        {
            ...
            uint256 expectedTotalBorrowerDebt = _scaleUp(
                borrowAmount + expectedBorrowerInterest + newBorrowAmount
            );
            assertEq(expectedTotalBorrowerDebt, 108.807497e18);

            uint256 expectedGlobalUr = MorigamiMath.mulDiv(
        ...
        // For the borrower and also the idle strategy
        {
            ...
            _scaleAndAssert(
                iUsdc.totalSupply(),
                globalCapacity + expectedIdleInterest + expectedBorrowerInterest
            );
        }
        ...
        uint256 maxBorrowable = globalCapacity -
            borrowAmount -
        ...
        assertEq(maxBorrowable, 11.192502e6);
        {
            ...
            _scaleAndAssert(
                lendingClerk.borrowerUtilisationRatio(address(borrower)),
                ((borrowAmount + expectedBorrowerInterest) * 1e6) / 1_000e6
            );

            // 20e6 USDC still left in the idle strategy manager
            assertEq(
                idleStrategyManager.availableToWithdraw(),
                globalCapacity - borrowAmount
            );
            assertEq(
                lendingClerk.availableToBorrow(address(borrower)),
                globalCapacity - borrowAmount - expectedBorrowerInterest - 1
            );
...

        {
            ...
            assertEq(
                lendingClerk.globalUtilisationRatio(),
                0.999999996223782856e18
            );
            assertEq(
                iUsdc.balanceOf(address(borrower)),
                99.999999622378285600e18
            );
            assertEq(
                usdcToken.balanceOf(address(alice)),
                borrowAmount + maxBorrowable
            );
            assertEq(
                lendingClerk.borrowerUtilisationRatio(address(borrower)),
                0.099999999622378286e18
            ); // 100e6 * 1e18 / 1_000e6

            assertEq(
                idleStrategyManager.availableToWithdraw(),
                globalCapacity - borrowAmount - maxBorrowable
            );
            assertEq(lendingClerk.availableToBorrow(address(borrower)), 0);

            _scaleAndAssert(
                9.832919927520480780e18,
                globalCapacity -
                    borrowAmount -
                    maxBorrowable +
                    expectedIdleInterest
            );
            assertEq(
                iUsdc.balanceOf(address(idleStrategyManager)),
                9.832919927520480780e18
            ...
            assertEq(
                lendingClerk.calculateCombinedInterestRate(address(borrower)),
                0.199999996223782856e18
            );
            assertEq(
                lendingClerk.calculateBorrowerInterestRate(address(borrower)),
                0.105555555534576572e18
            );
            assertEq(
                lendingClerk.calculateGlobalInterestRate(),
                0.199999996223782856e18
    ...
    function test_borrow_fail_zeroAmount() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CommonEventsAndErrors.ExpectedNonZero.selector
            )
        );
    ...
    function test_borrow_fail_globalBorrowPaused() public {
        ...
        vm.startPrank(address(borrower));
        vm.expectRevert(
            abi.encodeWithSelector(IMorigamiLendingClerk.BorrowPaused.selector)
        );
        lendingClerk.borrow(100e6, alice);
    }
    ...
    function test_borrow_success() public {
        ...
        IMorigamiDebtToken.DebtorPosition memory debtPosition = iUsdc
            .getDebtorPosition(address(borrower));
    ...
    function test_borrowMax_hitCircuitBreaker() public {
        ...
        uint256 _availableToBorrow = lendingClerk.availableToBorrow(
            address(borrower)
        );
...
contract MorigamiLendingClerkTestRepay is MorigamiLendingClerkTestBase {
    ...
    function test_repay_fail_globalRepayPaused() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IMorigamiLendingClerk.RepayPaused.selector)
        );
        lendingClerk.repay(100e6, address(borrower));
    }
    ...
    function test_repay_success_withDebt() public {
        ...
        assertEq(
            lendingClerk.globalUtilisationRatio(),
            0.081055762565158356e18
        );
        // roughly (500-100) / 1000 = 40%
        assertEq(
            lendingClerk.borrowerUtilisationRatio(address(borrower)),
            0.405278812825791778e18
        );
        ...
        assertEq(
            lendingClerk.calculateGlobalInterestRate(),
            0.054503097920286576e18
        );

        assertEq(iUsdc.balanceOf(address(borrower)), 405.278812825791777500e18);
        assertEq(
            lendingClerk.borrowerDebt(address(borrower)),
            405.278812825791777500e18
        );
        assertEq(lendingClerk.borrowerDebt(address(bob)), 0); // unknown borrower
        assertEq(
            lendingClerk.borrowerDebt(address(idleStrategyManager)),
            4_618.531202417431381000e18
        );
        assertEq(iUsdc.balanceOf(alice), 0);
        assertEq(iUsdc.balanceOf(address(lendingClerk)), 0);
        assertEq(
    ...
    function test_repay_success_onBehalfOf() public {
        ...
        setExplicitAccess(
            lendingClerk,
            bob,
            MorigamiLendingClerk.repay.selector,
            true
        );
    ...
    function test_repay_success_capped() public {
        ...
        assertEq(
            iUsdc.balanceOf(address(borrower)),
            500e18 + expectedBorrowerInterest
        );
...

        {
            ...
            assertEq(usdcToken.balanceOf(alice), 500e6);
            assertEq(
                usdcToken.balanceOf(bob),
                1_000e6 - 500e6 - expectedRepayInterest
            );
            assertEq(usdcToken.balanceOf(address(lendingClerk)), 0);
            assertEq(
                usdcToken.balanceOf(address(idleStrategyManager)),
                5_000e6 + expectedRepayInterest
            );
...
contract MorigamiLendingClerkTestViews is MorigamiLendingClerkTestBase {
    function test_globalDebtCeiling_edgeConditions() public {
        ...
        assertEq(
            lendingClerk.globalUtilisationRatio(),
            (_scaleUp(1) * 1e18) / 5_000e18
        );
        assertEq(
            lendingClerk.borrowerUtilisationRatio(address(borrower)),
            (_scaleUp(1) * 1e18) / 1_000e18
        );
    ...
    function test_borrowerBalanceSheet_knownBorrower() public {
        ...
        (assetBalances, debtTokenBalance) = lendingClerk.borrowerBalanceSheet(
            address(borrower)
        );
...
contract MorigamiLendingClerkTest18dpAsset is MorigamiLendingClerkTestBase {
    function test_scale18dpAsset() public {
        DummyMintableToken daiToken = new DummyMintableToken(
            origamiMultisig,
            "Deposit Token",
            "token",
            18
        );
        MorigamiOToken oDai = new MorigamiOToken(
            origamiMultisig,
            "Morigami DAI Token",
            "oDAI"
        );
        MorigamiIdleStrategyManager daiIdleStrategyManager = new MorigamiIdleStrategyManager(
                origamiMultisig,
                address(daiToken)
            );
        MorigamiDebtToken iDai = new MorigamiDebtToken(
            "Morigami iDAI",
            "iDAI",
            origamiMultisig
        );
        MorigamiCircuitBreakerAllUsersPerPeriod cbDaiBorrow = new MorigamiCircuitBreakerAllUsersPerPeriod(
                origamiMultisig,
                address(cbProxy),
                26 hours,
                13,
        ...
        MorigamiLendingClerk daiLendingClerk = new MorigamiLendingClerk(
            origamiMultisig,
            address(daiToken),
            address(oDai),
        ...
        MockBorrower daiBorrower = new MockBorrower(
            address(daiToken),
        ...
        uint256 expectedNewIr = 0.11e18;
        {
            ...
            assertEq(
                daiLendingClerk.globalUtilisationRatio(),
                0.081081219276487689e18
            );
            // roughly 10k / 55.5k
            assertEq(
                daiLendingClerk.borrowerUtilisationRatio(address(daiBorrower)),
            ...
            assertEq(
                daiToken.balanceOf(address(daiIdleStrategyManager)),
                supplyAmount - borrowAmount
            );
            assertEq(
                daiIdleStrategyManager.availableToWithdraw(),
                supplyAmount - borrowAmount
            );
            assertEq(
                daiLendingClerk.availableToBorrow(address(daiBorrower)),
                borrowerCeiling - borrowAmount
            );
            assertEq(
                daiLendingClerk.totalAvailableToWithdraw(),
                supplyAmount - borrowAmount
            );

            assertEq(
                daiLendingClerk.calculateCombinedInterestRate(
                    address(daiBorrower)
                ),
                expectedNewIr
            );
            assertEq(
                daiLendingClerk.calculateBorrowerInterestRate(
                    address(daiBorrower)
                ),
                expectedNewIr
            );
            assertEq(
                daiLendingClerk.calculateGlobalInterestRate(),
                0.054504512182027094e18
            );

            assertEq(iDai.balanceOf(address(daiBorrower)), borrowAmount);
            assertEq(
                daiLendingClerk.borrowerDebt(address(daiBorrower)),
                borrowAmount
            );
            assertEq(
                daiLendingClerk.borrowerDebt(address(daiIdleStrategyManager)),
                113_333.123123123123123127e18
            );
            assertEq(
                iDai.balanceOf(address(daiIdleStrategyManager)),
                supplyAmount - borrowAmount
            );

            IMorigamiDebtToken.DebtorPosition memory debtPosition = iDai
                .getDebtorPosition(address(daiBorrower));
        ...
        uint256 expectedInterest = 90.820900487624409999e18;
        {
            assertEq(
                daiLendingClerk.globalUtilisationRatio(),
                0.073709484283571434e18
            );
            assertEq(
                daiLendingClerk.borrowerUtilisationRatio(address(daiBorrower)),
                0.163634776208777240e18
            );
            assertEq(
                daiToken.balanceOf(address(daiBorrower)),
            ...
            assertEq(
                daiLendingClerk.availableToBorrow(address(daiBorrower)),
                borrowerCeiling - borrowAmount - expectedInterest + repayAmount
            );
            assertEq(
                daiLendingClerk.totalAvailableToWithdraw(),
                supplyAmount - borrowAmount - expectedInterest + repayAmount
            );
            ...
            assertEq(
                daiLendingClerk.calculateGlobalInterestRate(),
                0.054094971349087302e18
            );

            assertEq(
                iDai.balanceOf(address(daiBorrower)),
                borrowAmount + expectedInterest - repayAmount
            );
            assertEq(
                daiLendingClerk.borrowerDebt(address(daiBorrower)),
                borrowAmount + expectedInterest - repayAmount
