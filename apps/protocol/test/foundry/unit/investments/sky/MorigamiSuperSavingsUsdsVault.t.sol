...
contract MorigamiSuperSavingsUsdsVaultTestBase is MorigamiTest {
    ...
    function addToSharePrice(uint256 amount) internal {
        deal(
            address(asset),
            address(manager),
            asset.balanceOf(address(manager)) + amount
        );
    }
}

contract MorigamiSuperSavingsUsdsVaultTestAdmin is
    MorigamiSuperSavingsUsdsVaultTestBase
{
    ...
    function test_initialization() public {
        ...
        assertEq(vault.name(), "Morigami sUSDS+s");
...
contract MorigamiSuperSavingsUsdsVaultTestAccess is
    MorigamiSuperSavingsUsdsVaultTestBase
{
...
contract MorigamiSuperSavingsUsdsVaultTestDeposit is
    MorigamiSuperSavingsUsdsVaultTestBase
{
    function test_deposit_basic() public {
        ...
        assertEq(
            ...
            BOOTSTRAPPED_USDS_AMOUNT + 123e18 + 0.1e18
        );
    ...
    function test_deposit_afterShareIncrease() public {
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + 233e18 + 0.1e18
        );
...
contract MorigamiSuperSavingsUsdsVaultTestMint is
    MorigamiSuperSavingsUsdsVaultTestBase
{
    function test_mint_basic() public {
        mint(alice, 123e18);

        uint256 expectedAssets = MorigamiMath.inverseSubtractBps(
            123e18,
            DEPOSIT_FEE,
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + expectedAssets + 0.1e18
        );
    ...
    function test_mint_beforeShareIncrease() public {
        ...
        uint256 expectedAssets = 100e18 +
            MorigamiMath.inverseSubtractBps(
                123e18,
                DEPOSIT_FEE,
                MorigamiMath.Rounding.ROUND_UP
            );
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + expectedAssets - 100e18 + 0.1e18
        );
        assertEq(vault.balanceOf(alice), 123e18);
        assertEq(vault.totalSupply(), 123e18 + 0.1e18);
    ...
    function test_mint_afterShareIncrease() public {
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + 245.387712287712287713e18
        );
        assertEq(vault.balanceOf(alice), 123e18);
...
contract MorigamiSuperSavingsUsdsVaultTestWithdraw is
    MorigamiSuperSavingsUsdsVaultTestBase
{
    function test_withdraw_basic() public {
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + 73e18 + 0.1e18
        );
        assertEq(vault.balanceOf(alice), expectedShares);
    ...
    function test_withdraw_afterShareIncrease() public {
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + 50e18 + 0.1e18
        );
...
contract MorigamiSuperSavingsUsdsVaultTestRedeem is
    MorigamiSuperSavingsUsdsVaultTestBase
{
    ...
    function test_redeem_afterShareIncrease() public {
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + 45.104995004995004996e18
        );
        assertEq(vault.balanceOf(alice), expectedShares);
        assertEq(vault.totalSupply(), expectedShares + 0.1e18);
