...
import {MorigamiSuperSavingsUsdsVault} from "contracts/investments/sky/MorigamiSuperSavingsUsdsVault.sol";
import {MorigamiSuperSavingsUsdsManager} from "contracts/investments/sky/MorigamiSuperSavingsUsdsManager.sol";
...
contract MorigamiSuperSavingsUsdsManagerTestBase is MorigamiTest {
    using MorigamiMath for uint256;

    DummyMintableToken public asset;
    MockSUsdsToken public sUSDS;
    MorigamiSuperSavingsUsdsVault public vault;
    MorigamiSuperSavingsUsdsManager public manager;
    ...
    event InKindFees(
        IMorigamiErc4626.FeeType feeType,
        uint256 feeBps,
        uint256 feeAmount
    );
    event Deposit(
    ...
    function setUp() public {
        ...
        vault = new MorigamiSuperSavingsUsdsVault(
            origamiMultisig,
            "Morigami sUSDS+s",
            "sUSDS+s",
            asset,
        ...
        manager = new MorigamiSuperSavingsUsdsManager(
            origamiMultisig,
            address(vault),
            address(sUSDS),
        ...
        skyFarm1RewardsToken = new DummyMintableToken(
            origamiMultisig,
            "SKY",
            "SKY",
            18
        );
        skyFarm1 = new DummySkyStakingRewards(
        ...
        skyFarm2RewardsToken = new DummyMintableToken(
            origamiMultisig,
            "SUBDAO1",
            "SUBDAO1",
            18
        );
        skyFarm2 = new DummySkyStakingRewards(
    ...
    function allFarmDetails()
        internal
        view
        returns (IMorigamiSuperSavingsUsdsManager.FarmDetails[] memory details)
    {
...
contract MorigamiSuperSavingsUsdsManagerTestAdmin is
    MorigamiSuperSavingsUsdsManagerTestBase
{
    ...
    function test_bad_constructor() public {
        ...
        new MorigamiSuperSavingsUsdsManager(
            origamiMultisig,
            address(vault),
            address(sUSDS),
            SWITCH_FARM_COOLDOWN,
    ...
    function test_initialization() public {
        ...
        (uint16 forCaller, uint16 forMorigami) = manager.performanceFeeBps();
        assertEq(forCaller, 100);
        assertEq(forMorigami, 400);

        // Not setup by default
        IMorigamiSuperSavingsUsdsManager.Farm memory farm = manager.getFarm(0);
        ...
        assertEq(
            asset.allowance(address(manager), address(sUSDS)),
            type(uint256).max
        );
    }
    ...
    function test_setPerformanceFees_success() public {
        ...
        assertEq(forMorigami, 399);

        vm.expectEmit(address(vault));
        emit PerformanceFeeSet(301);
        manager.setPerformanceFees(101, 200);
        (forCaller, forMorigami) = manager.performanceFeeBps();
        assertEq(forCaller, 101);
        assertEq(forMorigami, 200);
    }
    ...
    function test_setPerformanceFees_withHarvest() public {
        ...
        assertEq(
            skyFarm1RewardsToken.balanceOf(feeCollector),
            14.285714285714285376e18 + 57.142857142857141504e18
        );
        assertEq(skyFarm1RewardsToken.balanceOf(origamiMultisig), 0);
        assertEq(
            skyFarm1RewardsToken.balanceOf(swapper),
            1357.142857142857110720e18
        );
    }
    ...
    function test_addFarm_failMaxFarms() public {
        ...
        for (uint256 i; i < 100; ++i) {
            DummySkyStakingRewards farm = new DummySkyStakingRewards(
                address(skyFarm1RewardsToken),
                address(asset)
            );
        ...
        // Last one will fail
        {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    IMorigamiSuperSavingsUsdsManager.MaxFarms.selector
                )
            );
    ...
    function test_addFarm_failNotUnique() public {
        ...
        for (uint256 i; i < 55; ++i) {
            farm = new DummySkyStakingRewards(
                address(skyFarm1RewardsToken),
                address(asset)
            );
            manager.addFarm(address(farm), uint16(i));
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiSuperSavingsUsdsManager.FarmExistsAlready.selector,
                address(farm)
            )
        );
        manager.addFarm(address(farm), 100);
    ...
    function test_addFarm_failZeroAddress() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiSuperSavingsUsdsManager.InvalidFarm.selector,
                1
            )
        );
    ...
    function test_addFarm_failWrongFarmToken() public {
        DummySkyStakingRewards badFarm = new DummySkyStakingRewards(
            address(skyFarm1RewardsToken),
            address(skyFarm2RewardsToken)
        );
    ...
    function test_addFarm_success() public {
        ...
        // Index 0 is empty
        {
            IMorigamiSuperSavingsUsdsManager.Farm memory farm = manager.getFarm(
                0
            );
            assertEq(address(farm.staking), address(0));
            assertEq(address(farm.rewardsToken), address(0));
        ...
        IMorigamiSuperSavingsUsdsManager.FarmDetails[]
            memory farmDetails = allFarmDetails();
        assertEq(farmDetails.length, 2);

        // Index 0 has the sUSDS
        {
            IMorigamiSuperSavingsUsdsManager.FarmDetails
                memory details = farmDetails[0];
        ...
        {
            IMorigamiSuperSavingsUsdsManager.FarmDetails
                memory details = farmDetails[1];

            assertEq(address(details.farm.staking), address(skyFarm1));
            assertEq(
                address(details.farm.rewardsToken),
                address(skyFarm1RewardsToken)
            );
    ...
    function test_removeFarm_failures() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiSuperSavingsUsdsManager.FarmStillInUse.selector,
                0
            )
        );
    ...
    function test_removeFarm_success() public {
        ...
        IMorigamiSuperSavingsUsdsManager.Farm memory farm = manager.getFarm(1);
        assertEq(address(farm.staking), address(skyFarm1));
        assertEq(address(farm.rewardsToken), address(skyFarm1RewardsToken));
...
contract MorigamiSuperSavingsUsdsManagerTestAccess is
    MorigamiSuperSavingsUsdsManagerTestBase
{
...
contract MorigamiSuperSavingsUsdsManagerTestDeposit is
    MorigamiSuperSavingsUsdsManagerTestBase
{
...
contract MorigamiSuperSavingsUsdsManagerTestWithdraw is
    MorigamiSuperSavingsUsdsManagerTestBase
{
    ...
    function test_withdraw_sUSDS_success() public {
        ...
        vm.startPrank(address(vault));
        assertEq(
            manager.withdraw(manager.totalAssets(), alice),
            100.013698630136986301e18
        );
...
contract MorigamiSuperSavingsUsdsManagerTestSwitch is
    MorigamiSuperSavingsUsdsManagerTestBase
{
    event SwitchedFarms(
    ...
    function test_switchFarms_failBeforeCooldown() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMorigamiSuperSavingsUsdsManager.BeforeCooldownEnd.selector
            )
        );
        manager.switchFarms(1);
    }
    ...
    function test_switchFarms_sUsdsToFarm_withBalance() public {
        ...
        assertEq(
            asset.allowance(address(manager), address(skyFarm1)),
            type(uint256).max
        );
        assertEq(asset.allowance(address(manager), address(skyFarm2)), 0);
    }
    ...
    function test_switchFarms_FarmToFarm_withBalance() public {
        ...
        assertEq(asset.allowance(address(manager), address(skyFarm1)), 0);
        assertEq(
            asset.allowance(address(manager), address(skyFarm2)),
            type(uint256).max
        );
    }
...
contract MorigamiSuperSavingsUsdsManagerTestRewards is
    MorigamiSuperSavingsUsdsManagerTestBase
{
    event ClaimedReward(
        uint32 indexed farmIndex,
    ...
    function test_claimFarmRewards_successWithBalance() public {
        ...
        assertEq(
            skyFarm1RewardsToken.balanceOf(alice),
            14.285714285714285376e18
        );
        assertEq(
            skyFarm1RewardsToken.balanceOf(feeCollector),
    ...
    function test_claimFarmRewards_successMultiple() public {
        ...
        assertEq(
            skyFarm2RewardsToken.balanceOf(alice),
            4.285714285714285440e18
        );
        assertEq(
            skyFarm2RewardsToken.balanceOf(feeCollector),
            17.142857142857141760e18
        );
        assertEq(
            skyFarm2RewardsToken.balanceOf(swapper),
            407.142857142857116800e18
        );
    }
...
contract MorigamiSuperSavingsUsdsManagerTestViews is
    MorigamiSuperSavingsUsdsManagerTestBase
{
    function test_supportsInterface() public {
        assertEq(
            manager.supportsInterface(
                type(IMorigamiSuperSavingsUsdsManager).interfaceId
            ),
            true
        );
        ...
        assertEq(
            manager.supportsInterface(
                type(IMorigamiManagerPausable).interfaceId
            ),
            false
        );
    }
    ...
    function test_totalAssets_multiple() public {
        ...
        assertEq(
            asset.balanceOf(address(sUSDS)),
            BOOTSTRAPPED_USDS_AMOUNT + 1_000e18 + 111e18
        );
        ...
        deal(
            address(asset),
            address(manager),
            asset.balanceOf(address(manager)) + 22.2e18,
            true
        );
        ...
        assertEq(
            manager.totalAssets(),
            111e18 + 22.2e18 + 0.015205479452054794e18
        );
        ...
        deal(
            address(asset),
            address(manager),
            asset.balanceOf(address(manager)) + 13e18,
            true
        );
        deal(
            address(sUSDS),
            address(manager),
            sUSDS.balanceOf(address(manager)) + 9e18,
            true
        );
        assertEq(
            manager.totalAssets(),
            111e18 +
                22.2e18 +
                0.015205479452054794e18 +
                13e18 +
                9e18 -
                0.079055622683519557e18
        );
    }
    ...
    function test_farmViewsPartial() public {
        ...
        farmIndexes[2] = 2;
        IMorigamiSuperSavingsUsdsManager.FarmDetails[]
            memory farmDetails = manager.farmDetails(farmIndexes);
        assertEq(farmDetails.length, 3);
