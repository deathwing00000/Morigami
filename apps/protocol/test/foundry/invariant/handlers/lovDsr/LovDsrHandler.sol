...
import {ExternalContracts, OUsdcContracts, LovTokenContracts} from "test/foundry/deploys/lovDsr/MorigamiLovTokenTestDeployer.t.sol";
...
contract LovDsrHandler is BaseHandler {
        ...
        returns (uint256 amountOut)
    {
        ...
        // The max is set so we don't trip borrow circuit breakers when borrower later
        {
            ...
            uint256 minReserves = lovTokenContracts
                .lovDsrManager
                .sharesToReserves(1, IMorigamiOracle.PriceType.SPOT_PRICE) + 2;
            uint256 minInvest = externalContracts.sDaiToken.previewMint(
                minReserves
            ) + 2;
            uint256 maxInvest = min(
                100_000e18,
                lovTokenContracts.lovDsrManager.maxInvest(
        ...
        assertGe(
            alRatioAfter,
            alRatioBefore,
            "Invariant violation: investLovDsr_dai A/L after invest should be higher"
        );
    }
        ...
        returns (uint256 amountOut)
    {
        uint256 maxAvailableToExit = min(
            ...
            lovTokenContracts.lovDsr.maxExit(
                address(externalContracts.daiToken)
            )
        );
    ...
    function rebalanceDown(
        uint256 targetAL,
        uint256 timeJumpSeed
    ) external instrument adjustTimestamp(timeJumpSeed) {
        ...
        (
            ...
            bool cappedBorrow
        ) = rebalanceDownParams(targetAL, alRatioBefore, 100);
        ...
        uint128 alRatioAfter = lovTokenContracts.lovDsrManager.rebalanceDown(
            params
        );
        vm.stopPrank();
        ...
        stateStore.setCappedRebalanceDown(
            cappedBorrow && alRatioAfter > Constants.REBALANCE_AL_CEILING
        );
    }
    ...
    function rebalanceUp(
        ...
        IMorigamiLovTokenErc4626Manager.RebalanceUpParams
            memory params = rebalanceUpParams(targetAL, alRatioBefore, 100);
        ...
        vm.startPrank(overlord);
        uint128 alRatioAfter = lovTokenContracts.lovDsrManager.rebalanceUp(
            params
        );
        vm.stopPrank();
        ...
        )
    {
        ...
        */
        ...
        managerLiabilities = lovTokenContracts.lovDsrManager.liabilities(
            IMorigamiOracle.PriceType.SPOT_PRICE
        );
        managerPrecision = 1e18;

        uint256 _netAssets = managerAssets -
            targetAL.mulDiv(
                managerLiabilities,
                managerPrecision,
        ...
        )
    {
        ...
        uint256 daiDepositAmount = externalContracts.sDaiToken.previewMint(
            reservesAmount
        );
        ...
        // and work backwards
        uint256 availableToBorrow = oUsdcContracts
            .lendingClerk
            .availableToBorrow(address(lovTokenContracts.lovDsrManager));
        if (availableToBorrow < params.borrowAmount) {
            ...
            reservesAmount = externalContracts.sDaiToken.previewDeposit(
                daiDepositAmount
            );
    ...
    function solveRebalanceUpAmount(
        ...
        */
        ...
        uint256 _liabilities = lovTokenContracts.lovDsrManager.liabilities(
            IMorigamiOracle.PriceType.SPOT_PRICE
        );
    ...
    function rebalanceUpParams(
        ...
        virtual
        returns (
            IMorigamiLovTokenErc4626Manager.RebalanceUpParams memory params
        )
    {
        // reserves (sDAI) amount
        params.minReserveAssetShares = solveRebalanceUpAmount(
            targetAL,
            currentAL
        );

        // How much DAI to sell for that reserves amount
        params.depositAssetsToWithdraw = externalContracts
            .sDaiToken
            .previewRedeem(params.minReserveAssetShares);
        ...
        params.minDebtAmountToRepay = params.depositAssetsToWithdraw.mulDiv(
            1e6,
            lovTokenContracts.daiUsdcOracle.latestPrice(
                IMorigamiOracle.PriceType.SPOT_PRICE,
                MorigamiMath.Rounding.ROUND_UP
            ),
            MorigamiMath.Rounding.ROUND_DOWN
