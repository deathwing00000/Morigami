...
contract OvUsdcHandler is BaseHandler {
        ...
        returns (uint256 amountOut)
    {
        ...
        assertEq(
            amountOut,
            amount * 1e12,
            "Invariant violation: investOvUsdc(usdc) unexpected amountOut"
        );
        assertEq(
            oUsdcContracts.ovUsdc.totalSupply(),
            oUsdcContracts.ovUsdc.totalReserves(),
            "Invariant violation: investOvUsdc(usdc) totalSupply != totalReserves"
        );
    }

    function getMaxAvailalbleToExit()
        internal
        view
        returns (uint256 maxAvailableToExit)
    {
        ...
        uint256 usdcAvailable = oUsdcContracts
            .lendingClerk
        ...
        returns (uint256 amountOut)
    {
        ...
        uint256 expectedAmountOut = amount
            ...
            .scaleDown(1e12, MorigamiMath.Rounding.ROUND_DOWN);

        assertEq(
            amountOut,
            expectedAmountOut,
            "Invariant violation: exitOvUsdc(usdc) unexpected amountOut"
        );
        assertEq(
            oUsdcContracts.ovUsdc.totalSupply(),
            oUsdcContracts.ovUsdc.totalReserves(),
            "Invariant violation: exitOvUsdc(usdc) totalSupply != totalReserves"
        );
    }
}
