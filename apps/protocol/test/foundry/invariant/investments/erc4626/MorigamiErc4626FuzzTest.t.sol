...
contract ERC4626FuzzTest is ERC4626Test, MorigamiTest {
    ...
    function setUpVault(Init memory init) public override {
        ...
        for (uint i = 0; i < N; i++) {
            ...
            try underlying.mint(user, shares) {} catch {
                vm.assume(false);
            }
            _approve(_underlying_, user, _vault_, shares);

            vm.prank(user);
            try vault.deposit(shares, user) {} catch {
                vm.assume(false);
            }
            ...
            try underlying.mint(user, assets) {} catch {
                vm.assume(false);
            }
        }
