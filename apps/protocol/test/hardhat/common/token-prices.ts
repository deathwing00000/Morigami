...
describe("Token Prices", async () => {
  ...
  before(async () => {
    ...
    tokenPricesInterface = new ethers.utils.Interface(
      JSON.stringify(TokenPrices__factory.abi)
    );
  });

  type TokenPricesArg = string | boolean | BigNumberish;

  const toPrecision = (v: string, dp: number = pricePrecision) =>
    ethers.utils.parseUnits(v, dp);
  ...
  const encodedOraclePrice = (
    oracle: string,
    stalenessThreshold: number
  ): string => encodeFunction("oraclePrice", oracle, stalenessThreshold);
  const encodedGmxVaultPrice = (vault: string, token: string): string =>
    encodeFunction("gmxVaultPrice", vault, token);
  const encodedGlpPrice = (glpManager: string): string =>
    encodeFunction("glpPrice", glpManager);
  const encodedUniV3Price = (pool: string, inQuotedOrder: boolean): string =>
    encodeFunction("univ3Price", pool, inQuotedOrder);
  const encodedTraderJoeBestPrice = (
    joeQuoter: string,
    sellToken: string,
    buyToken: string
  ): string =>
    encodeFunction("traderJoeBestPrice", joeQuoter, sellToken, buyToken);
  const encodedMulPrice = (v1Bytes: string, v2Bytes: string): string =>
    encodeFunction("mul", v1Bytes, v2Bytes);
  const encodedDivPrice = (numerator: string, denominator: string): string =>
    encodeFunction("div", numerator, denominator);
  const encodedScalar = (amount: BigNumberish): string =>
    encodeFunction("scalar", amount);
  const encodedAliasFor = (sourceToken: string): string =>
    encodeFunction("aliasFor", sourceToken);
  const encodedRepricingTokenPrice = (repricingToken: string): string =>
    encodeFunction("repricingTokenPrice", repricingToken);
  const encodedErc4626TokenPrice = (vault: string): string =>
    encodeFunction("erc4626TokenPrice", vault);
  const encodedWstEthRatio = (stEthToken: string): string =>
    encodeFunction("wstEthRatio", stEthToken);
  const encodedMorigamiOraclePrice = (
  ...
  describe("Arbitrum", async () => {
    ...
    async function setup() {
      ...
      tokenPrices = await new TokenPrices__factory(owner).deploy(
        pricePrecision
      );
      gmxVaultAddr = await GMX_GlpManager__factory.connect(
    ...
    beforeEach(async () => {
      ({ tokenPrices, gmxVaultAddr } = await loadFixture(setup));
    });

    it("admin", async () => {
      await shouldRevertNotOwner(
        tokenPrices
          .connect(alan)
          .setTokenPriceFunction(ZERO_ADDRESS, ZERO_ADDRESS)
      );
    });
    ...
    it("eth price", async () => {
      ...
      await expect(
        tokenPrices.setTokenPriceFunction(addresses.weth, encodedEthUsd)
      )
    ...
    it("GLP price with empty setup", async () => {
      ...
      const gmxContracts = await deployGmx(
        owner,
        rewardsPerSec,
        rewardsPerSec,
        rewardsPerSec,
      ...
      const price = await tokenPrices.glpPrice(
    ...
    it("Uniswap V3 Price", async () => {
      ...
      // Not in quoted order
      const encodedEthGmxInverse = encodedUniV3Price(
        addresses.ethGmxUniV3Pool,
        false
      );
      await tokenPrices.setTokenPriceFunction(
        addresses.ethGmxUniV3Pool,
        encodedEthGmxInverse
      );

      const priceInverse = await tokenPrices.tokenPrice(
        addresses.ethGmxUniV3Pool
      );
      expect(priceInverse).to.eq(
        toPrecision("0.034215702404965558514726581213")
      );
    });
    ...
    it("aliasFor()", async () => {
      ...
      expect(gmxPrice)
        .to.eq(aliasPrice)
        .eq(toPrecision("43.099467377390816127490190759359"));
    });

    it("Unknown token", async () => {
      await expect(
        tokenPrices.tokenPrice(addresses.ethUsdOracle)
      ).to.be.revertedWithCustomError(tokenPrices, "FailedPriceLookup");
    });

    it("multi price", async () => {
      // ETH
      const encodedEthUsdOracle = encodedOraclePrice(
        addresses.ethUsdOracle,
        600
      );
      await tokenPrices.setTokenPriceFunction(
        addresses.weth,
      ...
      // GMX
      const encodedEthGmx = encodedUniV3Price(addresses.ethGmxUniV3Pool, true);
      const encodedEthUsdGmx = encodedGmxVaultPrice(
        gmxVaultAddr,
        addresses.weth
      );
      ...
      const price = await tokenPrices.tokenPrices([
        addresses.glp,
        addresses.weth,
        addresses.gmx,
      ]);
    ...
    it("lower precision", async () => {
      ...
      await tokenPricesLower.setTokenPriceFunction(
        addresses.weth,
        encodedEthUsdOracle
      );
      ...
      const price = await tokenPricesLower.tokenPrices([
        addresses.glp,
        addresses.weth,
        addresses.gmx,
      ]);
  ...
  describe("Avalanche", async () => {
    ...
    it("Trader Joe V2 Price", async () => {
      const gmxWavaxPrice = await tokenPrices.traderJoeBestPrice(
        addresses.traderJoeQuoter,
        addresses.gmx,
        addresses.wavax
      );
      expect(gmxWavaxPrice).to.eq(
        toPrecision("4.206218032836104331131753600091")
      );

      const wavaxGmxPrice = await tokenPrices.traderJoeBestPrice(
        addresses.traderJoeQuoter,
        addresses.wavax,
        addresses.gmx
      );
      expect(wavaxGmxPrice).to.eq(
        toPrecision("0.237985638774157491474423269809")
      );

      const wavaxUsdcPrice = await tokenPrices.traderJoeBestPrice(
        addresses.traderJoeQuoter,
        addresses.wavax,
        addresses.usdc
      );
      expect(wavaxUsdcPrice).to.eq(
        toPrecision("18.060913827678474919538505458814")
      );

      const usdcWavaxPrice = await tokenPrices.traderJoeBestPrice(
        addresses.traderJoeQuoter,
        addresses.usdc,
        addresses.wavax
      );
      expect(usdcWavaxPrice).to.eq(
        toPrecision("0.055368183825892921736394525445")
      );

      const aliasAddr = alan.getAddress();
      const encodedWavaxGmx = encodedTraderJoeBestPrice(
        addresses.traderJoeQuoter,
        addresses.wavax,
        addresses.gmx
      );
      ...
      const encodedAvaxUsd = encodedGmxVaultPrice(
        gmxVaultAddr,
        addresses.wavax
      );
  ...
  describe("Local", async () => {
    ...
    async function setup() {
      ...
      reserveToken = await new DummyMintableToken__factory(owner).deploy(
        owner.getAddress(),
        "oToken",
        "oToken",
        18
      );
      repricingToken = await new DummyRepricingToken__factory(owner).deploy(
        owner.getAddress(),
        "ovToken",
        "ovToken",
        reserveToken.address,
        repricingTokenVestingDuration
      );
    ...
    beforeEach(async () => {
      ({ tokenPrices, reserveToken, repricingToken } = await loadFixture(
        setup
      ));
    });
    ...
    it("repricing token price", async () => {
      ...
      const encodedReserveTokenOracle = encodedOraclePrice(
        reserveTokenOracle.address,
        60
      );
      await tokenPrices.setTokenPriceFunction(
        reserveToken.address,
        encodedReserveTokenOracle
      );

      const encodedUsd = encodedRepricingTokenPrice(repricingToken.address);
      await tokenPrices.setTokenPriceFunction(
        repricingToken.address,
        encodedUsd
      );
      ...
      expect(price).to.eq(
        ethers.utils.parseUnits(reserveTokenPrice.toString(), 30)
      );
      ...
      expect(price3).to.eq(
        ethers.utils.parseUnits(updatedPrice.toString(), 30)
      );
    });

    it("ERC-4626 token price", async () => {
      const daiToken = await new DummyMintableToken__factory(owner).deploy(
        owner.getAddress(),
        "DAI",
        "DAI",
        18
      );
      const sDaiToken = await new MockSDaiToken__factory(owner).deploy(
        daiToken.address
      );
      ...
      const daiOracle = await new DummyOracle__factory(owner).deploy(
        nonStaleAnswer,
        8
      );
    ...
    it("negative oracle price", async () => {
      ...
      await expect(
        tokenPrices.tokenPrice(reserveToken.address)
    ...
    it("stale oracle price", async () => {
      ...
      expect(
        await tokenPrices.oraclePrice(reserveTokenOracle.address, 100)
      ).to.eq(
        ethers.utils.parseUnits(
          ethers.utils.formatUnits(price.toString(), 8),
          30
        )
      );
    });
    ...
    it("wstEth/ETH ratio", async () => {
      ...
      await stEth
        .connect(alan)
        .submit(ZERO_ADDRESS, { value: ethers.utils.parseEther("10") });
      await mineForwardSeconds(365 * 86400);
      ...
      expect(await tokenPrices.tokenPrice(wstEthRatioToken)).to.eq(
        ethers.utils.parseUnits("1.040810775512543952", 30)
      );
    });
    ...
    it("wstEth/USD price", async () => {
      ...
      expect(await tokenPrices.tokenPrice(wstEthToken)).to.eq(
        ethers.utils
          .parseUnits("1.040810775512543952", 18)
          .mul(
            ethers.utils
              .parseUnits("0.9998", 6)
              .mul(ethers.utils.parseUnits("2500", 6))
          )
      );
    });

    it("Morigami oracle price", async () => {
      ...
      const origamiOracle = await new MorigamiVolatileChainlinkOracle__factory(
        owner
      ).deploy(
        {
          description: "ETH/USD",
      ...
      await tokenPrices.setTokenPriceFunction(
        ...
        encodedMorigamiOraclePrice(
          origamiOracle.address,
          PriceType.SPOT_PRICE,
          RoundingMode.ROUND_DOWN
        )
      );
      expect(await tokenPrices.tokenPrice(ETH_ADDRESS)).to.eq(
        ethers.utils.parseUnits("2500", 30)
      );
    });
  });
});
