...
describe("Elevated Access Upgradeablity", async () => {
  ...
  beforeEach(async () => {
    ({ elevatedAccessUpgradeable } = await loadFixture(setup));
  });
  ...
  describe("upgradeable", async () => {
    it("construction and init", async () => {
      ...
      // Already initialized
      await expect(
        elevatedAccessUpgradeable.do_init(ownerAddr)
    ...
    it("should upgrade", async () => {
      ...
      await upgrades.validateUpgrade(
        elevatedAccessUpgradeable.address,
        new DummyElevatedAccessUpgradeablev2__factory(owner),
        { kind: "uups" }
      );
      ...
      await expect(
        upgradeUupsProxy(
          elevatedAccessUpgradeable.address,
          undefined,
          new DummyElevatedAccessUpgradeablev2__factory(alan)
        )
      ).to.be.revertedWithCustomError(
        elevatedAccessUpgradeable,
        "InvalidAccess"
      ...
      const v2 = DummyElevatedAccessUpgradeablev2__factory.connect(
        elevatedAccessUpgradeable.address,
        owner
      );
      ...
      await expect(
        v2.connect(alan).authorizeUpgrade()
      ).to.be.revertedWithCustomError(
        elevatedAccessUpgradeable,
        "InvalidAccess"
      );
      ...
      await expect(v2.connect(alan).initialize(ownerAddr)).to.revertedWith(
        "Initializable: contract is already initialized"
      );

      await expect(
        elevatedAccessUpgradeable.MorigamiElevatedAccess_init(ownerAddr)
      ).to.revertedWith("Initializable: contract is not initializing");
      await expect(
        elevatedAccessUpgradeable.MorigamiElevatedAccess_init_unchained(
          ownerAddr
        )
      ).to.revertedWith("Initializable: contract is not initializing");
    });
