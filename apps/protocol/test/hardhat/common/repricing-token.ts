...
import { MorigamiSignerWithAddress, getSigners } from "../signers";

describe("Repricing Token", async () => {
  let reserveToken: MintableToken;
  let repricingToken: DummyRepricingToken;
  let owner: MorigamiSignerWithAddress;
  let operator: MorigamiSignerWithAddress;
  let alan: MorigamiSignerWithAddress;
  let bob: MorigamiSignerWithAddress;
  let spender: MorigamiSignerWithAddress;
  let gov: MorigamiSignerWithAddress;
  let govAddr: string;
  ...
  async function setup() {
    reserveToken = await new DummyMintableToken__factory(gov).deploy(
      govAddr,
      "reserveToken",
      "rToken",
      18
    ...
    repricingToken = await new DummyRepricingToken__factory(gov).deploy(
      govAddr,
      "ovTokenName",
      "ovToken",
  ...
  beforeEach(async () => {
    ({ reserveToken, repricingToken } = await loadFixture(setup));
  });
  ...
  it("Init", async () => {
    ...
    expect(await repricingToken.reservesVestingDuration()).eq(
      reservesVestingDuration
    );
    ...
    expect(await repricingToken.reservesPerShare()).eq(
  ...
  it("admin", async () => {
    await shouldRevertInvalidAccess(
      repricingToken,
      repricingToken.connect(owner).setReservesVestingDuration(0)
    );
    await shouldRevertInvalidAccess(
      repricingToken,
      repricingToken
        .connect(alan)
        .recoverToken(repricingToken.address, alan.getAddress(), 10)
    );
    await shouldRevertInvalidAccess(
      repricingToken,
      repricingToken.connect(alan).addPendingReserves(0)
    );
    ...
    await expect(
      repricingToken.connect(operator).addPendingReserves(0)
  ...
  it("gov can recover tokens", async () => {
    // Add some reserves.
    {
      ...
      await reserveToken
        .connect(operator)
        .approve(repricingToken.address, amount);
    ...
    // Any extra direct dontaions (no add reserves) can be recovered
    {
      ...
      await expect(
        recoverToken(reserveToken, amount + 1, repricingToken, owner)
      )
        .to.be.revertedWithCustomError(repricingToken, "InvalidAmount")
    ...
    {
      const amount = 50;
      const rando = await new DummyMintableToken__factory(gov).deploy(
        govAddr,
        "rando",
        "rando",
        18
  ...
  it("Should setReservesVestingDuration()", async () => {
    ...
    const expectedAccrued = Math.floor(
      (500 * 10_000) / reservesVestingDuration
    );
  ...
  async function addReservesAndMint(
    reservesAmount: BigNumberish,
    repricingAmount: BigNumberish
  ) {
    if (!BigNumber.from(reservesAmount).isZero()) {
      await reserveToken.mint(operator.getAddress(), reservesAmount);
      await reserveToken
        .connect(operator)
        .approve(repricingToken.address, reservesAmount);
  ...
  it("should _issueSharesFromReserves correctly", async () => {
    ...
    await expect(
      repricingToken
        .connect(bob)
        .issueSharesFromReserves(0, alan.getAddress(), 0)
    ...
    await expect(
      repricingToken
        .connect(bob)
        .issueSharesFromReserves(10, alan.getAddress(), 100)
    ).to.revertedWithCustomError(repricingToken, "Slippage");

    // success
    await addReservesAndMint(
    ...
    await expect(
      repricingToken
        .connect(bob)
        .issueSharesFromReserves(100, alan.getAddress(), 0)
    )
    ...
    expect(await reserveToken.balanceOf(repricingToken.address)).eq(
      ethers.utils.parseEther("100").add(100)
    );

    // Burn some of the reserve tokens, so the invariant on balance check no longer holds
    await reserveToken.burn(
    ...
    await reserveToken.mint(
      repricingToken.address,
      ethers.utils.parseEther("10")
    );
    await repricingToken
      .connect(bob)
      .issueSharesFromReserves(100, alan.getAddress(), 0);
  });
  ...
  it("should _redeemReservesFromShares correctly", async () => {
    ...
    await expect(
      repricingToken
        .connect(bob)
        .redeemReservesFromShares(0, alan.getAddress(), 0)
    ).to.revertedWithCustomError(repricingToken, "ExpectedNonZero");
    ...
    await expect(
      repricingToken
        .connect(bob)
        .redeemReservesFromShares(10, alan.getAddress(), 100)
    )
      .to.revertedWithCustomError(repricingToken, "InsufficientBalance")
    ...
    // Issue alan with shares
    {
      ...
      await repricingToken
        .connect(alan)
        .issueSharesFromReserves(1_000, alan.getAddress(), 0);
      await addReservesAndMint(500, 0);
    }
    ...
    await expect(
      repricingToken
        .connect(alan)
        .redeemReservesFromShares(100, bob.getAddress(), 0)
    )
      .to.emit(repricingToken, "Transfer")
    ...
    await expect(
      repricingToken
        .connect(alan)
        .redeemReservesFromShares(100, alan.getAddress(), 0)
    ).to.revertedWithCustomError(repricingToken, "InsufficientBalance");
    ...
    await repricingToken
      .connect(alan)
      .redeemReservesFromShares(100, repricingToken.address, 0);
  ...
  it("should calculate unvestedRewards", async () => {
    ...
    await reserveToken
      .connect(bob)
      .approve(repricingToken.address, vestedAmount);
    await repricingToken
      .connect(bob)
      .issueSharesFromReserves(vestedAmount, bob.getAddress(), 0);
    ...
    let elapsedSecs = 1;
    {
      ...
      expect(uvr.accrued).eq(
        pendingAmount.mul(elapsedSecs).div(reservesVestingDuration)
      );
      expect(uvr.outstanding).eq(pendingAmount.sub(uvr.accrued));
      expect(await repricingToken.totalReserves()).eq(
        vestedAmount.add(uvr.accrued)
      );
    }
  ...
  it("should calculate unvestedRewards for instantaneous vesting", async () => {
    ...
    let elapsedSecs = 1;
    {
      ...
      expect(await repricingToken.totalReserves()).eq(
        vestedAmount.add(pendingAmount)
      );
    }
  ...
  it("Should addPendingReserves()", async () => {
    ...
    await reserveToken
      ...
      .approve(repricingToken.address, reservesAmount);
    await expect(
      repricingToken.connect(operator).addPendingReserves(reservesAmount)
    )
  ...
  it("should checkpoint reserves", async () => {
    ...
    expect(await repricingToken.pendingReserves()).eq(0);
    expect(await repricingToken.lastVestingCheckpoint()).eq(
