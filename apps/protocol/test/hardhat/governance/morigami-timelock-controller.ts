...
import {
  DummyTimelockController,
  DummyTimelockController__factory,
  DummyMintableToken,
  DummyMintableToken__factory,
} from "../../../typechain";
import { ZERO_ADDRESS, mineForwardSeconds } from "../helpers";
...
describe("Morigami Timelock Controller", async () => {
  ...
  async function setup() {
    timelock = await new DummyTimelockController__factory(owner).deploy(
      60,
      [proposer.getAddress()],
      [executor.getAddress()],
      ZERO_ADDRESS
    );
    token = await new DummyMintableToken__factory(owner).deploy(
      timelock.address,
      "reserveToken",
      "rToken",
      18
  ...
  beforeEach(async () => {
    ({ token, timelock } = await loadFixture(setup));
  });

  it("Add Minter", async () => {
    const encoded = token.interface.encodeFunctionData("addMinter", [
  ...
  it("Handle underlying revert string", async () => {
    const encoded = token.interface.encodeFunctionData("transferFrom", [
      await bob.getAddress(),
      timelock.address,
      100000,
    ]);
