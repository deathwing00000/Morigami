...
import { MorigamiSignerWithAddress } from "./signers";
...
export async function recoverToken(
  ...
  await expect(
    from.recoverToken(token.address, await signer.getAddress(), amount)
  )
...
type ERC20Light =
  | StakingLight
  | (StakingLight & { name: () => Promise<string> });
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    if (delta === "gt") {
      assert(
        ...
        `Expected the new balance ${fromAtto(
          newBalances[i]
        )} for "${address}" on token '${tokenName}' to be greater-than the old balance ${fromAtto(
          oldBalances[i]
        )} (check #${i})`
      );
    } else if (delta === "gte") {
      assert(
        ...
        `Expected the new balance ${fromAtto(
          newBalances[i]
        )} for "${address}" on token '${tokenName}' to be greater-than-or-equal to the old balance ${fromAtto(
          oldBalances[i]
        )} (check #${i})`
      );
    } else if (delta === "lt") {
      assert(
        ...
        `Expected the new balance ${fromAtto(
          newBalances[i]
        )} for "${address}" on token '${tokenName}' to be less-than the old balance ${fromAtto(
          oldBalances[i]
        )} (check #${i})`
      );
    } else if (delta === "lte") {
      assert(
        ...
        `Expected the new balance ${fromAtto(
          newBalances[i]
        )} for "${address}" on token '${tokenName}' to be less-than-or-equal to the old balance ${fromAtto(
          oldBalances[i]
        )} (check #${i})`
      );
    } else {
      ...
      assert(
        ...
        `Expected "${address}" on token '${tokenName}' to change balance by ${fromAtto(
          expectedChange
...
export const delta = (a: BigNumberish, b: BigNumberish): BigNumber => {
  return BigNumber.from(a).gt(b)
    ? BigNumber.from(a).sub(b)
    : BigNumber.from(b).sub(a);
};
...
export const expectApproxEqAbs = (
  actual: BigNumberish,
...
export const expectApproxEqRel = (
  actual: BigNumberish,
  expected: BigNumberish,
...
export const expectApproxEqRelPred = (
  expected: BigNumber,
  maxPercentDelta: BigNumberish
) => {
...
const investQuoteTypes =
  "tuple(address fromToken, uint256 fromTokenAmount, uint256 maxSlippageBps, uint256 deadline, uint256 expectedInvestmentAmount, uint256 minInvestmentAmount, bytes underlyingInvestmentQuoteData)";
export const encodeInvestQuoteData = (
  quoteData: IMorigamiInvestment.InvestQuoteDataStruct
): string => {
  return ethers.utils.defaultAbiCoder.encode([investQuoteTypes], [quoteData]);
};

export const decodeInvestQuoteData = (
  encodedQuoteData: string
): IMorigamiInvestment.InvestQuoteDataStruct => {
...
const exitQuoteTypes =
  "tuple(uint256 investmentTokenAmount, address toToken, uint256 maxSlippageBps, uint256 deadline, uint256 expectedToTokenAmount, uint256 minToTokenAmount, bytes underlyingInvestmentQuoteData)";
export const encodeExitQuoteData = (
  quoteData: IMorigamiInvestment.ExitQuoteDataStruct
): string => {
  return ethers.utils.defaultAbiCoder.encode([exitQuoteTypes], [quoteData]);
};

export const decodeExitQuoteData = (
  encodedQuoteData: string
): IMorigamiInvestment.ExitQuoteDataStruct => {
  return ethers.utils.defaultAbiCoder.decode(
    [exitQuoteTypes],
    encodedQuoteData
  )[0];
...
export const signedPermit = async (
  signer: MorigamiSignerWithAddress,
  token: ERC20Permit,
  spender: string,
  amount: BigNumberish,
  deadline: number
...
export const testErc20Permit = async (
  ...
  // Check for expired deadlines
  {
    ...
    const { v, r, s } = await signedPermit(
      signer,
      token,
  ...
  );
  {
    ...
    expect(
      await token.allowance(signer.getAddress(), spender.getAddress())
    ).to.eq(allowanceBefore.add(amount));
  }
...
export async function setExplicitAccess(
  ...
  const access: IMorigamiElevatedAccess.ExplicitAccessStruct[] = fnNames.map(
    (fn) => {
      return {
