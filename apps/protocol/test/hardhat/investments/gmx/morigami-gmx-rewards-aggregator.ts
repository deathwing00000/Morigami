...
import {
  ...
  DummyDex,
  DummyDex__factory,
  MorigamiGmxInvestment,
  MorigamiGmxInvestment__factory,
  MorigamiGlpInvestment,
  MorigamiGlpInvestment__factory,
} from "../../../../typechain";
import { deployGmx, GmxContracts, updateDistributionTime } from "./gmx-helpers";
import { loadFixture } from "@nomicfoundation/hardhat-network-helpers";
import {
  GmxVaultType,
  encodeGlpHarvestParams,
  encodeGmxHarvestParams,
} from "../../../../scripts/deploys/helpers";
...
describe("Morigami GMX Rewards Aggregator", async () => {
  ...
  before(async () => {
    [owner, bob, alan, operator, feeCollector, compoundingFeeCollector, gov] =
      await getSigners();
  ...
  async function setup() {
    ...
    // Set up DEX
    {
      ...
      await gmxContracts.gmxToken.mint(
        dex.address,
        ethers.utils.parseEther("100000")
      );
      await gmxContracts.wrappedNativeToken.mint(
        dex.address,
        ethers.utils.parseEther("100000")
      );
    }
    ...
    // Setup ovTokens
    {
      ...
      ovGmxToken = await new MorigamiInvestmentVault__factory(gov).deploy(
        govAddr,
        "ovGmxToken",
        "ovGmxToken",
        oGmxToken.address,
      ...
      ovGlpToken = await new MorigamiInvestmentVault__factory(gov).deploy(
        govAddr,
        "ovGlpToken",
        "ovGlpToken",
        oGlpToken.address,
    ...
    {
      origamiGmxRewardsAggr = await new MorigamiGmxRewardsAggregator__factory(
        gov
      ).deploy(
        govAddr,
        GmxVaultType.GMX,
      ...
      origamiGlpRewardsAggr = await new MorigamiGmxRewardsAggregator__factory(
        gov
      ).deploy(
        govAddr,
        GmxVaultType.GLP,
  ...
  describe("Admin", async () => {
    it("Construction", async () => {
      expect(await origamiGmxRewardsAggr.gmxManager()).eq(
        origamiGmxManager.address
      );
      expect(await origamiGlpRewardsAggr.gmxManager()).eq(
        origamiGmxManager.address
      );
      ...
      expect(await origamiGmxRewardsAggr.glpManager()).eq(
        origamiGlpManager.address
      );
      expect(await origamiGlpRewardsAggr.glpManager()).eq(
        origamiGlpManager.address
      );

      expect(await origamiGmxRewardsAggr.rewardTokensList()).deep.eq([
        gmxContracts.wrappedNativeToken.address,
        oGmxToken.address,
        oGlpToken.address,
      ]);
      expect(await origamiGlpRewardsAggr.rewardTokensList()).deep.eq([
        gmxContracts.wrappedNativeToken.address,
        oGmxToken.address,
        oGlpToken.address,
      ]);
      ...
      expect(await origamiGmxRewardsAggr.performanceFeeCollector()).eq(
        await compoundingFeeCollector.getAddress()
      );
      expect(await origamiGlpRewardsAggr.performanceFeeCollector()).eq(
        await compoundingFeeCollector.getAddress()
      );
    });
    ...
    const dummyHarvestParams = async (): Promise<string> => {
      ...
      const quote = await oGmxToken.investQuote(
        amount,
        gmxContracts.gmxToken.address,
        100,
        ZERO_DEADLINE
      ...
      const params: MorigamiGmxRewardsAggregator.HarvestGmxParamsStruct = {
        nativeToGmxSwapData: dex.interface.encodeFunctionData("swapToGMX", [
          amount,
        ]),
    ...
    it("admin", async () => {
      ...
      await shouldRevertInvalidAccess(
        ...
        origamiGmxRewardsAggr
          .connect(owner)
          .setPerformanceFeeCollector(await alan.getAddress())
      );
      await shouldRevertInvalidAccess(
        origamiGmxRewardsAggr,
        origamiGmxRewardsAggr
      ...
      await shouldRevertInvalidAccess(
        origamiGmxRewardsAggr,
        origamiGmxRewardsAggr.connect(alan).harvestRewards(harvestParams)
      );

      // Happy Paths
      await expect(
        origamiGmxRewardsAggr.connect(operator).harvestRewards(harvestParams)
      ).to.revertedWith("ERC20: transfer amount exceeds balance");

      await origamiGmxRewardsAggr
        .connect(gov)
        .setMorigamiGmxManagers(
          GmxVaultType.GMX,
          origamiGmxManager.address,
      ...
      await origamiGmxRewardsAggr
        ...
        .setPerformanceFeeCollector(await alan.getAddress());
      await expect(
        origamiGmxRewardsAggr.recoverToken(
    ...
    it("Should setMorigamiGmxManagers()", async () => {
      ...
      await expect(
        origamiGmxRewardsAggr.setMorigamiGmxManagers(
          GmxVaultType.GMX,
          bobAddr,
          alanAddr
        )
    ...
    it("Should setPerformanceFeeCollector()", async () => {
      await expect(
        origamiGmxRewardsAggr.setPerformanceFeeCollector(
  ...
  async function getMorigamiStakedGmxRatios(precision: BigNumber) {
    const stakedGmxFromGmxEarnAccount =
      await gmxContracts.stakedGmxTracker.depositBalances(
        gmxEarnAccount.address,
        gmxContracts.gmxToken.address
      );
    const stakedGmxFromGlpEarnAccount =
      await gmxContracts.stakedGmxTracker.depositBalances(
        primaryGlpEarnAccount.address,
        gmxContracts.gmxToken.address
      );
    const stakedGmxTotal =
      await gmxContracts.stakedGmxTracker.totalDepositSupply(
        gmxContracts.gmxToken.address
      );

    const stakedEsGmxFromGmxEarnAccount =
      await gmxContracts.stakedGmxTracker.depositBalances(
        gmxEarnAccount.address,
        gmxContracts.esGmxToken.address
      );
    const stakedEsGmxFromGlpEarnAccount =
      await gmxContracts.stakedGmxTracker.depositBalances(
        primaryGlpEarnAccount.address,
        gmxContracts.esGmxToken.address
      );
    const stakedEsGmxTotal =
      await gmxContracts.stakedGmxTracker.totalDepositSupply(
        gmxContracts.esGmxToken.address
      );

    const stakedMultPointsFromGmxEarnAccount =
      await gmxContracts.feeGmxTracker.depositBalances(
        gmxEarnAccount.address,
        gmxContracts.multiplierPointsToken.address
      );
    const stakedMultPointsFromGlpEarnAccount =
      await gmxContracts.feeGmxTracker.depositBalances(
        primaryGlpEarnAccount.address,
        gmxContracts.multiplierPointsToken.address
      );
    const stakedMultPointsTotal =
      await gmxContracts.feeGmxTracker.totalDepositSupply(
        gmxContracts.multiplierPointsToken.address
      );

    // esGMX rewards from staked GMX + staked esGMX
    const origamiEsGmxTotal = stakedGmxFromGmxEarnAccount
      .add(stakedGmxFromGlpEarnAccount)
      .add(stakedEsGmxFromGmxEarnAccount)
      .add(stakedEsGmxFromGlpEarnAccount);
    const allEsGmxTotal = stakedGmxTotal.add(stakedEsGmxTotal);
    const expectedEsGmxRatio = origamiEsGmxTotal
      .mul(precision)
      .div(allEsGmxTotal);

    // ETH rewards from staked GMX + staked esGMX + staked mult points
    const origamiEthTotal = origamiEsGmxTotal
      .add(stakedMultPointsFromGmxEarnAccount)
      .add(stakedMultPointsFromGlpEarnAccount);
  ...
  async function getMorigamiStakedGlpRatios(precision: BigNumber) {
    const totalDepositSupply =
      await gmxContracts.feeGlpTracker.totalDepositSupply(
        gmxContracts.glpToken.address
      );
    const primaryGlpRatio = (
      await gmxContracts.feeGlpTracker.depositBalances(
        primaryGlpEarnAccount.address,
        gmxContracts.glpToken.address
      )
    )
      .mul(precision)
      .div(totalDepositSupply);
    const secondaryGlpRatio = (
      await gmxContracts.feeGlpTracker.depositBalances(
        secondaryGlpEarnAccount.address,
        gmxContracts.glpToken.address
      )
  ...
  async function getAggregatorRewardBalances(
    aggregator: MorigamiGmxRewardsAggregator
  ) {
    return {
  ...
  const harvestGlp = async () => {
    ...
    const balancesBefore = await getAggregatorRewardBalances(
      origamiGlpRewardsAggr
    );
    const ethAmountToInvest = harvestableRewards[0].add(
      harvestableRewards[1].div(100)
    );
    const oGlpInvestQuote = await oGlpToken.investQuote(
      ethAmountToInvest,
      gmxContracts.wrappedNativeToken.address,
      100,
      ZERO_DEADLINE
    );
    const oGmxExitQuote = await oGmxToken.exitQuote(
      harvestableRewards[1],
      gmxContracts.gmxToken.address,
      100,
      ZERO_DEADLINE
    );
    const totalOGlpAvailable =
      oGlpInvestQuote.quoteData.expectedInvestmentAmount.add(
        balancesBefore.oGlp
      );
    ...
    const glpHarvestParams: MorigamiGmxRewardsAggregator.HarvestGlpParamsStruct =
      {
        ...
        gmxToNativeSwapData: dex.interface.encodeFunctionData(
          "swapToWrappedNative",
          [harvestableRewards[1]]
        ), // GMX -> wETH
    ...
    if (fee.isZero()) {
      await expect(
        origamiGlpRewardsAggr
          .connect(operator)
          .harvestRewards(encodeGlpHarvestParams(glpHarvestParams), {
            gasLimit: 5000000,
          })
      )
    ...
    const reservesAddedAfterFee = totalOGlpAvailable
      .mul(90)
      .div(100)
      .mul(10_000 - fee.toNumber())
      .div(10_000);
  ...
  const harvestGmx = async () => {
    ...
    const addToReserveAmountPct = 9_000;
    const oGmxInvestQuote = await oGmxToken.investQuote(
      newGmx,
      gmxContracts.gmxToken.address,
      100,
      ZERO_DEADLINE
    ...
    if (fee.isZero()) {
      await expect(
        origamiGmxRewardsAggr
          .connect(operator)
          .harvestRewards(encodeGmxHarvestParams(gmxHarvestParams), {
            gasLimit: 5000000,
          })
      )
    ...
    const reservesAddedAfterFee = addToReserveAmount
      .mul(10_000 - fee.toNumber())
      .div(10_000);
  ...
  describe("harvestableRewards", async () => {
    it("harvestableRewards - GLP only", async () => {
      ...
      let harvestableRewardsGlp =
        await origamiGlpRewardsAggr.harvestableRewards();
      ...
      const precision = ethers.utils.parseEther("1");
      {
        ...
        await gmxContracts.bnbToken
          .connect(bob)
          .approve(await gmxContracts.glpRewardRouter.glpManager(), amount);
        const bobQuote = await origamiGmxManager.investOGlpQuote(
          amount,
          gmxContracts.bnbToken.address,
          ZERO_ADDRESS,
          ZERO_DEADLINE
        ...
        await gmxContracts.glpRewardRouter
          ...
          .mintAndStakeGlp(
            ...
            amount,
            MIN_USDG,
            bobQuote.quoteData.minInvestmentAmount
          );

        // GLP ==> GLP Manager
        const tokenAddr = gmxContracts.bnbToken.address;
        await gmxContracts.bnbToken.mint(primaryGlpEarnAccount.address, amount);
        const origamiQuote = await origamiGlpManager.investOGlpQuote(
          amount,
          tokenAddr,
          ZERO_ADDRESS,
          ZERO_DEADLINE
        );
        await primaryGlpEarnAccount
          .connect(operator)
          .mintAndStakeGlp(
            amount,
            tokenAddr,
            MIN_USDG,
            origamiQuote.quoteData.minInvestmentAmount
          );
      ...
      expectApproxEqRel(
        harvestableRewardsGlp[0],
        ethPerSecond.mul(86402).mul(primaryGlpRatio).div(precision),
        MAX_REL_DELTA
      );
      expectApproxEqRel(
        harvestableRewardsGlp[1],
        esGmxPerSecond.mul(86402).mul(primaryGlpRatio).div(precision),
        MAX_REL_DELTA
      );
      ...
      harvestableRewardsGlp = await origamiGlpRewardsAggr.harvestableRewards();
      const { primaryGlpRatio: primaryGlpRatio2 } =
        await getMorigamiStakedGlpRatios(precision);

      const existingBalances = await getAggregatorRewardBalances(
        origamiGlpRewardsAggr
      );
      expectApproxEqRel(
        harvestableRewardsGlp[0].sub(existingBalances.weth),
        ethPerSecond.mul(86400).mul(primaryGlpRatio2).div(precision),
        MAX_REL_DELTA
      );
      expectApproxEqRel(
        harvestableRewardsGlp[1].sub(existingBalances.oGmx),
        esGmxPerSecond.mul(86400).mul(primaryGlpRatio2).div(precision),
        MAX_REL_DELTA
      );

      const feeCollectorBalanceAfter = await oGlpToken.balanceOf(
        compoundingFeeCollector.getAddress()
      );

      // We left 10% of the amount in to carry over
      expect(
        harvestableRewardsGlp[2]
          .add(reservesAddedAfterFee)
          .add(feeCollectorBalanceAfter)
      ).eq(totalOGlpAvailable);
    });

    it("harvestableRewards - GMX only", async () => {
      // Nothing staked -> nothing earnt
      let harvestableRewardsGmx =
        await origamiGmxRewardsAggr.harvestableRewards();
      ...
      // Morigami applies some GMX
      {
        ...
        await gmxContracts.gmxToken
          .connect(bob)
          .approve(gmxContracts.stakedGmxTracker.address, amount);
      ...
      let { expectedEsGmxRatio, expectedEthRatio } =
        await getMorigamiStakedGmxRatios(precision);
      expectApproxEqRel(
        harvestableRewardsGmx[0],
        ethPerSecond.mul(86400).mul(expectedEthRatio).div(precision),
        MAX_REL_DELTA
      );
      expectApproxEqRel(
        harvestableRewardsGmx[1],
        esGmxPerSecond.mul(86400).mul(expectedEsGmxRatio).div(precision),
        MAX_REL_DELTA
      );
      ...
      harvestableRewardsGmx = await origamiGmxRewardsAggr.harvestableRewards();
      ({ expectedEsGmxRatio, expectedEthRatio } =
        await getMorigamiStakedGmxRatios(precision));
      const existingBalances = await getAggregatorRewardBalances(
        origamiGmxRewardsAggr
      );
      expectApproxEqRel(
        harvestableRewardsGmx[0].sub(existingBalances.weth),
        ethPerSecond.mul(86400).mul(expectedEthRatio).div(precision),
        MAX_REL_DELTA
      );
      expectApproxEqRel(
        harvestableRewardsGmx[1].sub(existingBalances.oGmx),
        esGmxPerSecond.mul(86400).mul(expectedEsGmxRatio).div(precision),
        MAX_REL_DELTA
      );
      expect(harvestableRewardsGmx[2]).eq(0); // No oGLP
    ...
    it("harvestableRewards - GMX & GLP combined", async () => {
      ...
      // Check GLP harvestableRewards
      {
        ...
        expectApproxEqRel(
          harvestableRewardsGlp[0],
          ethPerSecond.mul(86405).mul(primaryGlpRatio).div(precision),
          MAX_REL_DELTA
        );
        expectApproxEqRel(
          harvestableRewardsGlp[1],
          esGmxPerSecond.mul(86405).mul(primaryGlpRatio).div(precision),
          MAX_REL_DELTA
        );
      ...
      // Check GMX harvestableRewards
      {
        ...
        const { expectedEsGmxRatio, expectedEthRatio } =
          await getMorigamiStakedGmxRatios(precision);
        expectApproxEqRel(
          harvestableRewardsGmx[0],
          ethPerSecond.mul(86400).mul(expectedEthRatio).div(precision),
      ...
      // Now GLP aggregator still only gets 50% of staked GLP rewards
      {
        harvestableRewardsGlp =
          await origamiGlpRewardsAggr.harvestableRewards();
        const existingGlpBalances = await getAggregatorRewardBalances(
          origamiGlpRewardsAggr
        );
        const { primaryGlpRatio } = await getMorigamiStakedGlpRatios(precision);

        expectApproxEqRel(
          harvestableRewardsGlp[0].sub(existingGlpBalances.weth),
          ethPerSecond.mul(86400).mul(primaryGlpRatio).div(precision),
          MAX_REL_DELTA
        );
        expectApproxEqRel(
          harvestableRewardsGlp[1].sub(existingGlpBalances.oGmx),
          esGmxPerSecond.mul(86400).mul(primaryGlpRatio).div(precision),
          MAX_REL_DELTA
        );
      ...
      // from both the GMX and GLP manager vs the rest of the pool (bob had 50% originally)
      {
        ...
        harvestableRewardsGmx =
          await origamiGmxRewardsAggr.harvestableRewards();
        const existingGmxBalances = await getAggregatorRewardBalances(
          origamiGmxRewardsAggr
        );
        expectApproxEqRel(
          harvestableRewardsGmx[0].sub(existingGmxBalances.weth),
          ethPerSecond.mul(86400).mul(expectedEthRatio).div(precision),
          MAX_REL_DELTA
        );
        expectApproxEqRel(
          harvestableRewardsGmx[1].sub(existingGmxBalances.oGmx),
          esGmxPerSecond.mul(86400).mul(expectedEsGmxRatio).div(precision),
          MAX_REL_DELTA
        );
        expect(harvestableRewardsGmx[2]).eq(0); // No oGLP
  ...
  describe("projectedRewardRates", async () => {
    it("projectedRewardRates - GMX & GLP combined", async () => {
      ...
      // Check GLP
      {
        ...
        const { primaryGlpRatio, secondaryGlpRatio } =
          await getMorigamiStakedGlpRatios(precision);
        expectApproxEqRel(
          rewardRatesGlp[0],
          removePerfFee(
            ethPerSecond
              .mul(primaryGlpRatio.add(secondaryGlpRatio))
              .div(precision)
          ),
          MAX_REL_DELTA
        );
        expectApproxEqRel(
          rewardRatesGlp[1],
          removePerfFee(esGmxPerSecond.mul(primaryGlpRatio).div(precision)),
          MAX_REL_DELTA
        );
      }
      ...
      // Check GMX
      {
        ...
        expectApproxEqRel(
          rewardRatesGmx[0],
          removePerfFee(ethPerSecond.mul(expectedEthRatio).div(precision)),
          MAX_REL_DELTA
        );
        expectApproxEqRel(
          rewardRatesGmx[1],
          removePerfFee(esGmxPerSecond.mul(expectedEsGmxRatio).div(precision)),
          MAX_REL_DELTA
        );
      }
      ...
      const rewardRatesGmxNoFees =
        await origamiGmxRewardsAggr.projectedRewardRates(false);
      expectApproxEqRel(
        removePerfFee(rewardRatesGmxNoFees[0]),
        rewardRatesGmx[0],
        MAX_REL_DELTA
      );
      expectApproxEqRel(
        removePerfFee(rewardRatesGmxNoFees[1]),
        rewardRatesGmx[1],
        MAX_REL_DELTA
      );
    });
  ...
  describe("harvestRewards", async () => {
    const harvestAndCheckGlp = async () => {
      const reservesBefore = (await ovGlpToken.vestedReserves()).add(
        await ovGlpToken.pendingReserves()
      );
      const feeCollectorBalanceBefore = await oGlpToken.balanceOf(
        compoundingFeeCollector.getAddress()
      );
      const { totalOGlpAvailable, reservesAddedAfterFee } = await harvestGlp();
      const reservesAfter = (await ovGlpToken.vestedReserves()).add(
        await ovGlpToken.pendingReserves()
      );
      ...
      const feesCollected = feeCollectorBalanceAfter.sub(
        feeCollectorBalanceBefore
      );
      expect(balances.oGlp.add(reservesAdded).add(feesCollected)).eq(
        totalOGlpAvailable
      );
    };

    const harvestAndCheckGmx = async () => {
      const reservesBefore = (await ovGmxToken.vestedReserves()).add(
        await ovGmxToken.pendingReserves()
      );
      const feeCollectorBalanceBefore = await oGmxToken.balanceOf(
        compoundingFeeCollector.getAddress()
      );
      const { totalOGmxAvailable, reservesAddedAfterFee } = await harvestGmx();
      const reservesAfter = (await ovGmxToken.vestedReserves()).add(
        await ovGmxToken.pendingReserves()
      );
      const reservesAdded = reservesAfter.sub(reservesBefore);
      ...
      const feeCollectorBalanceAfter = await oGmxToken.balanceOf(
        compoundingFeeCollector.getAddress()
      );
      const feesCollected = feeCollectorBalanceAfter.sub(
        feeCollectorBalanceBefore
      ...
      expectApproxEqRel(
        balances.oGmx.add(reservesAdded).add(feesCollected),
        totalOGmxAvailable,
        MAX_REL_DELTA
      );
  ...
  describe("ZeroEx Custom Error", async () => {
    it("A custom error thrown in the 0x proxy should be handled correctly", async () => {
      ...
      await expect(
        origamiGmxRewardsAggr.connect(operator).harvestRewards(gmxHarvestParams)
      ).to.revertedWithCustomError(dex, "InvalidParam");

      const gmxHarvestParams2 = encodeGmxHarvestParams({
        nativeToGmxSwapData:
          dex.interface.encodeFunctionData("revertNoMessage"), // UnknownSwapError
        oGmxInvestQuoteData: oGmxInvestQuote.quoteData,
        addToReserveAmountPct: 1_000,
      });
      await expect(
        origamiGmxRewardsAggr
          .connect(operator)
          .harvestRewards(gmxHarvestParams2)
      )
