...
describe("Morigami GMX Manager", async () => {
    ...
    describe("Local setup", async () => {
        ...
        async function setupMorigamiGmxManager() {
            ...
            // Setup the GMX Manager/Earn Account
            {
                ...
                await setExplicitAccess(
                    ...
                    ["investOGmx", "exitOGmx", "applyGmx", "harvestRewards", "harvestSecondaryRewards", "investOGlp", "exitOGlp"],
                    true
                );
            ...
            // Setup the GLP Manager/Earn Account
            {
                ...
                await setExplicitAccess(
                    origamiGlpManager,
                    await operator.getAddress(),
                    ["harvestRewards", "harvestSecondaryRewards", "investOGlp", "exitOGlp"],
                    true
                );
        ...
        describe("Admin", async () => {
            ...
            it("admin", async () => {
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).setOGmxRewardsFeeRate(0));
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).setEsGmxVestingRate(0));
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).setSellFeeRate(0));
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).setFeeCollector(ZERO_ADDRESS));
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).setPrimaryEarnAccount(ZERO_ADDRESS));
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).setSecondaryEarnAccount(ZERO_ADDRESS));
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).setRewardsAggregators(ZERO_ADDRESS, ZERO_ADDRESS));
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(alan).recoverToken(gmxContracts.bnbToken.address, alan.getAddress(), 10));
                const paused = {
                ...
                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(alan).setPauser(alan.getAddress(), true));

                await shouldRevertInvalidAccess(origamiGmxManager, origamiGmxManager.connect(owner).initGmxContracts(
                    gmxContracts.gmxRewardRouter.address,
                    gmxContracts.glpRewardRouter.address,
                ));
            ...
            it("Should setOGmxRewardsFeeRate()", async () => {
                await expect(origamiGmxManager.setOGmxRewardsFeeRate(10_001))
            ...
            it("Should setEsGmxVestingRate()", async () => {
                await expect(origamiGmxManager.setEsGmxVestingRate(10_001))
                    .to.revertedWithCustomError(origamiGmxManager, "InvalidParam");
            ...
            it("Should setSellFeeRate()", async () => {
                await expect(origamiGmxManager.setSellFeeRate(10_001))
                    .to.revertedWithCustomError(origamiGmxManager, "InvalidParam");
        ...
        describe("Rewards", async () => {
            ...
            it("harvestRewards - GMX - no vesting", async () => {
                ...
                // With fees
                {
                    ...
                    expect(oGmxRewards2).gte(oGmxRewards.add(expectedEsGmx.mul(70).div(100)).sub(1)); // slight rounding diff
