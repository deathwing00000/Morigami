...
describe("Morigami GLP Investment", async () => {
  ...
  before(async () => {
    [owner, alan, bob, fred, feeCollector, dailyTransferKeeper, gov] =
      await getSigners();
  ...
  async function setup() {
    ...
    await setExplicitAccess(
      ...
      ["transferStakedGlp", "unstakeAndRedeemGlp"],
      true
    );
    ...
    await setExplicitAccess(
      secondaryEarnAccount,
      await dailyTransferKeeper.getAddress(),
    ...
    randoErc20 = await new DummyMintableToken__factory(gov).deploy(
      govAddr,
      "rando",
      "rando",
      18
  ...
  it("constructor", async () => {
    ...
    expect(await oGLP.wrappedNativeToken()).eq(
      gmxContracts.wrappedNativeToken.address
    );
    expect(await oGLP.apiVersion()).eq("0.2.0");
  });

  it("admin", async () => {
    await shouldRevertInvalidAccess(
      oGLP,
      oGLP.connect(owner).setMorigamiGlpManager(ZERO_ADDRESS)
    );
    await shouldRevertInvalidAccess(
      oGLP,
      oGLP
  ...
  it("areInvestmentsPaused/areExitsPaused should be correct", async () => {
    ...
    const glpAmount = ethers.utils.parseEther("10");
    {
      ...
      const investQuote = await oGLP.investQuote(
        investAmount,
        tokenAddr,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await gmxContracts.bnbToken
        .connect(fred)
        .approve(oGLP.address, investAmount);
      await oGLP.connect(fred).investWithToken(investQuote.quoteData);
      await secondaryEarnAccount
        .connect(dailyTransferKeeper)
        .transferStakedGlpOrPause(glpAmount, primaryEarnAccount.address);
  ...
  it("Should only receive eth from the wrappedNative token", async () => {
    await expect(
      ...
      .to.revertedWithCustomError(oGLP, "InvalidSender")
  ...
  it("Should set gmx manager", async () => {
    ...
    const origamiGlpManager2 = await new MorigamiGmxManager__factory(
      owner
    ).deploy(
      govAddr,
      gmxContracts.gmxRewardRouter.address,
  ...
  it("max Exit and Invest", async () => {
    expect(await oGLP.maxInvest(await alan.getAddress())).eq(
      ethers.constants.MaxUint256
    );
    expect(await oGLP.maxInvest(gmxContracts.bnbToken.address)).eq(
      ethers.constants.MaxUint256
    );

    expect(await oGLP.maxExit(await alan.getAddress())).eq(
      ethers.constants.MaxUint256
    );
    expect(await oGLP.maxExit(gmxContracts.bnbToken.address)).eq(
      ethers.constants.MaxUint256
    );
  });
  ...
  describe("Invest oGLP", async () => {
    it("Invest oGLP Quote", async () => {
      ...
      const quote = await oGLP.investQuote(
        amount,
        gmxContracts.bnbToken.address,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      expect(quote.quoteData.minInvestmentAmount).eq(0);
    ...
    it("Invest oGLP Quote - with slippage", async () => {
      ...
      expect(quote.quoteData.deadline).eq(ZERO_DEADLINE);
      expect(quote.quoteData.expectedInvestmentAmount).eq(expectedGlp);
      expect(quote.quoteData.minInvestmentAmount).eq(
        applySlippage(expectedGlp, slippage)
      );
      expect(quote.investFeeBps).deep.eq([35]);
    });
    ...
    it("Invest oGLP with token", async () => {
      ...
      // 0 amount errors
      {
        const manualQuote = {
          ...(
            await oGLP.investQuote(100, tokenAddr, ZERO_SLIPPAGE, ZERO_DEADLINE)
          ).quoteData,
          fromTokenAmount: BN_ZERO,
        };
        await expect(
          oGLP.investWithToken(manualQuote)
      ...
      await expect(oGLP.investWithToken(quote.quoteData)).to.be.revertedWith(
      ...
      // 1 more than the GLP quote amount fails
      {
        const manualQuote = {
          ...
          expectedInvestmentAmount:
            quote.quoteData.expectedInvestmentAmount.add(1),
          minInvestmentAmount: quote.quoteData.expectedInvestmentAmount.add(1),
        };
        await expect(
          oGLP.connect(alan).investWithToken(manualQuote)
      ...
      // Alan has the oGlp
      expect(await oGLP.balanceOf(alan.getAddress())).eq(
        quote.quoteData.expectedInvestmentAmount
      );
      ...
      ).eq(quote.quoteData.expectedInvestmentAmount);
    });
    ...
    it("Invest oGLP with staked GLP", async () => {
      ...
      let glpAmount;
      {
        ...
        await gmxContracts.bnbToken
          .connect(alan)
          .approve(gmxContracts.glpRewardRouter.glpManager(), amount);
        await gmxContracts.glpRewardRouter
          .connect(alan)
          .mintAndStakeGlp(gmxContracts.bnbToken.address, amount, 0, 0);
        glpAmount = await gmxContracts.stakedGlpTracker.balanceOf(
          alan.getAddress()
        );
      }

      // Need to wait for the 15min cooldown first
      await gmxContracts.stakedGlp
        .connect(alan)
        .approve(oGLP.address, glpAmount);
      const quote2 = await oGLP.investQuote(
        glpAmount,
        tokenAddr,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await expect(
        oGLP.connect(alan).investWithToken(quote2.quoteData)
    ...
    it("Invest oGLP with ETH", async () => {
      ...
      await expect(
        oGLP.investWithNative(manualQuote, { value: 0 })
      ).to.be.revertedWithCustomError(oGLP, "ExpectedNonZero");
      ...
      // 1 more than the GLP quote amount fails
      {
        ...
        await expect(
          oGLP.connect(alan).investWithNative(manualQuote, { value: amount })
        ).to.be.revertedWith("GlpManager: insufficient GLP output");
      }
      ...
      expect(ethBalBefore.sub(ethBalAfter)).lte(
        amount.add(ethers.utils.parseEther("0.001"))
      );
  ...
  describe("Exit oGLP", async () => {
    it("Exit Glp Quote", async () => {
      ...
      const amount = ethers.utils.parseEther("500");
      const quote = await oGLP.exitQuote(
        amount,
        gmxContracts.bnbToken.address,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      expect(quote.quoteData.maxSlippageBps).eq(ZERO_SLIPPAGE);
      expect(quote.quoteData.deadline).eq(ZERO_DEADLINE);
      expect(quote.quoteData.expectedToTokenAmount).eq(
        BigNumber.from("1664166666666666666")
      ); // 500/300 - 15bps fee
      expect(quote.quoteData.minToTokenAmount).eq(0);
    ...
    it("Exit Glp Quote - with slippage", async () => {
      ...
      expect(quote.quoteData.minToTokenAmount).eq(
        applySlippage(quote.quoteData.expectedToTokenAmount, slippage)
      );
      expect(quote.quoteData.underlyingInvestmentQuoteData).eq(EmptyBytes);
    });

    async function investAndTransferStakedGlp(
      tokenAddr: string,
      ...
      await expectBalancesChangeBy(
        ...
        [
          gmxContracts.stakedGlpTracker,
          secondaryEarnAccount,
          investQuote.quoteData.expectedInvestmentAmount,
        ]
      );
    ...
    async function transferStakedGlp(amount: BigNumber) {
      ...
      await expectBalancesChangeBy(
        async () => {
          await secondaryEarnAccount
            .connect(dailyTransferKeeper)
            .transferStakedGlp(amount, primaryEarnAccount.address);
        },
    ...
    it("Invest and immediately exit oGLP fails - no GLP in primary", async () => {
      ...
      const exitQuote = await oGLP.exitQuote(
        exitAmount,
        tokenAddr,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await expect(
        oGLP.connect(alan).exitToToken(exitQuote.quoteData, alan.getAddress())
      ...
      await expect(
        oGLP.connect(alan).exitToToken(exitQuote.quoteData, ZERO_ADDRESS)
      )
        .to.revertedWithCustomError(oGLP, "InvalidAddress")
        .withArgs(ZERO_ADDRESS);
      ...
      await expectBalancesChangeBy(
        async () => {
          await expect(
            oGLP
              .connect(alan)
              .exitToToken(exitQuote.quoteData, alan.getAddress())
          )
            .to.emit(oGLP, "Exited")
            .withArgs(
              await alan.getAddress(),
              exitAmount,
              tokenAddr,
              exitQuote.quoteData.expectedToTokenAmount,
              await alan.getAddress()
            );
        },
    ...
    it("Exit oGLP to token", async () => {
      ...
      await expect(
        oGLP.connect(alan).exitToToken(quote.quoteData, alan.getAddress())
      ...
      // 0 amount errors
      {
        ...
        await expect(
          oGLP.connect(alan).exitToToken(manualQuote, alan.getAddress())
        ).to.be.revertedWithCustomError(oGLP, "ExpectedNonZero");
      }
    ...
    it("Exit oGLP to token - with fees", async () => {
      const tokenAddr = gmxContracts.bnbToken.address;
      const investAmount = ethers.utils.parseEther("100");
      await origamiGlpManager.setSellFeeRate(3_000);

      // Ensure there's some volume in the primary account invest investing (going to the secondary) and transferring the primary
      const initialOGlpMint = await investAndTransferStakedGlp(
        tokenAddr,
        investAmount,
        fred
      );
      ...
      expect(await oGLP.totalSupply()).eq(
        initialOGlpMint.add(investQuote.quoteData.expectedInvestmentAmount)
      );
    ...
    it("Exit oGLP to token - with 100% fees", async () => {
      const tokenAddr = gmxContracts.bnbToken.address;
      const investAmount = ethers.utils.parseEther("100");
      await origamiGlpManager.setSellFeeRate(10_000);
    ...
    it("Exit oGLP to staked GLP", async () => {
      ...
      let glpAmount: BigNumber;
      {
        // Alan first invests GLP directly, rather than via Morigami
        {
          ...
          await gmxContracts.bnbToken
            .connect(alan)
            .approve(gmxContracts.glpRewardRouter.glpManager(), investAmount);
          await gmxContracts.glpRewardRouter
            .connect(alan)
            .mintAndStakeGlp(gmxContracts.bnbToken.address, investAmount, 0, 0);
          glpAmount = await gmxContracts.stakedGlpTracker.balanceOf(
            alan.getAddress()
          );
        }
      ...
      await expectBalancesChangeBy(
        async () => {
          const exitQuote2 = await oGLP.exitQuote(
            glpAmount,
            gmxContracts.stakedGlp.address,
            ZERO_SLIPPAGE,
            ZERO_DEADLINE
          ...
          await expect(
            oGLP
              .connect(alan)
              .exitToToken(exitQuote2.quoteData, alan.getAddress())
          )
            .to.emit(oGLP, "Exited")
            .withArgs(
              await alan.getAddress(),
      ...
      await expectBalancesChangeBy(
        async () => {
          ...
          await expect(
            oGLP
              .connect(alan)
              .exitToNative(exitQuote.quoteData, alan.getAddress())
          )
            .to.emit(oGLP, "Exited")
            .withArgs(
              await alan.getAddress(),
    ...
    it("Exit oGLP to ETH", async () => {
      ...
      const exitQuote1 = await oGLP.exitQuote(
        investAmount,
        ZERO_ADDRESS,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await expect(
        oGLP.connect(alan).exitToNative(exitQuote1.quoteData, alan.getAddress())
      ).to.be.revertedWith("ERC20: transfer amount exceeds balance");

      {
        ...
        await expect(
          oGLP.connect(alan).exitToNative(manualQuote, alan.getAddress())
        )
          .to.be.revertedWithCustomError(oGLP, "InvalidToken")
          .withArgs(tokenAddr);
      }
      ...
      await expectBalancesChangeBy(
        async () => {
          await oGLP
            .connect(alan)
            .investWithNative(investQuote.quoteData, { value: investAmount });
        },
        [oGLP, alan, investQuote.quoteData.expectedInvestmentAmount],
        [gmxContracts.stakedGlpTracker, primaryEarnAccount, 0],
        [
      ...
      );
      {
        ...
        const ethBalAfter = await alan.getBalance();
        expect(ethBalAfter.sub(ethBalBefore)).gte(
          exitQuote.quoteData.expectedToTokenAmount.sub(
            ethers.utils.parseEther("0.002")
          )
        );
        expect(ethBalAfter.sub(ethBalBefore)).lt(
          exitQuote.quoteData.expectedToTokenAmount
        );
      }
      ...
      await oGLP.mint(alan.getAddress(), exitAmount);
      await expect(
        oGLP.connect(alan).exitToNative(exitQuote.quoteData, alan.getAddress())
      ).to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");
    });
  });
});
