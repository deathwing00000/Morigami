...
describe("Morigami GMX", async () => {
  ...
  beforeEach(async () => {
    ({ gmxContracts, gmxEarnAccount, origamiGmxManager, oGMX, randoErc20 } =
  ...
  it("admin", async () => {
    const { quoteData: investQuote } = await oGMX.investQuote(
      10,
      gmxContracts.gmxToken.address,
      ZERO_SLIPPAGE,
      ZERO_DEADLINE
    );
    const { quoteData: exitQuote } = await oGMX.exitQuote(
      10,
      gmxContracts.gmxToken.address,
      ZERO_SLIPPAGE,
      ZERO_DEADLINE
    ...
    await expect(
      oGMX.investWithNative(investQuote, { value: 0 })
    ).to.be.revertedWithCustomError(oGMX, "Unsupported");
    await expect(
      oGMX.exitToNative(exitQuote, alan.getAddress())
    ).to.be.revertedWithCustomError(oGMX, "Unsupported");

    await shouldRevertInvalidAccess(
      oGMX,
      oGMX.connect(owner).setMorigamiGmxManager(ZERO_ADDRESS)
    );
    await shouldRevertInvalidAccess(
      oGMX,
      oGMX
    ...
    await expect(
      oGMX.recoverToken(gmxContracts.bnbToken.address, alan.getAddress(), 10)
  ...
  it("areInvestmentsPaused/areExitsPaused should be correct", async () => {
    ...
    await setExplicitAccess(origamiGmxManager, govAddr, ["setPauser"], true);
  ...
  it("Should set gmx manager", async () => {
    ...
    const origamiGmxManager2 = await new MorigamiGmxManager__factory(
      gov
    ).deploy(
      govAddr,
      gmxContracts.gmxRewardRouter.address,
  ...
  it("Should get accepted tokens", async () => {
    ...
    expect(tokens).deep.eq([gmxContracts.gmxToken.address]);
  ...
  describe("Invest/Exit OGmx", async () => {
    it("Invest/Exit quote", async () => {
      // investQuote
      {
        ...
        const quote1 = await oGMX.investQuote(
          100,
          gmxContracts.gmxToken.address,
          ZERO_SLIPPAGE,
          ZERO_DEADLINE
      ...
      // exitQuote
      {
        ...
        const quote2 = await oGMX.exitQuote(
          100,
          gmxContracts.gmxToken.address,
          ZERO_SLIPPAGE,
          ZERO_DEADLINE
    ...
    it("max Exit and Invest", async () => {
      expect(await oGMX.maxInvest(await alan.getAddress())).eq(
        ethers.constants.MaxUint256
      );
      expect(await oGMX.maxInvest(gmxContracts.bnbToken.address)).eq(
        ethers.constants.MaxUint256
      );

      expect(await oGMX.maxExit(await alan.getAddress())).eq(
        ethers.constants.MaxUint256
      );
      expect(await oGMX.maxExit(gmxContracts.bnbToken.address)).eq(
        ethers.constants.MaxUint256
      );
    });
    ...
    it("Invest OGmx with GMX", async () => {
      // 0 fromTokenAmount errors
      {
        ...
        await expect(
          oGMX.investWithToken(manualQuote)
      ...
      const quote = await oGMX.investQuote(
        amount,
        gmxContracts.gmxToken.address,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await expect(oGMX.investWithToken(quote.quoteData)).to.be.revertedWith(
      ...
      expect(await gmxContracts.gmxToken.balanceOf(gmxEarnAccount.address)).eq(
        0
      );
    ...
    it("Invest/Exit OGmx with native fails", async () => {
      const investQuote = await oGMX.investQuote(
        100,
        gmxContracts.gmxToken.address,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await expect(
        oGMX.investWithNative(investQuote.quoteData)
      ).to.be.revertedWithCustomError(oGMX, "Unsupported");

      const exitQuote = await oGMX.exitQuote(
        100,
        gmxContracts.gmxToken.address,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await expect(
        oGMX.exitToNative(exitQuote.quoteData, alan.getAddress())
      ).to.be.revertedWithCustomError(oGMX, "Unsupported");
    });

    it("Exit OGmx to GMX", async () => {
      const amount = ethers.utils.parseEther("100");
      const quote = await oGMX.exitQuote(
        amount,
        gmxContracts.gmxToken.address,
        ZERO_SLIPPAGE,
        ZERO_DEADLINE
      ...
      await expect(
        oGMX.connect(alan).exitToToken(quote.quoteData, alan.getAddress())
      ...
      // 0 investmentTokenAmount errors
      {
        ...
        await expect(
          oGMX.connect(alan).exitToToken(manualQuote, alan.getAddress())
        ).to.be.revertedWithCustomError(origamiGmxManager, "ExpectedNonZero");
      }
      ...
      {
        await expect(
          oGMX.connect(alan).exitToToken(exitQuote.quoteData, alan.getAddress())
        )
          .to.emit(oGMX, "Exited")
          .withArgs(
            await alan.getAddress(),
        ...
        expect(await gmxContracts.gmxToken.balanceOf(alan.getAddress())).eq(
          amount
        );
        expect(
          await gmxContracts.gmxToken.balanceOf(origamiGmxManager.address)
