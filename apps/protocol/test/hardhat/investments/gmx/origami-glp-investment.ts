...
describe("Morigami GLP Investment", async () => {
    ...
    it("max Exit and Invest", async () => {
        expect(await oGLP.maxInvest(await alan.getAddress())).eq(ethers.constants.MaxUint256);
        expect(await oGLP.maxInvest(gmxContracts.bnbToken.address)).eq(ethers.constants.MaxUint256);

        expect(await oGLP.maxExit(await alan.getAddress())).eq(ethers.constants.MaxUint256);
        expect(await oGLP.maxExit(gmxContracts.bnbToken.address)).eq(ethers.constants.MaxUint256);
    });
