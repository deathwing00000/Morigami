...
import {
  ...
  IGmxVester__factory,
  GMX_Vester__factory,
  MorigamiGmxEarnAccount,
  MorigamiGmxEarnAccount__factory,
  MorigamiGmxManager,
  MorigamiGmxManager__factory,
  DummyMorigamiGmxEarnAccount__factory,
} from "../../../../typechain";
...
describe("Morigami GMX Earn Account", async () => {
  let owner: Signer;
  let alan: Signer;
  let bob: Signer;
  let operator: Signer;
  let gov: Signer;
  let govAddr: string;
  let origamiGmxEarnAccount: MorigamiGmxEarnAccount;
  ...
  async function setup() {
    ...
    expect(await gmxContracts.gmxToken.balanceOf(await owner.getAddress())).eq(
      gmxAmount
    );

    esGmxVester = IGmxVester__factory.connect(
    ...
    await setExplicitAccess(
      ...
      [
        ...
        "harvestRewards",
        "transferStakedGlp",
        "transferStakedGlpOrPause",
        "handleRewards",
        "depositIntoEsGmxVesting",
        "withdrawFromEsGmxVesting",
      ],
      true
    );
  ...
  beforeEach(async () => {
    ({ gmxContracts, esGmxVester, origamiGmxEarnAccount, origamiGmxManager } =
      await loadFixture(setup));
  });

  describe("Common Admin", async () => {
    it("admin tests", async () => {
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount
          .connect(owner)
          .initGmxContracts(
      ...
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount.connect(alan).stakeGmx(0)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount.connect(alan).unstakeGmx(0)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount.connect(alan).stakeEsGmx(0)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount.connect(alan).unstakeEsGmx(0)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount
          .connect(alan)
          .mintAndStakeGlp(0, ZERO_ADDRESS, 0, 0)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount
          .connect(alan)
          .unstakeAndRedeemGlp(0, ZERO_ADDRESS, 0, ZERO_ADDRESS)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount.connect(alan).harvestRewards(10_000)
      );
      ...
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount.connect(alan).handleRewards(handleRewardsParams)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount
          .connect(alan)
          .depositIntoEsGmxVesting(ZERO_ADDRESS, 0)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount
          .connect(alan)
          .withdrawFromEsGmxVesting(ZERO_ADDRESS)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount.connect(alan).transferStakedGlp(0, ZERO_ADDRESS)
      );
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        origamiGmxEarnAccount
          .connect(alan)
          .transferStakedGlpOrPause(0, ZERO_ADDRESS)
      );
      ...
      await expect(
        origamiGmxEarnAccount
          .connect(alan)
          .mintAndStakeGlp(0, gmxContracts.bnbToken.address, 0, 0)
      ).to.be.revertedWith("RewardRouter: invalid _amount");
      await expect(
        origamiGmxEarnAccount
          .connect(alan)
      ...
      await origamiGmxEarnAccount
        .connect(alan)
        .handleRewards(handleRewardsParams);
      await expect(
        origamiGmxEarnAccount
          .connect(alan)
          .depositIntoEsGmxVesting(gmxContracts.gmxRewardRouter.gmxVester(), 0)
      ).to.be.revertedWith("Vester: invalid _amount");
      await expect(
        origamiGmxEarnAccount
          .connect(alan)
          .withdrawFromEsGmxVesting(gmxContracts.gmxRewardRouter.gmxVester())
    ...
    it("constructor", async () => {
      expect(await origamiGmxEarnAccount.gmxRewardRouter()).eq(
        gmxContracts.gmxRewardRouter.address
      );
      expect(await origamiGmxEarnAccount.glpRewardRouter()).eq(
        gmxContracts.glpRewardRouter.address
      );
      expect(await origamiGmxEarnAccount.gmxToken()).eq(
        gmxContracts.gmxToken.address
      );
      expect(await origamiGmxEarnAccount.esGmxToken()).eq(
        gmxContracts.esGmxToken.address
      );
      expect(await origamiGmxEarnAccount.wrappedNativeToken()).eq(
        gmxContracts.wrappedNativeToken.address
      );
      expect(await origamiGmxEarnAccount.bnGmxAddr()).eq(
        gmxContracts.multiplierPointsToken.address
      );
      expect(await origamiGmxEarnAccount.stakedGmxTracker()).eq(
        gmxContracts.stakedGmxTracker.address
      );
      expect(await origamiGmxEarnAccount.feeGmxTracker()).eq(
        gmxContracts.feeGmxTracker.address
      );
      expect(await origamiGmxEarnAccount.esGmxVester()).eq(esGmxVester.address);
      expect(await origamiGmxEarnAccount.stakedGlp()).eq(
        gmxContracts.stakedGlp.address
      );
      ...
      const underlyingImplAddress =
        await upgrades.erc1967.getImplementationAddress(
          origamiGmxEarnAccount.address
        );
      const underlyingImpl = MorigamiGmxEarnAccount__factory.connect(
        underlyingImplAddress,
        gov
      );
      expect(await underlyingImpl.gmxToken()).eq(gmxContracts.gmxToken.address);
      expect(await underlyingImpl.esGmxToken()).eq(
        gmxContracts.esGmxToken.address
      );
      expect(await underlyingImpl.wrappedNativeToken()).eq(
        gmxContracts.wrappedNativeToken.address
      );
  ...
  async function checkTrackerBalances(
    ...
    const ethBal = await gmxContracts.wrappedNativeToken.balanceOf(
      gmxEarnAccount
    );
    ...
    const stakedGlp = await gmxContracts.feeGlpTracker.depositBalances(
      gmxEarnAccount,
      gmxContracts.glpToken.address
    );
    const stakedGmx = await gmxContracts.stakedGmxTracker.depositBalances(
      gmxEarnAccount,
      gmxContracts.gmxToken.address
    );
    const stakedEsGmx = await gmxContracts.stakedGmxTracker.depositBalances(
      gmxEarnAccount,
      gmxContracts.esGmxToken.address
    );
    ...
    const expectedStakedGmxPlusEsGmx = BigNumber.from(expectedStakedGmx).add(
      BigNumber.from(expectedStakedEsGmx)
    );

    // The stakedGmxTracker tracks both GMX and esGMX. stakedAmounts() equals the sum of both
    const totalGmxTrackerStaked =
      await gmxContracts.stakedGmxTracker.stakedAmounts(gmxEarnAccount);
  ...
  describe("Staking GMX", async () => {
    it("should stake GMX", async () => {
      ...
      // Not enough GMX
      await expect(
        origamiGmxEarnAccount.connect(operator).stakeGmx(stakeAmount)
      ).to.be.revertedWith("BaseToken: transfer amount exceeds balance");

      await gmxContracts.gmxToken.transfer(
        origamiGmxEarnAccount.address,
        stakeAmount
      );
    ...
    it("should unstake GMX", async () => {
      ...
      // No staked balance
      await expect(
        origamiGmxEarnAccount.connect(operator).unstakeGmx(stakeAmount)
      ...
      await expect(
        origamiGmxEarnAccount.connect(operator).unstakeGmx(unstakeAmount)
      )
  ...
  describe("Buying and staking GLP", async () => {
    it("should buy & stake GLP", async () => {
      ...
      const quote = (
        await origamiGmxManager.investOGlpQuote(
          amount,
          tokenAddr,
          ZERO_SLIPPAGE,
          ZERO_DEADLINE
      ...
      await expect(
        origamiGmxEarnAccount
          ...
          .mintAndStakeGlp(
            ...
            tokenAddr,
            MIN_USDG,
            quote.expectedInvestmentAmount.add(1)
          )
      ...
      expect(
        await gmxContracts.bnbToken.balanceOf(origamiGmxEarnAccount.address)
      ).eq(0);
      expect(
        await gmxContracts.stakedGlpTracker.balanceOf(
          origamiGmxEarnAccount.address
        )
      ).eq(quote.expectedInvestmentAmount);
    });
    ...
    it("should transfer staked GLP", async () => {
      ...
      await expect(
        origamiGmxEarnAccount
          .connect(operator)
          .transferStakedGlp(glpAmount, alan.getAddress())
      ...
      expect(
        await gmxContracts.stakedGlpTracker.balanceOf(alan.getAddress())
      ).eq(glpAmount);
    });
    ...
    it("should transfer staked GLP in cooldown", async () => {
      ...
      await expect(
        origamiGmxEarnAccount
          .connect(operator)
          .transferStakedGlpOrPause(transfer1Amount, alan.getAddress())
      )
      ...
      await expect(
        origamiGmxEarnAccount
          .connect(operator)
          .mintAndStakeGlp(amount, tokenAddr, 0, 0)
      ).to.revertedWithCustomError(
        origamiGmxEarnAccount,
        "GlpInvestmentsPaused"
      );
      ...
      ).eq(glpAmount.sub(transfer1Amount));
      expect(
        await gmxContracts.stakedGlpTracker.balanceOf(alan.getAddress())
      ).eq(transfer1Amount);
      ...
      await expect(
        origamiGmxEarnAccount
          .connect(operator)
          .transferStakedGlpOrPause(transfer2Amount, alan.getAddress())
      )
        .to.emit(origamiGmxEarnAccount, "StakedGlpTransferred")
    ...
    it("should unstake & sell GLP", async () => {
      ...
      await expect(
        origamiGmxEarnAccount
          .connect(operator)
          .unstakeAndRedeemGlp(stakeAmount, tokenAddr, 0, alan.getAddress())
      ).to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

      await gmxContracts.bnbToken.mint(
        origamiGmxEarnAccount.address,
        stakeAmount
      );
      const buyQuote = (
      ...
      await origamiGmxEarnAccount
        ...
        .mintAndStakeGlp(
          ...
          tokenAddr,
          MIN_USDG,
          buyQuote.expectedInvestmentAmount
        );
      ...
      await expect(
        origamiGmxEarnAccount
          .connect(operator)
          .unstakeAndRedeemGlp(unstakeAmount, tokenAddr, 0, alan.getAddress())
      ...
      // Unstake some
      const sellQuote = (
        await origamiGmxManager.exitOGlpQuote(
          unstakeAmount,
          tokenAddr,
          ZERO_SLIPPAGE,
          ZERO_DEADLINE
      ...
      await expect(
        origamiGmxEarnAccount
          ...
          .unstakeAndRedeemGlp(
            unstakeAmount,
            tokenAddr,
            sellQuote.expectedToTokenAmount,
            alan.getAddress()
          )
      )
      ...
      // Alan got the BNB
      expect(await gmxContracts.bnbToken.balanceOf(await alan.getAddress())).eq(
        sellQuote.expectedToTokenAmount
      );
      // Staked amount of GLP has dropped
      const expectedRemainingStaked =
        buyQuote.expectedInvestmentAmount.sub(unstakeAmount);
      expect(
        await gmxContracts.stakedGlpTracker.balanceOf(
          origamiGmxEarnAccount.address
        )
      ).eq(expectedRemainingStaked);

      // And the rest...
      const sellQuote2 = (
        await origamiGmxManager.exitOGlpQuote(
          expectedRemainingStaked,
          tokenAddr,
          ZERO_SLIPPAGE,
          ZERO_DEADLINE
        )
      ...
      await expect(
        origamiGmxEarnAccount
          ...
          .unstakeAndRedeemGlp(
            expectedRemainingStaked,
            tokenAddr,
            sellQuote2.expectedToTokenAmount,
            alan.getAddress()
          )
      )
        .to.emit(gmxContracts.glpRewardRouter, "UnstakeGlp")
      ...
      expect(await gmxContracts.bnbToken.balanceOf(await alan.getAddress())).eq(
        sellQuote.expectedToTokenAmount.add(sellQuote2.expectedToTokenAmount)
      );
      expect(
        await gmxContracts.stakedGlpTracker.balanceOf(
          origamiGmxEarnAccount.address
      ...
      await expect(
        origamiGmxEarnAccount
          ...
          .unstakeAndRedeemGlp(
            unstakeAmount,
            tokenAddr,
            sellQuote.expectedToTokenAmount.add(1),
            alan.getAddress()
          )
  ...
  describe("Harvesting Rewards", async () => {
    async function getMatchingEventArgs(
    ...
    function checkRewardEvents(
      ...
      // since the block time incremented.
      {
        ...
        // ETH from GLP
        expect(eventArgs.wrappedNativeFromGlp).gte(
          harvestableRewardsGlp.wrappedNativeAmount
        );
        expect(eventArgs.wrappedNativeFromGlp).lte(
          harvestableRewardsGlp.wrappedNativeAmount.add(
            ethPerSecond.mul(harvestableEpsilonSecs)
          )
        );

        // ETH from GMX
        expect(eventArgs.wrappedNativeFromGmx).gte(
          harvestableRewardsGmx.wrappedNativeAmount
        );
        expect(eventArgs.wrappedNativeFromGmx).lte(
          harvestableRewardsGmx.wrappedNativeAmount.add(
            ethPerSecond.mul(harvestableEpsilonSecs)
          )
        );
        ...
        expect(eventArgs.esGmxFromGlp).lte(
          harvestableRewardsGlp.esGmxAmount.add(
            esGmxPerSecond.mul(harvestableEpsilonSecs)
          )
        );

        // esGMX from GMX
        expect(eventArgs.esGmxFromGmx).gte(harvestableRewardsGmx.esGmxAmount);
        expect(eventArgs.esGmxFromGmx).lte(
          harvestableRewardsGmx.esGmxAmount.add(
            esGmxPerSecond.mul(harvestableEpsilonSecs)
          )
        );
      }
      ...
      // ETH claimed
      {
        ...
        expect(eventArgs.wrappedNativeFromGlp).gte(
          ethPerSecond.mul(elapsedSecs).mul(multiplier)
        );
        expect(eventArgs.wrappedNativeFromGlp).lte(
          ethPerSecond.mul(elapsedSecs + epsilonSecs).mul(multiplier)
        );
      ...
      // esGMX rewards
      {
        ...
        const esGmxVestingPerSecond = esGmxPerSecond
          .mul(esGmxVestingRate)
          .div(10_000);
        expect(eventArgs.esGmxVesting).gte(
          esGmxVestingPerSecond.mul(elapsedSecs).mul(multiplier)
        );
        expect(eventArgs.esGmxVesting).lte(
          esGmxVestingPerSecond.mul(elapsedSecs + epsilonSecs).mul(multiplier)
        );
      }
      ...
      // Any GMX that have vested (over 1 year)
      {
        ...
        expect(eventArgs.vestedGmx).gte(
          beingVestedAsBN.mul(elapsedSecs).div(one_year)
        );
        expect(eventArgs.vestedGmx).lte(
          beingVestedAsBN.mul(elapsedSecs + epsilonSecs).div(one_year)
        );
      }
    }
    ...
    async function testHarvestRewards(
      ...
      await origamiGmxEarnAccount
        .connect(operator)
        .harvestRewards(requestedEsGmxVestingRate);
      await checkTrackerBalances(
        origamiGmxEarnAccount.address,
        0,
        0,
      ...
      if (useGlp) {
        ...
        const origamiGmxManager = await new MorigamiGmxManager__factory(
          gov
        ).deploy(
          govAddr,
          gmxContracts.gmxRewardRouter.address,
      ...
      await checkTrackerBalances(
        ...
        0,
        0,
        expectedStakedGlpAmount,
        expectedStakedGmxAmount,
        0
      );
      await checkTrackerBalances(await operator.getAddress(), 0, 0, 0, 0, 0, 0);

      const harvestableRewardsGlp =
        await origamiGmxEarnAccount.harvestableRewards(GmxVaultType.GLP);
      const harvestableRewardsGmx =
        await origamiGmxEarnAccount.harvestableRewards(GmxVaultType.GMX);

      // Harvest rewards
      // Ethers messes up the gas estimation on this one for some reason - so need to specify manually.
      const tx = await origamiGmxEarnAccount
        .connect(operator)
        .harvestRewards(requestedEsGmxVestingRate);
      ...
      checkRewardEvents(
        ...
        elapsedSecs,
        10,
        harvestableRewardsGlp,
        harvestableRewardsGmx,
        expectedStakedGlpAmount,
        expectedStakedGmxAmount,
        estimatedEsGmxVestingRate1,
        0
      );

      const totalEth: BigNumber = eventArgs.wrappedNativeFromGmx.add(
        eventArgs.wrappedNativeFromGlp
      );
      const reinvestedEsGmx: BigNumber = eventArgs.esGmxFromGmx
        .add(eventArgs.esGmxFromGlp)
        .sub(eventArgs.esGmxVesting);
      await checkTrackerBalances(
        origamiGmxEarnAccount.address,
        0,
        0,
      ...
      await mineForwardSeconds(elapsedSecs);
      const harvestableRewardsGlp2 =
        await origamiGmxEarnAccount.harvestableRewards(GmxVaultType.GLP);
      const harvestableRewardsGmx2 =
        await origamiGmxEarnAccount.harvestableRewards(GmxVaultType.GMX);

      const tx2 = await origamiGmxEarnAccount
        .connect(operator)
        .harvestRewards(requestedEsGmxVestingRate);
      ...
      checkRewardEvents(
        ...
        elapsedSecs,
        10,
        harvestableRewardsGlp2,
        harvestableRewardsGmx2,
        expectedStakedGlpAmount,
        stakedGmxAndEsGmx,
        estimatedEsGmxVestingRate2,
        esGmxBeingVested
      );

      // The earn account has increased it's amount of staked esGMX and multiplier points again
      const reinvestedEsGmx2: BigNumber = eventArgs2.esGmxFromGmx
        .add(eventArgs2.esGmxFromGlp)
        .sub(eventArgs2.esGmxVesting);
      await checkTrackerBalances(
        origamiGmxEarnAccount.address,
        0,
        0,
      ...
      // The Morigami GMX Manager now has some extra ETH, and also some GMX from the vested esGMX
      const totalEth2: BigNumber = totalEth
        .add(eventArgs2.wrappedNativeFromGmx)
        .add(eventArgs2.wrappedNativeFromGlp);
      await checkTrackerBalances(
        await operator.getAddress(),
        totalEth2,
  ...
  describe("Checking Reward Rates", async () => {
    it("Should get GMX reward rates", async () => {
      ...
      await origamiGmxEarnAccount.connect(operator).stakeGmx(stakeAmount);
      let rewardRatesGlp = await origamiGmxEarnAccount.rewardRates(
        GmxVaultType.GLP
      );
      let rewardRatesGmx = await origamiGmxEarnAccount.rewardRates(
        GmxVaultType.GMX
      );
      ...
      // Bob buys the same amount GMX directly (not via Morigami)
      await gmxContracts.gmxToken.transfer(bob.getAddress(), stakeAmount);
      await gmxContracts.gmxToken
        .connect(bob)
        .approve(gmxContracts.stakedGmxTracker.address, stakeAmount);
      await gmxContracts.gmxRewardRouter.connect(bob).stakeGmx(stakeAmount);

      rewardRatesGlp = await origamiGmxEarnAccount.rewardRates(
        GmxVaultType.GLP
      );
      rewardRatesGmx = await origamiGmxEarnAccount.rewardRates(
        GmxVaultType.GMX
      );
      ...
      // Morigami's GMX now has 50% of the total
    ...
    it("Should get GLP reward rates", async () => {
      ...
      await origamiGmxEarnAccount
        ...
        .mintAndStakeGlp(
          ...
          gmxContracts.bnbToken.address,
          MIN_USDG,
          origamiQuote.expectedInvestmentAmount
        );
      let rewardRatesGlp = await origamiGmxEarnAccount.rewardRates(
        GmxVaultType.GLP
      );
      ...
      await gmxContracts.bnbToken
        .connect(bob)
        .approve(await gmxContracts.glpRewardRouter.glpManager(), stakeAmount);
      const bobQuote = (
        await origamiGmxManager.investOGlpQuote(
          stakeAmount,
          gmxContracts.bnbToken.address,
      ...
      await gmxContracts.glpRewardRouter
        .connect(bob)
        .mintAndStakeGlp(
          gmxContracts.bnbToken.address,
          stakeAmount,
          MIN_USDG,
          bobQuote.expectedInvestmentAmount
        );

      // Now Morigami gets roughly 50% (slightly different GLP quotes)
      const precision = ethers.utils.parseEther("1");
      const expectedRatio = origamiQuote.expectedInvestmentAmount
        .mul(precision)
        .div(
          origamiQuote.expectedInvestmentAmount.add(
            bobQuote.expectedInvestmentAmount
          )
        );
      ...
      expect(rewardRatesGlp.wrappedNativeTokensPerSec).eq(
        ethPerSecond.mul(expectedRatio).div(precision)
      );
      expect(rewardRatesGlp.esGmxTokensPerSec).eq(
        esGmxPerSecond.mul(expectedRatio).div(precision)
      );
    ...
    it("Should get GMX + GLP reward rates", async () => {
      ...
      let origamiGlpQuote;
      {
        ...
        await origamiGmxEarnAccount
          ...
          .mintAndStakeGlp(
            ...
            gmxContracts.bnbToken.address,
            MIN_USDG,
            origamiGlpQuote.expectedInvestmentAmount
          );
      }
      ...
      let bobGlpQuote;
      {
        ...
        await gmxContracts.bnbToken
          .connect(bob)
          .approve(
            await gmxContracts.glpRewardRouter.glpManager(),
            stakeAmount
          );
        bobGlpQuote = (
          await origamiGmxManager.investOGlpQuote(
        ...
        await gmxContracts.glpRewardRouter
          ...
          .mintAndStakeGlp(
            ...
            stakeAmount,
            MIN_USDG,
            bobGlpQuote.expectedInvestmentAmount
          );
      }

      // Now Morigami gets roughly 50% (slightly different GLP quotes)
      const precision = ethers.utils.parseEther("1");
      const expectedRatio = origamiGlpQuote.expectedInvestmentAmount
        .mul(precision)
        .div(
          origamiGlpQuote.expectedInvestmentAmount.add(
            bobGlpQuote.expectedInvestmentAmount
          )
        );
      const expectedGlpEthPerSec = ethPerSecond
        .mul(expectedRatio)
        .div(precision);
      const expectedGlpEsGmxPerSec = esGmxPerSecond
        .mul(expectedRatio)
        .div(precision);
  ...
  describe("Checking Manual GMX calls", async () => {
    it("Should handleRewards()", async () => {
      ...
      const stakedEsGmxBefore =
        await gmxContracts.stakedGmxTracker.depositBalances(
          origamiGmxEarnAccount.address,
          gmxContracts.esGmxToken.address
        );
      ...
      expect(positionsAfter.gmxPositions.stakedEsGmx).gt(
        positionsBefore.gmxPositions.stakedEsGmx
      );
      expect(positionsAfter.gmxPositions.stakedMultiplierPoints).gt(
        positionsBefore.gmxPositions.stakedMultiplierPoints
      );
    ...
    it("Should deposit and withdrawl in vesting", async () => {
      ...
      await origamiGmxEarnAccount
        .connect(operator)
        .handleRewards(handleRewardsParams);
      const esGmxBal = await gmxContracts.esGmxToken.balanceOf(
        origamiGmxEarnAccount.address
      );
      expect(esGmxBal).gt(0);

      // Deposit into vesting
      const vester = GMX_Vester__factory.connect(
        await gmxContracts.gmxRewardRouter.gmxVester(),
        gov
      );
      await origamiGmxEarnAccount
        .connect(operator)
        .depositIntoEsGmxVesting(vester.address, esGmxBal);

      let vestedBal = await vester.getTotalVested(
        origamiGmxEarnAccount.address
      );
      ...
      await origamiGmxEarnAccount
        .connect(operator)
        .withdrawFromEsGmxVesting(vester.address);
  ...
  describe("Positions", async () => {
    async function verifyPositions(
      ...
      const { gmxPositions, glpPositions } =
        await origamiGmxEarnAccount.positions();
    ...
    it("gmx based positions", async () => {
      ...
      // Morigami stakes GMX and also has some on hand unstaked
      ...
      const expectedStakedMultPoints = ethers.utils.parseEther(
        "0.692486364789446981"
      );
      await verifyPositions(
        amount,
        amount,
        BN_ZERO,
        expectedStakedEsGmx,
        expectedStakedMultPoints
      );

      // Unstake some esGMX
      const unstakedEsGmx = ethers.utils.parseEther("1000");
      await origamiGmxEarnAccount.connect(operator).unstakeEsGmx(unstakedEsGmx);
      const expectedStakedMultPoints2 = ethers.utils.parseEther(
        "0.352353920610624382"
      ); // mult points get burnt from unstaking esGMX
      ...
      await origamiGmxEarnAccount
        .connect(operator)
        .depositIntoEsGmxVesting(gmxContracts.gmxVester.address, vestingEsGmx);
      ...
      await mineForwardSeconds(86400);
      const expectedClaimableGmx = ethers.utils.parseEther(
        "0.342465753424657534"
      );
    ...
    it("glp based positions", async () => {
      ...
      // staked esGMX and mult points have now increased after claiming+staking
      const expectedStakedEsGmx = ethers.utils.parseEther(
        "1785.734953013409999999"
      );
      const expectedStakedMultPoints = ethers.utils.parseEther(
        "4.946388424358840807"
      );
      await verifyPositions(
        BN_ZERO,
        BN_ZERO,
      ...
      await origamiGmxEarnAccount.connect(operator).unstakeEsGmx(unstakedEsGmx);
      const expectedStakedMultPoints2 = ethers.utils.parseEther(
        "2.176467880692967311"
      ); // mult points get burnt from unstaking esGMX
      let updatedStakedEsGMX = expectedStakedEsGmx.sub(unstakedEsGmx);
      let claimableNativeFromGmx = ethers.utils.parseEther(
        "0.000041335969999999"
      );
      let claimableEsGmxFromGmx = ethers.utils.parseEther(
        "0.020667989409999999"
      );
      let claimableNativeFromGlp = ethers.utils.parseEther(
        "0.000041335969999999"
      );
      let claimableEsGmxFromGlp = ethers.utils.parseEther(
        "0.020667989409999999"
      );
      await verifyPositions(
        BN_ZERO,
        ...
        updatedStakedEsGMX,
        expectedStakedMultPoints2,
        claimableNativeFromGmx,
        claimableEsGmxFromGmx,
        BN_ZERO,
        BN_ZERO,
        BN_ZERO,
        quote.expectedInvestmentAmount,
        claimableNativeFromGlp,
        claimableEsGmxFromGlp
      );
      ...
      let claimableMultPointsFromGmx = ethers.utils.parseEther(
        "0.000024915491914427"
      );
      await origamiGmxEarnAccount
        .connect(operator)
        .depositIntoEsGmxVesting(gmxContracts.glpVester.address, vestingEsGmx);
      await verifyPositions(
        BN_ZERO,
        BN_ZERO,
        unstakedEsGmx.sub(vestingEsGmx),
        updatedStakedEsGMX,
        expectedStakedMultPoints2,
        claimableNativeFromGmx,
        claimableEsGmxFromGmx,
        claimableMultPointsFromGmx,
        BN_ZERO,
        BN_ZERO,
        quote.expectedInvestmentAmount,
        claimableNativeFromGlp,
      ...
      claimableEsGmxFromGmx = ethers.utils.parseEther(
        "1785.755621002819999998"
      );
      claimableMultPointsFromGmx = ethers.utils.parseEther(
        "2.152723416898517166"
      );

      claimableNativeFromGlp = ethers.utils.parseEther("3.571510479939999999");
      claimableEsGmxFromGlp = ethers.utils.parseEther(
        "1785.755621002819999999"
      );
      const expectedClaimableGmx = ethers.utils.parseEther(
        "0.342465753424657534"
      );
  ...
  describe("upgrade", async () => {
    it("should upgrade() - an existing var is the same", async () => {
      ...
      await expect(
        upgradeUupsProxy(
          origamiGmxEarnAccount.address,
          [gmxContracts.gmxRewardRouter.address],
          new DummyMorigamiGmxEarnAccount__factory(owner)
        )
      ...
      await upgradeUupsProxy(
        origamiGmxEarnAccount.address,
        [gmxContracts.gmxRewardRouter.address],
        new DummyMorigamiGmxEarnAccount__factory(gov)
      );
    ...
    it("should upgrade() with the call - the new storage var is set as expected", async () => {
      ...
      const newAcct = DummyMorigamiGmxEarnAccount__factory.connect(
        origamiGmxEarnAccount.address,
        gov
      );
      ...
      await shouldRevertInvalidAccess(
        origamiGmxEarnAccount,
        newAcct.connect(alan).authorizeUpgrade()
      );
    ...
    it("after upgrading, calling a method on the implementation contract directly doesn't affect the old contract", async () => {
      ...
      const oldUnderlyingImplAddress =
        await upgrades.erc1967.getImplementationAddress(
          origamiGmxEarnAccount.address
        );
      const oldUnderlyingImpl = MorigamiGmxEarnAccount__factory.connect(
        oldUnderlyingImplAddress,
        gov
      );
      ...
      const newContract = DummyMorigamiGmxEarnAccount__factory.connect(
        origamiGmxEarnAccount.address,
        gov
      );

      const newUnderlyingImplAddress =
        await upgrades.erc1967.getImplementationAddress(
          origamiGmxEarnAccount.address
        );
      const newUnderlyingImpl = DummyMorigamiGmxEarnAccount__factory.connect(
        newUnderlyingImplAddress,
        gov
      );

      {
        expect(await newUnderlyingImpl.gmxToken()).eq(
          gmxContracts.gmxToken.address
        );
        expect(await newUnderlyingImpl.esGmxToken()).eq(
          gmxContracts.esGmxToken.address
        );
        expect(await newUnderlyingImpl.wrappedNativeToken()).eq(
          gmxContracts.wrappedNativeToken.address
        );
        // Whereas a non-immutable is zero on the underlying.
      ...
      await shouldRevertInvalidAccess(
        oldUnderlyingImpl,
        oldUnderlyingImpl.proposeNewOwner(await bob.getAddress())
      );
      ...
      expect(await newUnderlyingImpl.newAddr()).not.eq(
        await newContract.newAddr()
      );
    });
  });
});
