...
import { 
    ...
    DummyMorigamiInvestmentManager, DummyMorigamiInvestmentManager__factory,
...
describe("Morigami Investment Vault", async () => {
    let owner: Signer;
    let operator: Signer;
    let alan: Signer;
    let bob: Signer;
    ...
    let ovToken: MorigamiInvestmentVault;
    let tokenPrices: TokenPrices;
    let investmentManager: DummyMorigamiInvestmentManager;
    ...
    async function setup() {
        // Setup oToken
        {
            ...
            underlyingExitToken = await new DummyMintableToken__factory(gov).deploy(govAddr, "exitToken", "exitToken", 18);
        ...
        // Setup ovToken
        {
            ...
            ovToken = await new MorigamiInvestmentVault__factory(gov).deploy(govAddr, "ovToken", "ovToken", oToken.address, tokenPrices.address, 500, vestingDuration);
        ...
        // Setup investment manager
        {
            ...
            investmentManager = await new DummyMorigamiInvestmentManager__factory(gov).deploy(
    ...
    it("constructor", async () => {
        ...
        const numerator = await ovToken.performanceFee();
        expect(numerator.toNumber()).to.eq(500);
        expect(await ovToken.apiVersion()).eq("0.2.0");

        await expect(
            new MorigamiInvestmentVault__factory(gov).deploy(govAddr, "ovToken", "ovToken", oToken.address, tokenPrices.address, 10_001, vestingDuration)
        ).to.revertedWithCustomError(ovToken, "InvalidParam");
    });

    it("admin", async () => {
        await shouldRevertInvalidAccess(ovToken, ovToken.connect(owner).setPerformanceFee(80));
        await shouldRevertInvalidAccess(ovToken, ovToken.connect(alan).addPendingReserves(0));

        // Happy paths
        await ovToken.connect(gov).setPerformanceFee(80);
    });

    it("Should setPerformanceFee()", async () => {
        await expect(ovToken.setPerformanceFee(10_001))
            .to.revertedWithCustomError(ovToken, "InvalidParam");
        await expect(ovToken.setPerformanceFee(80))
    ...
    it("maxInvest and maxExit", async () => {
        // If reserve token, it's uncapped
        expect(await ovToken.maxInvest(oToken.address)).eq(ethers.constants.MaxUint256);
        // Otherwise uses the underlying oToken (dummy is hardcoded to 321e18)
        expect(await ovToken.maxInvest(underlyingInvestToken.address)).eq(ethers.utils.parseEther("321"));

        expect(await ovToken.maxExit(oToken.address)).eq(ethers.constants.MaxUint256);
        expect(await ovToken.maxExit(underlyingInvestToken.address)).eq(ethers.utils.parseEther("654"));
    });
    ...
    it("correctly wrapped invest quote", async () => {
        ...
        // Another allowed token, with fees
        {
            ...
            const underlyingQuote = await oToken.investQuote(amount, underlyingInvestToken.address, ONE_HUNDRED_PCT_IN_BPS, ZERO_DEADLINE);
            const expectedSharesAmount = await ovToken.reservesToShares(underlyingQuote.quoteData.expectedInvestmentAmount);
            expect(expectedSharesAmount).eq(amount.mul(9_900).div(ONE_HUNDRED_PCT_IN_BPS));
        ...
        expect(reservesPerShare).eq(ONE_ETH.mul(11_000).div(ONE_HUNDRED_PCT_IN_BPS));
        ...
        // Another allowed token, with fees. Now expect less shares as the price has increased
        {
            ...
            expect(expectedSharesAmount).eq(amount.mul(9_900).div(ONE_HUNDRED_PCT_IN_BPS).mul(ONE_ETH).div(reservesPerShare));
    ...
    it("correctly wrapped exit quote", async () => {
        ...
        // The reserve token, expected the same amount since no shares
        {
            const amount = ethers.utils.parseEther("100");
            const quote = await ovToken.exitQuote(amount, oToken.address, ZERO_SLIPPAGE, ZERO_DEADLINE);
            expect(quote.quoteData.expectedToTokenAmount).eq(amount);
        ...
        // Another allowed token, with fees
        {
            ...
            const expectedReserveAmount = await ovToken.sharesToReserves(amount);
            expect(quote.quoteData.expectedToTokenAmount).eq(amount.mul(10_000-500).div(10_000));
            const underlyingQuote = await oToken.exitQuote(expectedReserveAmount, underlyingExitToken.address, ONE_HUNDRED_PCT_IN_BPS, ZERO_DEADLINE);
            ...
            expect(quote.quoteData.minToTokenAmount).eq(expectedReserveAmount.mul(10_000-500).div(10_000));
            expect(quote.quoteData.underlyingInvestmentQuoteData).eq(
                encodeExitQuoteData(underlyingQuote.quoteData)
            );
    ...
    it("whitelist checked for investWithToken", async () => {
        const amount = ethers.utils.parseEther("1000");
        const quote = await ovToken.investQuote(amount, oToken.address, ZERO_SLIPPAGE, ZERO_DEADLINE);

        const wrapper: DummyProtocolWrapper = await new DummyProtocolWrapper__factory(gov).deploy();
        await expect(wrapper.connect(bob).investWithToken(ovToken.address, quote.quoteData))
            .to.revertedWithCustomError(ovToken, "InvalidAccess");

        // Get past the whitelist
        await ovToken.setAllowAccount(wrapper.address, true);
        await expect(wrapper.connect(bob).investWithToken(ovToken.address, quote.quoteData))
            .to.revertedWith("ERC20: insufficient allowance");
    });

    it("whitelist checked for investWithNative", async () => {
        const amount = ethers.utils.parseEther("1000");
        const quote = await ovToken.investQuote(amount, ZERO_ADDRESS, ZERO_SLIPPAGE, ZERO_DEADLINE);

        const wrapper: DummyProtocolWrapper = await new DummyProtocolWrapper__factory(gov).deploy();
        await expect(wrapper.connect(bob).investWithNative(ovToken.address, quote.quoteData, {value: amount}))
            .to.revertedWithCustomError(ovToken, "InvalidAccess");

        // Get past the whitelist
        await ovToken.setAllowAccount(wrapper.address, true);
        await wrapper.connect(bob).investWithNative(ovToken.address, quote.quoteData, {value: amount});
    });
    ...
    it("invalid quoteData investWithToken", async () => {       
        const amount = ethers.utils.parseEther("1000");
        await oToken.connect(operator).mint(bob.getAddress(), amount);
        await oToken.connect(bob).approve(ovToken.address, amount);

        const quoteData = (await ovToken.investQuote(amount, underlyingInvestToken.address, ZERO_SLIPPAGE, ZERO_DEADLINE)).quoteData;
        const underlyingQuoteData = decodeInvestQuoteData(quoteData.underlyingInvestmentQuoteData);

        // Invest with a mutated fromToken
        await expect(ovToken.connect(bob).investWithToken({
            ...quoteData,
            underlyingInvestmentQuoteData: encodeInvestQuoteData(
                {
                    ...underlyingQuoteData,
                    fromToken: rewardToken1.address
                }
            )
        })).to.revertedWithCustomError(ovToken, "InvalidToken").withArgs(rewardToken1.address);

        // Invest with a mutated fromTokenAmount
        const badAmount = ethers.utils.parseEther("333");
        await expect(ovToken.connect(bob).investWithToken({
            ...quoteData,
            underlyingInvestmentQuoteData: encodeInvestQuoteData(
                {
                    ...underlyingQuoteData,
                    fromTokenAmount: badAmount
                }
            )
        })).to.revertedWithCustomError(ovToken, "InvalidAmount").withArgs(underlyingInvestToken.address, badAmount);
    });
    ...
    it("correctly wrapped invest with native", async () => {       
        const investWithNative = async () => {
            ...
            expectApproxEqRel(bobEthAfter.sub(bobEthBefore).mul(-1).sub(expectedGas), amount, tolerance(0.015));
    ...
    it("invalid quoteData exitToToken", async () => {   
        await bootstrapReserves();
        const amount = ethers.utils.parseEther("1000");
        ...
        const quoteData = (await ovToken.exitQuote(amount, underlyingExitToken.address, ZERO_SLIPPAGE, ZERO_DEADLINE)).quoteData;
        const underlyingQuoteData = decodeExitQuoteData(quoteData.underlyingInvestmentQuoteData);

        // Exit with a mutated toToken
        await expect(ovToken.connect(bob).exitToToken({
            ...quoteData,
            underlyingInvestmentQuoteData: encodeExitQuoteData(
                {
                    ...underlyingQuoteData,
                    toToken: rewardToken1.address
                }
            )
        }, bob.getAddress())).to.revertedWithCustomError(ovToken, "InvalidToken").withArgs(rewardToken1.address);
    });

    it("exitToToken doesn't depend on underlyingExitQuoteData.investmentTokenAmount", async () => {   
        await bootstrapReserves();
        const amount = ethers.utils.parseEther("1000");
        ...
        // Mutating the underlying quote data's investmentTokenAmount
        // doesn't impact the expected result.
        const underlyingQuoteData = decodeExitQuoteData(quoteData.underlyingInvestmentQuoteData);
        const newQuoteData = {
            ...quoteData,
            underlyingInvestmentQuoteData: encodeExitQuoteData(
        ...
        const expectedReserveAmount = await ovToken.sharesToReserves(amount);
        await expectBalancesChangeBy(async () => { 
            await expect(ovToken.connect(bob).exitToToken(newQuoteData, alan.getAddress()))
                .to.emit(ovToken, "Exited")
                .withArgs(await bob.getAddress(), amount, underlyingExitToken.address, quoteData.expectedToTokenAmount, await alan.getAddress())
                .to.emit(oToken, "Exited")
                .withArgs(ovToken.address, expectedReserveAmount, underlyingExitToken.address, quoteData.expectedToTokenAmount, await alan.getAddress())
            },
            [underlyingExitToken, alan, quoteData.expectedToTokenAmount],
            [underlyingExitToken, bob, 0],
            [underlyingExitToken, oToken, quoteData.expectedToTokenAmount.mul(-1)],
            [ovToken, bob, amount.mul(-1)],
            [oToken, ovToken, expectedReserveAmount.mul(-1)],
        );
    });
    ...
    it("should invest -> addPendingReserves -> exit correctly", async () => {
        ...
        // Now Exit
        {
            ...
            await expect(
                ovToken.connect(bob).exitToToken(quote.quoteData, ZERO_ADDRESS)
            ).to.revertedWithCustomError(ovToken, "InvalidAddress")
    ...
    it("should invest -> addPendingReserves -> exit correctly", async () => {
        ...
        const toExit = BigNumber.from(1);
        const toTransfer = numShares.sub(toExit);
        await ovToken.connect(bob).transfer(await alan.getAddress(), toTransfer);

        // Now Exit
        {
            const quote = await ovToken.exitQuote(toExit, underlyingExitToken.address, ZERO_SLIPPAGE, ZERO_DEADLINE);
            await ovToken.connect(bob).exitToToken(quote.quoteData, bob.getAddress());
        }

        const finalAmount = await underlyingExitToken.balanceOf(bob.getAddress());
        const expectedFinalAmount = BigNumber.from(0);
        expect(finalAmount).to.eq(expectedFinalAmount);
    });
    ...
    it("Calculate APR", async () => {
        ...
        const checkApr = async (weeklyDistribution: number, reserves: number) => {
            ...
            const expectedApr = Math.floor(ONE_HUNDRED_PCT_IN_BPS * annualDistribution / reserves);
