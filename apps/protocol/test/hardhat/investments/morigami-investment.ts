...
import {
  DummyMorigamiInvestment,
  DummyMorigamiInvestment__factory,
  DummyMintableToken__factory,
  DummyMintableToken,
} from "../../../typechain";
import { expectBalancesChangeBy } from "../helpers";
...
describe("Morigami Investment Base Class", async () => {
  ...
  async function setup() {
    underlyingInvestToken = await new DummyMintableToken__factory(gov).deploy(
      govAddr,
      "investToken",
      "investToken",
      18
  ...
  beforeEach(async () => {
    ({ oToken, underlyingInvestToken } = await loadFixture(setup));
  });
