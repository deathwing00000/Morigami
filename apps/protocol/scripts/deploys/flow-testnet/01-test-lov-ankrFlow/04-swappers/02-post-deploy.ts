...
async function updateSwappersSafeBatch() {
  const batch = createSafeBatch(
    1,
    swapperMappings().map((mapping) =>
      setSwapperFunction(mapping.contract, mapping.swapper)
    )
  );
...
async function main() {
  ...
  await mine(
    INSTANCES.SWAPPERS.DIRECT_SWAPPER.whitelistRouter(
      ADDRS.EXTERNAL.KITTY_PUNCH.ROUTER_V2,
      true
    )
  );
