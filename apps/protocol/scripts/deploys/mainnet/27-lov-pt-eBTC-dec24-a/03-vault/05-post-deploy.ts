...
async function updatePricesSafeBatch(contract: TokenPrices) {
  ...
  const batch = createSafeBatch(
    1,
    [
      setTokenPriceFunction(contract, ADDRS.EXTERNAL.ETHERFI.EBTC_TOKEN,
        encodedPrices.eBtcToUsd
      ),
      setTokenPriceFunction(contract, ADDRS.EXTERNAL.PENDLE.EBTC_DEC24.PT_TOKEN,
        encodedPrices.ptToUsd
      ),
      setTokenPriceFunction(contract, ADDRS.LOV_PT_EBTC_DEC24_A.TOKEN,
        encodedPrices.lovTokenToUsd
      ),
    ],
  );
...
async function main() {
  ...
  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.ZEROLEND_BORROW_LEND.setPositionOwner(
      ADDRS.LOV_PT_EBTC_DEC24_A.MANAGER
    ),
  );

  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.MANAGER.setOracles(
      ADDRS.ORACLES.PT_EBTC_DEC24_EBTC,
      ADDRS.ORACLES.PT_EBTC_DEC24_EBTC
    )
  );
  ...
  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.MANAGER.setUserALRange(
      DEFAULT_SETTINGS.LOV_PT_EBTC_DEC24_A.USER_AL_FLOOR,
      DEFAULT_SETTINGS.LOV_PT_EBTC_DEC24_A.USER_AL_CEILING
    )
  );
  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.MANAGER.setRebalanceALRange(
      DEFAULT_SETTINGS.LOV_PT_EBTC_DEC24_A.REBALANCE_AL_FLOOR,
      DEFAULT_SETTINGS.LOV_PT_EBTC_DEC24_A.REBALANCE_AL_CEILING
    )
  );
  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.MANAGER.setSwapper(
      ADDRS.SWAPPERS.DIRECT_SWAPPER
    )
  );

  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.MANAGER.setFeeConfig(
      DEFAULT_SETTINGS.LOV_PT_EBTC_DEC24_A.MIN_DEPOSIT_FEE_BPS,
      DEFAULT_SETTINGS.LOV_PT_EBTC_DEC24_A.MIN_EXIT_FEE_BPS,
      DEFAULT_SETTINGS.LOV_PT_EBTC_DEC24_A.FEE_LEVERAGE_FACTOR
    )
  );

  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.TOKEN.setManager(
      ADDRS.LOV_PT_EBTC_DEC24_A.MANAGER
    )
  );
  
  await mine(
    INSTANCES.LOV_PT_EBTC_DEC24_A.MANAGER.setAllowAll(
      true
    )
  );
